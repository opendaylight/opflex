...
match_wc_init(struct match *match, const struct flow *flow)
{
    ...
    if (flow->dl_type == htons(ETH_TYPE_IPV6)) {
        ...
        for (i = 0; i < FLOW_MAX_MPLS_LABELS; i++) {
            wc->masks.mpls_lse[i] = PAG_BE32_MAX;
...
{
    pag_assert(reg_idx < FLOW_N_REGS);
    flow_wildcards_set_reg_mask(&match->wc, reg_idx, mask);
    match->flow.regs[reg_idx] = value & mask;
}

void
match_set_metadata(struct match *match, pag_be64 metadata)
{
    match_set_metadata_masked(match, metadata, PAG_BE64_MAX);
}

void
match_set_metadata_masked(struct match *match,
                          pag_be64 metadata, pag_be64 mask)
{
...
void
match_set_tun_id(struct match *match, pag_be64 tun_id)
{
    match_set_tun_id_masked(match, tun_id, PAG_BE64_MAX);
}

void
match_set_tun_id_masked(struct match *match, pag_be64 tun_id, pag_be64 mask)
{
...
void
match_set_tun_src(struct match *match, pag_be32 src)
{
    match_set_tun_src_masked(match, src, PAG_BE32_MAX);
}

void
match_set_tun_src_masked(struct match *match, pag_be32 src, pag_be32 mask)
{
...
void
match_set_tun_dst(struct match *match, pag_be32 dst)
{
    match_set_tun_dst_masked(match, dst, PAG_BE32_MAX);
}

void
match_set_tun_dst_masked(struct match *match, pag_be32 dst, pag_be32 mask)
{
...
void
match_set_dl_type(struct match *match, pag_be16 dl_type)
{
    match->wc.masks.dl_type = PAG_BE16_MAX;
...
void
match_set_dl_tci(struct match *match, pag_be16 tci)
{
    match_set_dl_tci_masked(match, tci, htons(0xffff));
}

void
match_set_dl_tci_masked(struct match *match, pag_be16 tci, pag_be16 mask)
{
 ...
 */
void
match_set_dl_vlan(struct match *match, pag_be16 dl_vlan)
{
    flow_set_dl_vlan(&match->flow, dl_vlan);
    if (dl_vlan == htons(OFP10_VLAN_NONE)) {
        match->wc.masks.vlan_tci = PAG_BE16_MAX;
    } else {
 ...
 * plus CFI). */
void
match_set_vlan_vid(struct match *match, pag_be16 vid)
{
...
void
match_set_vlan_vid_masked(struct match *match, pag_be16 vid, pag_be16 mask)
{
    pag_be16 pcp_mask = htons(VLAN_PCP_MASK);
    pag_be16 vid_mask = htons(VLAN_VID_MASK | VLAN_CFI);
...
void
match_set_mpls_lse(struct match *match, int idx, pag_be32 lse)
{
    match->wc.masks.mpls_lse[idx] = PAG_BE32_MAX;
...
void
match_set_mpls_label(struct match *match, int idx, pag_be32 mpls_label)
{
    match->wc.masks.mpls_lse[idx] |= htonl(MPLS_LABEL_MASK);
...
void
match_set_tp_src(struct match *match, pag_be16 tp_src)
{
    match_set_tp_src_masked(match, tp_src, PAG_BE16_MAX);
}

void
match_set_tp_src_masked(struct match *match, pag_be16 port, pag_be16 mask)
{
...
void
match_set_tp_dst(struct match *match, pag_be16 tp_dst)
{
    match_set_tp_dst_masked(match, tp_dst, PAG_BE16_MAX);
}

void
match_set_tp_dst_masked(struct match *match, pag_be16 port, pag_be16 mask)
{
...
void
match_set_tcp_flags(struct match *match, pag_be16 flags)
{
    match_set_tcp_flags_masked(match, flags, PAG_BE16_MAX);
}

void
match_set_tcp_flags_masked(struct match *match, pag_be16 flags, pag_be16 mask)
{
...
void
match_set_nw_src(struct match *match, pag_be32 nw_src)
{
    match->flow.nw_src = nw_src;
    match->wc.masks.nw_src = PAG_BE32_MAX;
}

void
match_set_nw_src_masked(struct match *match,
                        pag_be32 nw_src, pag_be32 mask)
{
...
void
match_set_nw_dst(struct match *match, pag_be32 nw_dst)
{
    match->flow.nw_dst = nw_dst;
    match->wc.masks.nw_dst = PAG_BE32_MAX;
}

void
match_set_nw_dst_masked(struct match *match, pag_be32 ip, pag_be32 mask)
{
...
void
match_set_ipv6_label(struct match *match, pag_be32 ipv6_label)
{
    match->wc.masks.ipv6_label = PAG_BE32_MAX;
    match->flow.ipv6_label = ipv6_label;
}


void
match_set_ipv6_label_masked(struct match *match, pag_be32 ipv6_label,
                            pag_be32 mask)
{
...
static void
format_ip_netmask(struct ds *s, const char *name, pag_be32 ip,
                  pag_be32 netmask)
{
    if (netmask) {
        ds_put_format(s, "%s=", name);
...
format_be16_masked(struct ds *s, const char *name,
                   pag_be16 value, pag_be16 mask)
{
    if (mask != htons(0)) {
        ds_put_format(s, "%s=", name);
        if (mask == PAG_BE16_MAX) {
...
format_be32_masked(struct ds *s, const char *name,
                   pag_be32 value, pag_be32 mask)
{
    if (mask != htonl(0)) {
        ds_put_format(s, "%s=", name);
        if (mask == PAG_BE32_MAX) {
...
format_be64_masked(struct ds *s, const char *name,
                   pag_be64 value, pag_be64 mask)
{
    if (mask != htonll(0)) {
        ds_put_format(s, "%s=%#"PRIx64, name, ntohll(value));
        if (mask != PAG_BE64_MAX) {
...
match_format(const struct match *match, struct ds *s, unsigned int priority)
{
    ...
    if (wc->masks.vlan_tci) {
        pag_be16 vid_mask = wc->masks.vlan_tci & htons(VLAN_VID_MASK);
        pag_be16 pcp_mask = wc->masks.vlan_tci & htons(VLAN_PCP_MASK);
        pag_be16 cfi = wc->masks.vlan_tci & htons(VLAN_CFI);
    ...
    if (f->dl_type == htons(ETH_TYPE_IPV6)) {
        ...
        if (wc->masks.ipv6_label) {
            if (wc->masks.ipv6_label == PAG_BE32_MAX) {
    ...
    if (is_ip_any(f) && f->nw_proto == IPPROTO_TCP && wc->masks.tcp_flags) {
        ...
        if (mask == TCP_FLAGS(PAG_BE16_MAX)) {
