...
odp_set_tunnel_action(const struct nlattr *a, struct flow_tnl *tun_key)
{
    ...
    pag_assert(fitness != ODP_FIT_ERROR);
}

static void
set_arp(struct ofpbuf *packet, const struct pag_key_arp *arp_key)
{
...
{
    enum pag_key_attr type = nl_attr_type(a);
    ...
    case PAG_KEY_ATTR_ETHERNET:
        odp_eth_set_addrs(packet,
                          nl_attr_get_unspec(a, sizeof(struct pag_key_ethernet)));
        break;

    case PAG_KEY_ATTR_IPV4:
        ipv4_key = nl_attr_get_unspec(a, sizeof(struct pag_key_ipv4));
        packet_set_ipv4(packet, ipv4_key->ipv4_src, ipv4_key->ipv4_dst,
                        ipv4_key->ipv4_tos, ipv4_key->ipv4_ttl);
        break;

    case PAG_KEY_ATTR_IPV6:
        ipv6_key = nl_attr_get_unspec(a, sizeof(struct pag_key_ipv6));
    ...
    case PAG_KEY_ATTR_TCP:
        tcp_key = nl_attr_get_unspec(a, sizeof(struct pag_key_tcp));
        packet_set_tcp_port(packet, tcp_key->tcp_src, tcp_key->tcp_dst);
        break;

    case PAG_KEY_ATTR_UDP:
        udp_key = nl_attr_get_unspec(a, sizeof(struct pag_key_udp));
        packet_set_udp_port(packet, udp_key->udp_src, udp_key->udp_dst);
        break;

    case PAG_KEY_ATTR_SCTP:
        sctp_key = nl_attr_get_unspec(a, sizeof(struct pag_key_sctp));
    ...
    case PAG_KEY_ATTR_ARP:
        set_arp(packet, nl_attr_get_unspec(a, sizeof(struct pag_key_arp)));
        break;
                   ...
                   odp_execute_cb dp_execute_action, bool more_actions)
{
    ...
    NL_NESTED_FOR_EACH_UNSAFE (a, left, action) {
        ...
        switch ((enum pag_sample_attr) type) {
        case PAG_SAMPLE_ATTR_PROBABILITY:
            if (random_uint32() >= nl_attr_get_u32(a)) {
                return;
            }
            break;

        case PAG_SAMPLE_ATTR_ACTIONS:
            subactions = a;
            break;

        case PAG_SAMPLE_ATTR_UNSPEC:
        case __PAG_SAMPLE_ATTR_MAX:
        default:
            PAG_NOT_REACHED();
        }
    }
                      ...
                      odp_execute_cb dp_execute_action, bool more_actions)
{
    ...
    NL_ATTR_FOR_EACH_UNSAFE (a, left, actions, actions_len) {
        ...
        case PAG_ACTION_ATTR_PUSH_VLAN: {
            const struct pag_action_push_vlan *vlan = nl_attr_get(a);
