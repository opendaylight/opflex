...
/* Provider interface to dpifs, which provide an interface to an Policy Agent
...
/* Policy Agent datapath interface.
 *
...
static inline void dpif_assert_class(const struct dpif *dpif,
                                     const struct dpif_class *dpif_class)
{
    pag_assert(dpif->dpif_class == dpif_class);
}
...
struct dpif_class {
    ...
    /* Returns the Netlink PID value to supply in PAG_ACTION_ATTR_USERSPACE
     * actions as the PAG_USERSPACE_ATTR_PID attribute's value, for use in
     ...
     * attributes with types PAG_KEY_ATTR_* in the 'put->key_len' bytes
     * starting at 'put->key'.  The associated actions are specified by the
     * Netlink attributes with types PAG_ACTION_ATTR_* in the
     ...
     * attributes with types PAG_KEY_ATTR_* in the 'del->key_len' bytes
     * starting at 'del->key'.
     *
