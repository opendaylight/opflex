 ...
 * internally Policy Agent supports a wider range.) */
void
...
static uint32_t
hash_metadata(pag_be64 metadata_)
{
    uint64_t metadata = (PAG_FORCE uint64_t) metadata_;
    return hash_2words(metadata, metadata >> 32);
}

static struct cls_partition *
find_partition(const struct classifier *cls, pag_be64 metadata, uint32_t hash)
{
...
classifier_replace(struct classifier *cls, struct cls_rule *rule)
{
    ...
    if (!old_rule) {
        ...
        if (minimask_get_metadata_mask(&rule->match.mask) == PAG_BE64_MAX) {
            pag_be64 metadata = miniflow_get_metadata(&rule->match.flow);
...
insert_subtable(struct classifier *cls, const struct minimask *mask)
{
    ...
    subtable->tag = (minimask_get_metadata_mask(mask) == PAG_BE64_MAX
...
static uint32_t
raw_get_prefix(const pag_be32 pr[], unsigned int ofs, unsigned int plen)
{
    uint32_t prefix;
 ...
 * 'pr'". */
static uint32_t
trie_get_prefix(const pag_be32 pr[], unsigned int ofs, unsigned int plen)
{
    if (!plen) {
        return 0;
    }
...
static unsigned int
prefix_equal_bits(uint32_t prefix, unsigned int nbits, const pag_be32 value[],
                  unsigned int ofs)
{
...
static unsigned int
trie_prefix_equal_bits(const struct trie_node *node, const pag_be32 prefix[],
 ...
 * be greater than 31. */
static unsigned int
be_get_bit_at(const pag_be32 value[], unsigned int ofs)
{
...
trie_branch_create(const pag_be32 *prefix, unsigned int ofs, unsigned int plen,
...
mask_set_prefix_bits(struct flow_wildcards *wc, uint8_t be32ofs,
                     unsigned int nbits)
{
    pag_be32 *mask = &((pag_be32 *)&wc->masks)[be32ofs];
    unsigned int i;
...
static struct trie_node **
trie_next_edge(struct trie_node *node, const pag_be32 value[],
               unsigned int ofs)
{
...
trie_next_node(const struct trie_node *node, const pag_be32 value[],
               unsigned int ofs)
{
    return node->edges[be_get_bit_at(value, ofs)];
}
 ...
 */
static unsigned int
trie_lookup_value(const struct trie_node *node, const pag_be32 value[],
                  unsigned int *checkbits)
{
                        ...
                        const struct mf_field *mf)
{
    ...
    for (; u32_ofs < u32_end; ++u32_ofs) {
        uint32_t mask;
        mask = ntohl((PAG_FORCE pag_be32)minimask_get(minimask, u32_ofs));
 ...
 */
static const pag_be32 *
minimatch_get_prefix(const struct minimatch *match, const struct mf_field *mf)
{
    return (PAG_FORCE const pag_be32 *)match->flow.values +
...
{
    const pag_be32 *prefix = minimatch_get_prefix(&rule->match, trie->field);
