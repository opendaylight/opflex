...
static struct pag_mutex netdev_mutex = PAG_MUTEX_INITIALIZER;

/* All created network devices. */
static struct shash netdev_shash PAG_GUARDED_BY(netdev_mutex)
 ...
 * netdev_class_mutex. */
static struct pag_mutex netdev_class_mutex PAG_ACQ_BEFORE(netdev_mutex);

/* Contains 'struct netdev_registered_class'es. */
static struct hmap netdev_classes PAG_GUARDED_BY(netdev_class_mutex)
...
static void restore_all_flags(void *aux PAG_UNUSED);
void update_device_args(struct netdev *, const struct shash *args);

static void
netdev_initialize(void)
    PAG_EXCLUDED(netdev_class_mutex, netdev_mutex)
{
    static struct pagthread_once once = PAGTHREAD_ONCE_INITIALIZER;

    if (pagthread_once_start(&once)) {
        pag_mutex_init_recursive(&netdev_class_mutex);
    ...
    PAG_EXCLUDED(netdev_class_mutex, netdev_mutex)
{
    ...
    pag_mutex_lock(&netdev_class_mutex);
    HMAP_FOR_EACH (rc, hmap_node, &netdev_classes) {
        if (rc->class->run) {
            rc->class->run();
        }
    }
    pag_mutex_unlock(&netdev_class_mutex);
}
...
netdev_lookup_class(const char *type)
    PAG_REQ_RDLOCK(netdev_class_mutex)
{
    struct netdev_registered_class *rc;
    ...
    if (netdev_lookup_class(new_class->type)) {
        ...
        if (!error) {
            ...
            VLOG_ERR("failed to initialize %s network device class: %s",
                     new_class->type, pag_strerror(error));
        }
    }
    pag_mutex_unlock(&netdev_class_mutex);
 ...
 * does not define it) or the datapath internal port name (e.g. pag-system).
 *
        ...
        if (rc) {
            ...
            if (netdev) {
                ...
                if (!error) {
                    ...
                    free(netdev->name);
                    pag_assert(list_is_empty(&netdev->saved_flags_list));
    ...
    if (netdev->netdev_class->set_config) {
        ...
        if (error) {
            VLOG_WARN("%s: could not set configuration (%s)",
                      netdev_get_name(netdev), pag_strerror(error));
        }
        return error;
...
netdev_unref(struct netdev *dev)
    PAG_RELEASES(netdev_mutex)
{
    pag_assert(dev->ref_cnt);
    if (!--dev->ref_cnt) {
...
netdev_rx_recv(struct netdev_rx *rx, struct ofpbuf *buffer)
{
    ...
    pag_assert(buffer->size == 0);
    pag_assert(ofpbuf_tailroom(buffer) >= ETH_TOTAL_MIN);
        ...
        *mtup = 0;
        if (error != EOPNOTSUPP) {
            VLOG_DBG_RL(&rl, "failed to retrieve MTU for network device %s: "
                         "%s", netdev_get_name(netdev), pag_strerror(error));
        }
    }
    return error;
}
 ...
 * as defined in pag-vswitchd.conf.db(5). */
int
...
netdev_arp_lookup(const struct netdev *netdev,
                  pag_be32 ip, uint8_t mac[ETH_ADDR_LEN])
{
...
netdev_get_carrier(const struct netdev *netdev)
{
    ...
    if (error) {
        VLOG_DBG("%s: failed to get network device carrier status, assuming "
                 "down: %s", netdev_get_name(netdev), pag_strerror(error));
        carrier = false;
    }
 ...
 * vswitchd/vswitch.xml (which is built as pag-vswitchd.conf.db(8)).
 *
 ...
 * table in vswitchd/vswitch.xml (which is built as pag-vswitchd.conf.db(8)).
...
{
    pag_assert(rx->netdev->ref_cnt > 0);
    return rx->netdev;
}
...
static void
restore_all_flags(void *aux PAG_UNUSED)
{
    struct shash_node *node;
