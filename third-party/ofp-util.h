...
/* Port numbers. */
enum ofperr ofputil_port_from_ofp11(pag_be32 ofp11_port,
                                    ofp_port_t *ofp10_port);
pag_be32 ofputil_port_to_ofp11(ofp_port_t ofp10_port);
...
pag_be32 ofputil_wcbits_to_netmask(int wcbits);
int ofputil_netmask_to_wcbits(pag_be32 netmask);
...
/* Bitmap of OpenFlow versions that Policy Agent supports. */
...
pag_be16 ofputil_dl_type_to_openflow(pag_be16 flow_dl_type);
pag_be16 ofputil_dl_type_from_openflow(pag_be16 ofp_dl_type);
...
enum ofputil_flow_mod_flags {
     ...
     *
     * (OFPUTIL_FF_EMERG would be here too, if PAG supported it.) */
...
struct ofputil_flow_mod {
    ...
    pag_be64 cookie;         /* Cookie bits to match. */
    pag_be64 cookie_mask;    /* 1-bit in each 'cookie' bit to match. */
     ...
     */
    pag_be64 new_cookie;     /* New cookie to install or UINT64_MAX. */
 ...
 */
enum PAG_PACKED_ENUM ofputil_action_code {
...
bool action_outputs_to_port(const union ofp_action *, pag_be16 port);
