...
static void
unixctl_help(struct unixctl_conn *conn, int argc PAG_UNUSED,
...
static void
unixctl_version(struct unixctl_conn *conn, int argc PAG_UNUSED,
                const char *argv[] PAG_UNUSED, void *aux PAG_UNUSED)
{
                         ...
                         unixctl_cb_func *cb, void *aux)
{
    ...
    pag_assert(!lookup || lookup->cb == cb);
 ...
 * "pag-appctl --target=<program>" will fail.)
 *
...
unixctl_server_create(const char *path, struct unixctl_server **serverp)
{
    ...
    *serverp = NULL;
    ...
    if (path) {
        char *abs_path = abs_file_name(pag_rundir(), path);
        punix_path = xasprintf("punix:%s", abs_path);
        free(abs_path);
    } else {
        punix_path = xasprintf("punix:%s/%s.%ld.ctl", pag_rundir(),
                               program_name, (long int) getpid());
    }

    error = pstream_open(punix_path, &listener, 0);
    if (error) {
        pag_error(error, "could not initialize control socket %s", punix_path);
        goto exit;
    }
...
unixctl_client_create(const char *path, struct jsonrpc **client)
{
    ...
    *client = NULL;
    ...
    abs_path = abs_file_name(pag_rundir(), path);
