...
set_nonblocking(int fd)
{
    ...
    if (flags != -1) {
        if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) != -1) {
            ...
            VLOG_ERR("fcntl(F_SETFL) failed: %s", pag_strerror(errno));
            return errno;
        }
    } else {
        VLOG_ERR("fcntl(F_GETFL) failed: %s", pag_strerror(errno));
        return errno;
    }
#else
    unsigned long arg = 1;
 ...
 *
 * Most Policy Agent code should not use this because it causes deadlocks:
 * getaddrinfo() sends out a DNS request but that starts a new flow for which
 * PAG must set up a flow, but it can't because it's waiting for a DNS reply.
                      ...
                      int *dirfdp)
{
    ...
    if (dirfd < 0) {
        ...
        VLOG_WARN_RL(&rl, "%s: open failed (%s)", dir, pag_strerror(error));
        free(dir);
                         ...
                         char linkname[MAX_UN_LEN + 1])
{
    ...
    for (i = 0; i < 1000; i++) {
        ...
        len = snprintf(linkname, MAX_UN_LEN + 1,
                       "%s/pag-un-c-%"PRIu32, tmpdir, random_uint32());
                 ...
                 int *dirfdp, char linkname[MAX_UN_LEN + 1])
{
    ...
    *dirfdp = -1;
    ...
    pag_strzcpy(un->sun_path, name, sizeof un->sun_path);
    ...
    if (fd >= 0) {
        if (fsync(fd)) {
            if (errno == EINVAL || errno == EROFS) {
                ...
                error = errno;
                VLOG_ERR("%s: fsync failed (%s)", dir, pag_strerror(error));
            }
        }
        close(fd);
    } else {
        error = errno;
        VLOG_ERR("%s: open failed (%s)", dir, pag_strerror(error));
    }
    free(dir);
#endif
...
{
    if (pipe(fds)) {
        VLOG_FATAL("failed to create pipe (%s)", pag_strerror(errno));
    }
}
...
#ifndef _WIN32
    if (fstat(fd, &s)) {
        ds_put_format(&string, "fstat failed (%s)", pag_strerror(errno));
                    ...
                    const char *cmd_name)
{
    ...
    pag_strzcpy(ifr->ifr_name, name, sizeof ifr->ifr_name);
...
{
    pag_assert(bufsize >= SS_NTOP_BUFSIZE);
    if (ss->ss_family == AF_INET) {
        const struct sockaddr_in *sin
            = ALIGNED_CAST(const struct sockaddr_in *, ss);
 ...
 *
 * pag_strerror() calls strerror_r() and would not get the correct error
 * string for Windows sockets, but is good for POSIX. */
const char *
sock_strerror(int error)
{
#ifdef _WIN32
    return pag_format_message(error);
#else
    return pag_strerror(error);
#endif
}
