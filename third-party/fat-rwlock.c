...
slot_destructor(void *slot_)
{
    ...
    pag_mutex_lock(&rwlock->mutex);
    free_slot(slot);
    pag_mutex_unlock(&rwlock->mutex);
}
...
{
    pagthread_key_create(&rwlock->key, slot_destructor);
    pag_mutex_init(&rwlock->mutex);
    pag_mutex_lock(&rwlock->mutex);
...
fat_rwlock_destroy(struct fat_rwlock *rwlock)
{
    ...
    pagthread_key_delete(rwlock->key);

    LIST_FOR_EACH_SAFE (slot, next, list_node, &rwlock->threads) {
        free_slot(slot);
    }
    pag_mutex_destroy(&rwlock->mutex);
}
...
fat_rwlock_get_slot__(struct fat_rwlock *rwlock)
{
    ...
    /* Fast path. */
    slot = pagthread_getspecific(rwlock->key);
    if (slot) {
        return slot;
    }
    ...
    pagthread_setspecific(rwlock->key, slot);
    ...
    PAG_NO_THREAD_SAFETY_ANALYSIS
{
    ...
    case 0:
        error = pag_mutex_trylock(&this->mutex);
        if (error) {
            return error;
        }
        /* fall through */
    ...
    PAG_NO_THREAD_SAFETY_ANALYSIS
{
    ...
    pag_assert(!this->depth);
    this->depth = UINT_MAX;

    pag_mutex_lock(&rwlock->mutex);
    LIST_FOR_EACH (slot, list_node, &rwlock->threads) {
        pag_mutex_lock(&slot->mutex);
    }
}
