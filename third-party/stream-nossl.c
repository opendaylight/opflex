...
/* Dummy function definitions, used when PAG is built without OpenSSL. */
...
{
    VLOG_FATAL("%s specified but Policy Agent was built without SSL support",
               detail);
}
...
void
stream_ssl_set_ca_cert_file(const char *file_name, bool bootstrap PAG_UNUSED)
{
    if (file_name != NULL) {
