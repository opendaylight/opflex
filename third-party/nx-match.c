        ...
        if (value[i] & ~mask[i]) {
            if (!VLOG_DROP_WARN(&rl)) {
                ...
                VLOG_WARN_RL(&rl, "NXM/OXM entry %s has 1-bits in value for "
                             ...
                             "of PAG may report this as an OpenFlow error.)",
                             s);
                break;
            }
        }
...
nx_pull_raw(const uint8_t *p, unsigned int match_len, bool strict,
            struct match *match, pag_be64 *cookie, pag_be64 *cookie_mask)
{
    uint32_t header;

    pag_assert((cookie != NULL) == (cookie_mask != NULL));
...
nx_pull_match__(struct ofpbuf *b, unsigned int match_len, bool strict,
                struct match *match,
                pag_be64 *cookie, pag_be64 *cookie_mask)
{
    uint8_t *p = NULL;
...
{
    pag_be32 n_header = htonl(header);
...
static void
nxm_put_16(struct ofpbuf *b, uint32_t header, pag_be16 value)
{
    nxm_put_header(b, header);
    ofpbuf_put(b, &value, sizeof value);
}
...
static void
nxm_put_16w(struct ofpbuf *b, uint32_t header, pag_be16 value, pag_be16 mask)
{
    nxm_put_header(b, header);
    ofpbuf_put(b, &value, sizeof value);
    ofpbuf_put(b, &mask, sizeof mask);
...
static void
nxm_put_16m(struct ofpbuf *b, uint32_t header, pag_be16 value, pag_be16 mask)
{
    switch (mask) {
    case 0:
        break;
...
static void
nxm_put_32(struct ofpbuf *b, uint32_t header, pag_be32 value)
{
    nxm_put_header(b, header);
    ofpbuf_put(b, &value, sizeof value);
}
...
static void
nxm_put_32w(struct ofpbuf *b, uint32_t header, pag_be32 value, pag_be32 mask)
{
    nxm_put_header(b, header);
    ofpbuf_put(b, &value, sizeof value);
    ofpbuf_put(b, &mask, sizeof mask);
...
static void
nxm_put_32m(struct ofpbuf *b, uint32_t header, pag_be32 value, pag_be32 mask)
{
    switch (mask) {
    case 0:
        break;
...
static void
nxm_put_64(struct ofpbuf *b, uint32_t header, pag_be64 value)
{
    nxm_put_header(b, header);
    ofpbuf_put(b, &value, sizeof value);
}
...
static void
nxm_put_64w(struct ofpbuf *b, uint32_t header, pag_be64 value, pag_be64 mask)
{
    nxm_put_header(b, header);
    ofpbuf_put(b, &value, sizeof value);
    ofpbuf_put(b, &mask, sizeof mask);
...
static void
nxm_put_64m(struct ofpbuf *b, uint32_t header, pag_be64 value, pag_be64 mask)
{
    switch (mask) {
    case 0:
        break;
           ...
           pag_be64 cookie, pag_be64 cookie_mask)
{
    ...
    /* 802.1Q. */
    if (oxm) {
        pag_be16 VID_CFI_MASK = htons(VLAN_VID_MASK | VLAN_CFI);
        pag_be16 vid = flow->vlan_tci & VID_CFI_MASK;
        pag_be16 mask = match->wc.masks.vlan_tci & VID_CFI_MASK;
...
oxm_put_match(struct ofpbuf *b, const struct match *match)
{
    ...
    pag_be64 cookie = htonll(0), cookie_mask = htonll(0);
...
nx_match_from_string_raw(const char *s, struct ofpbuf *b)
{
    ...
    for (s += strspn(s, ", "); *s; s += strspn(s, ", ")) {
        ...
        name_len = strcspn(s, "(");
        if (s[name_len] != '(') {
            pag_fatal(0, "%s: missing ( at end of nx_match", full_s);
        }

        header = parse_nxm_field_name(name, name_len);
        if (!header) {
            pag_fatal(0, "%s: unknown field `%.*s'", full_s, name_len, s);
        }
        ...
        if (n != nxm_field_bytes(header)) {
            pag_fatal(0, "%.2s: hex digits expected", s);
        }
        if (NXM_HASMASK(header)) {
            s += strspn(s, " ");
            if (*s != '/') {
                pag_fatal(0, "%s: missing / in masked field %.*s",
        ...
        s += strspn(s, " ");
        if (*s != ')') {
            pag_fatal(0, "%s: missing ) following field %.*s",
                      full_s, name_len, name);
        }
        s++;
    }
...

/* Parses 's' as a "move" action, in the form described in pag-ofctl(8), into
 * '*move'.
 *
...
/* Parses 's' as a "load" action, in the form described in pag-ofctl(8), into
 * '*load'.
 *
 * Returns NULL if successful, otherwise a malloc()'d string describing the
 * error.  The caller is responsible for freeing the returned string. */
             ...
             struct flow *flow, struct flow_wildcards *wc)
{
    ...
    pag_be64 src_data_be = htonll(src_data);
