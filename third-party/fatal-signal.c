...
fatal_signal_init(void)
{
    ...
    if (!inited) {
        ...
        pag_mutex_init_recursive(&mutex);
#ifndef _WIN32
        xpipe_nonblocking(signal_fds);
#else
        wevent = CreateEvent(NULL, TRUE, FALSE, NULL);
        if (!wevent) {
            char *msg_buf = pag_lasterror_to_string();
        ...
        for (i = 0; i < ARRAY_SIZE(fatal_signals); i++) {
            ...
            if (old_sa.sa_handler == SIG_DFL
                ...
                VLOG_FATAL("signal failed (%s)", pag_strerror(errno));
            }
#else
...
fatal_signal_unlink_file_now(const char *file)
{
    ...
    if (error) {
        VLOG_WARN("could not unlink \"%s\" (%s)", file, pag_strerror(error));
    }
...
static void
unlink_files(void *aux PAG_UNUSED)
{
    do_unlink_files();
}

static void
cancel_files(void *aux PAG_UNUSED)
{
    sset_clear(&files);
    added_hook = false;
}
