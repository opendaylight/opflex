...
bool
route_table_get_name(pag_be32 ip, char name[IFNAMSIZ])
{
    int ifindex;
    ...
    if (route_table_get_ifindex(ip, &ifindex)) {
        ...
        if (nn) {
            pag_strlcpy(name, nn->ifname, IFNAMSIZ);
            return true;
        }
    }
 ...
 * Returns true if successful, otherwise false. */
bool
route_table_get_ifindex(pag_be32 ip_, int *ifindex)
{
...
static void
route_table_change(const struct route_table_msg *change PAG_UNUSED,
                   void *aux PAG_UNUSED)
{
...
name_table_reset(void)
{
    ...
    while (nl_dump_next(&dump, &reply, &buf)) {
        ...
        if (rtnetlink_link_parse(&reply, &change)
            ...
            pag_strlcpy(nn->ifname, change.ifname, IFNAMSIZ);
...
static void
name_table_change(const struct rtnetlink_link_change *change PAG_UNUSED,
                  void *aux PAG_UNUSED)
{
