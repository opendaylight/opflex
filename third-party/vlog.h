 ...
 *
 * pag-appctl(8) defines each of the log levels. */
#define VLOG_LEVELS                             \
...
#define VLOG_FACILITIES                                                 \
    VLOG_FACILITY(SYSLOG, "pag|%05N|%c%T|%p|%m")                        \
...
struct vlog_rate_limit {
    ...
    struct pag_mutex mutex;     /* Mutual exclusion for rate limit. */
};
