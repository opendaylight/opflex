...
static const uint8_t eth_addr_broadcast[ETH_ADDR_LEN] PAG_UNUSED
    = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };

static const uint8_t eth_addr_stp[ETH_ADDR_LEN] PAG_UNUSED
    = { 0x01, 0x80, 0xC2, 0x00, 0x00, 0x00 };

static const uint8_t eth_addr_lacp[ETH_ADDR_LEN] PAG_UNUSED
    = { 0x01, 0x80, 0xC2, 0x00, 0x00, 0x02 };

static const uint8_t eth_addr_bfd[ETH_ADDR_LEN] PAG_UNUSED
...
void eth_push_vlan(struct ofpbuf *, pag_be16 tpid, pag_be16 tci);
...
void set_mpls_lse(struct ofpbuf *, pag_be32 label);
void push_mpls(struct ofpbuf *packet, pag_be16 ethtype, pag_be32 lse);
void pop_mpls(struct ofpbuf *, pag_be16 ethtype);

void set_mpls_lse_ttl(pag_be32 *lse, uint8_t ttl);
void set_mpls_lse_tc(pag_be32 *lse, uint8_t tc);
void set_mpls_lse_label(pag_be32 *lse, pag_be32 label);
void set_mpls_lse_bos(pag_be32 *lse, uint8_t bos);
pag_be32 set_mpls_lse_values(uint8_t ttl, uint8_t tc, uint8_t bos,
                             pag_be32 label);
 ...
 * int a, b;
 *
 * if (pag_scan(string, "%d"ETH_ADDR_SCAN_FMT"%d",
...
static inline bool eth_type_mpls(pag_be16 eth_type)
{
...
vlan_tci_to_vid(pag_be16 vlan_tci)
{
...
static inline int
vlan_tci_to_pcp(pag_be16 vlan_tci)
{
...
static inline int
vlan_tci_to_cfi(pag_be16 vlan_tci)
{
...
struct vlan_header {
    pag_be16 vlan_tci;          /* Lowest 12 bits are VLAN ID. */
    pag_be16 vlan_next_type;
};
...
struct vlan_eth_header {
    ...
    pag_be16 veth_type;         /* Always htons(ETH_TYPE_VLAN). */
    pag_be16 veth_tci;          /* Lowest 12 bits are VLAN ID. */
    pag_be16 veth_next_type;
});
...
mpls_lse_to_label(pag_be32 mpls_lse)
{
...
mpls_lse_to_tc(pag_be32 mpls_lse)
{
 ...
 * return mpls ttl */
static inline uint8_t
mpls_lse_to_ttl(pag_be32 mpls_lse)
{
...
/* Set TTL in mpls lse. */
static inline void
flow_set_mpls_lse_ttl(pag_be32 *mpls_lse, uint8_t ttl)
{
 ...
 * return mpls BoS bit  */
static inline uint8_t
mpls_lse_to_bos(pag_be32 mpls_lse)
{
 ...
 * int a, b;
 *
 * if (pag_scan(string, "%d"IP_SCAN_FMT"%d", &a, IP_SCAN_ARGS(&ip), &b)) {
 *     ...
 * }
 */
#define IP_SCAN_FMT "%"SCNu8".%"SCNu8".%"SCNu8".%"SCNu8
#define IP_SCAN_ARGS(ip)                                    \
        ((void) (pag_be32) *(ip), &((uint8_t *) ip)[0]),    \
        &((uint8_t *) ip)[1],                               \
        &((uint8_t *) ip)[2],                               \
        &((uint8_t *) ip)[3]
...
static inline bool
ip_is_cidr(pag_be32 netmask)
{
    uint32_t x = ~ntohl(netmask);
    return !(x & (x + 1));
}
static inline bool
ip_is_multicast(pag_be32 ip)
{
    return (ip & htonl(0xf0000000)) == htonl(0xe0000000);
}
int ip_count_cidr_bits(pag_be32 netmask);
void ip_format_masked(pag_be32 ip, pag_be32 mask, struct ds *);
...
struct ip_header {
    ...
    pag_be16 ip_tot_len;
    pag_be16 ip_id;
    pag_be16 ip_frag_off;
    uint8_t ip_ttl;
    uint8_t ip_proto;
    pag_be16 ip_csum;
    pag_16aligned_be32 ip_src;
    pag_16aligned_be32 ip_dst;
};
...
struct tcp_header {
    pag_be16 tcp_src;
    pag_be16 tcp_dst;
    pag_16aligned_be32 tcp_seq;
    pag_16aligned_be32 tcp_ack;
    pag_be16 tcp_ctl;
    pag_be16 tcp_winsz;
    pag_be16 tcp_csum;
    pag_be16 tcp_urg;
...
struct arp_eth_header {
    /* Generic members. */
    pag_be16 ar_hrd;           /* Hardware type. */
    pag_be16 ar_pro;           /* Protocol type. */
    uint8_t ar_hln;            /* Hardware address length. */
    uint8_t ar_pln;            /* Protocol address length. */
    pag_be16 ar_op;            /* Opcode. */

    /* Ethernet+IPv4 specific members. */
    uint8_t ar_sha[ETH_ADDR_LEN]; /* Sender hardware address. */
    pag_16aligned_be32 ar_spa;           /* Sender protocol address. */
    uint8_t ar_tha[ETH_ADDR_LEN]; /* Target hardware address. */
    pag_16aligned_be32 ar_tpa;           /* Target protocol address. */
};
...
union pag_16aligned_in6_addr {
    pag_be16 be16[8];
    pag_16aligned_be32 be32[4];
};

/* Like struct in6_hdr, but whereas that struct requires 32-bit alignment, this
 * one only requires 16-bit alignment. */
struct pag_16aligned_ip6_hdr {
    union {
        struct pag_16aligned_ip6_hdrctl {
            pag_16aligned_be32 ip6_un1_flow;
            pag_be16 ip6_un1_plen;
            uint8_t ip6_un1_nxt;
            uint8_t ip6_un1_hlim;
        } ip6_un1;
        uint8_t ip6_un2_vfc;
    } ip6_ctlun;
    union pag_16aligned_in6_addr ip6_src;
    union pag_16aligned_in6_addr ip6_dst;
};

/* Like struct in6_frag, but whereas that struct requires 32-bit alignment,
 * this one only requires 16-bit alignment. */
struct pag_16aligned_ip6_frag {
    uint8_t ip6f_nxt;
    uint8_t ip6f_reserved;
    pag_be16 ip6f_offlg;
    pag_16aligned_be32 ip6f_ident;
};
 ...
 *
 * if (pag_scan(string, "%d"IPV6_SCAN_FMT"%d", &a, ipv6_s, &b)
...
static inline bool dl_type_is_ip_any(pag_be16 dl_type)
{
...
void packet_set_ipv4(struct ofpbuf *, pag_be32 src, pag_be32 dst, uint8_t tos,
                     uint8_t ttl);
void packet_set_ipv6(struct ofpbuf *, uint8_t proto, const pag_be32 src[4],
                     const pag_be32 dst[4], uint8_t tc,
                     pag_be32 fl, uint8_t hlmit);
void packet_set_tcp_port(struct ofpbuf *, pag_be16 src, pag_be16 dst);
void packet_set_udp_port(struct ofpbuf *, pag_be16 src, pag_be16 dst);
void packet_set_sctp_port(struct ofpbuf *, pag_be16 src, pag_be16 dst);
