...
unix_open(const char *name, char *suffix, struct stream **streamp,
          uint8_t dscp PAG_UNUSED)
{
    char *connect_path;
    int fd;

    connect_path = abs_file_name(pag_rundir(), suffix);
    fd = make_unix_socket(SOCK_STREAM, true, NULL, connect_path);

    if (fd < 0) {
        VLOG_DBG("%s: connection failed (%s)",
                 connect_path, pag_strerror(-fd));
        free(connect_path);
        return -fd;
    }
...
static int
punix_open(const char *name PAG_UNUSED, char *suffix,
           struct pstream **pstreamp, uint8_t dscp PAG_UNUSED)
{
    char *bind_path;
    int fd, error;

    bind_path = abs_file_name(pag_rundir(), suffix);
    fd = make_unix_socket(SOCK_STREAM, true, bind_path, NULL);
    if (fd < 0) {
        VLOG_ERR("%s: binding failed: %s", bind_path, pag_strerror(errno));
        free(bind_path);
        return errno;
    }
    ...
    if (listen(fd, 10) < 0) {
        error = errno;
        VLOG_ERR("%s: listen: %s", name, pag_strerror(error));
        close(fd);
        free(bind_path);
        return error;
    }
