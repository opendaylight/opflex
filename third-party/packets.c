...
{
    if (pag_scan(s, ETH_ADDR_SCAN_FMT, ETH_ADDR_SCAN_ARGS(ea))) {
        return true;
    } else {
 ...
 * This function is used by Policy Agent to compose packets in cases where
...
void
eth_push_vlan(struct ofpbuf *packet, pag_be16 tpid, pag_be16 tci)
{
...
static void
set_ethertype(struct ofpbuf *packet, pag_be16 eth_type)
{
    struct eth_header *eh = packet->data;

    if (eh->eth_type == htons(ETH_TYPE_VLAN)) {
        pag_be16 *p;
        p = ALIGNED_CAST(pag_be16 *,
...
void
set_mpls_lse_ttl(pag_be32 *lse, uint8_t ttl)
{
...
void
set_mpls_lse_tc(pag_be32 *lse, uint8_t tc)
{
...
void
set_mpls_lse_label(pag_be32 *lse, pag_be32 label)
{
...
void
set_mpls_lse_bos(pag_be32 *lse, uint8_t bos)
{
...
pag_be32
set_mpls_lse_values(uint8_t ttl, uint8_t tc, uint8_t bos, pag_be32 label)
{
    pag_be32 lse = htonl(0);
...
void
set_mpls_lse(struct ofpbuf *packet, pag_be32 mpls_lse)
{
...
void
push_mpls(struct ofpbuf *packet, pag_be16 ethtype, pag_be32 lse)
{
    struct mpls_hdr mh;
 ...
 * Ethertype). */
void
pop_mpls(struct ofpbuf *packet, pag_be16 ethtype)
{
...
int
ip_count_cidr_bits(pag_be32 netmask)
{
    return 32 - ctz32(ntohl(netmask));
}

void
ip_format_masked(pag_be32 ip, pag_be32 mask, struct ds *s)
{
...
packet_set_ipv4_addr(struct ofpbuf *packet,
                     pag_16aligned_be32 *addr, pag_be32 new_addr)
{
    struct ip_header *nh = packet->l3;
    pag_be32 old_addr = get_16aligned_be32(addr);
...
{
    const struct pag_16aligned_ip6_hdr *nh;
    int nexthdr;
    size_t len;
    size_t remaining;
    ...
    nh = ALIGNED_CAST(struct pag_16aligned_ip6_hdr *, data);
    data += sizeof *nh;
    remaining -= sizeof *nh;
    nexthdr = nh->ip6_nxt;
    ...
    while (1) {
        ...
        if (nexthdr == IPPROTO_AH) {
            ...
            const struct pag_16aligned_ip6_frag *frag_hdr
                = ALIGNED_CAST(struct pag_16aligned_ip6_frag *, data);
...
packet_update_csum128(struct ofpbuf *packet, uint8_t proto,
                     pag_16aligned_be32 addr[4], const pag_be32 new_addr[4])
{
                     ...
                     bool recalculate_csum)
{
    ...
    memcpy(addr, new_addr, sizeof(pag_be32[4]));
}

static void
packet_set_ipv6_flow_label(pag_16aligned_be32 *flow_label, pag_be32 flow_key)
{
    pag_be32 old_label = get_16aligned_be32(flow_label);
    pag_be32 new_label = (old_label & htonl(~IPV6_LABEL_MASK)) | flow_key;
    put_16aligned_be32(flow_label, new_label);
}

static void
packet_set_ipv6_tc(pag_16aligned_be32 *flow_label, uint8_t tc)
{
    pag_be32 old_label = get_16aligned_be32(flow_label);
    pag_be32 new_label = (old_label & htonl(0xF00FFFFF)) | htonl(tc << 20);
    put_16aligned_be32(flow_label, new_label);
}
 ...
 * markers. */
void
packet_set_ipv4(struct ofpbuf *packet, pag_be32 src, pag_be32 dst,
...
void
packet_set_ipv6(struct ofpbuf *packet, uint8_t proto, const pag_be32 src[4],
                const pag_be32 dst[4], uint8_t key_tc, pag_be32 key_fl,
                uint8_t key_hl)
{
    struct pag_16aligned_ip6_hdr *nh = packet->l3;

    if (memcmp(&nh->ip6_src, src, sizeof(pag_be32[4]))) {
        packet_set_ipv6_addr(packet, proto, nh->ip6_src.be32, src, true);
    }

    if (memcmp(&nh->ip6_dst, dst, sizeof(pag_be32[4]))) {
...
static void
packet_set_port(pag_be16 *port, pag_be16 new_port, pag_be16 *csum)
{
    if (*port != new_port) {
...
void
packet_set_tcp_port(struct ofpbuf *packet, pag_be16 src, pag_be16 dst)
{
    struct tcp_header *th = packet->l4;
 ...
 * with its l4 marker properly populated. */
void
packet_set_udp_port(struct ofpbuf *packet, pag_be16 src, pag_be16 dst)
{
    struct udp_header *uh = packet->l4;
 ...
 * with its l4 marker properly populated. */
void
packet_set_sctp_port(struct ofpbuf *packet, pag_be16 src, pag_be16 dst)
{
    struct sctp_header *sh = packet->l4;
    pag_be32 old_csum, old_correct_csum, new_csum;
