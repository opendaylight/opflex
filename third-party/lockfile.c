 ...
 * once. */
static struct pag_mutex lock_table_mutex = PAG_MUTEX_INITIALIZER;
static struct hmap lock_table__ = HMAP_INITIALIZER(&lock_table__);
static struct hmap *const lock_table PAG_GUARDED_BY(lock_table_mutex)
    = &lock_table__;
...
lockfile_lock(const char *file, struct lockfile **lockfilep)
{
     ...
     * because the Policy Agent code that currently uses lock files does so in
...
lockfile_find(dev_t device, ino_t inode) PAG_REQUIRES(&lock_table_mutex)
{
    struct lockfile *lockfile;
...
static void
lockfile_unhash(struct lockfile *lockfile) PAG_REQUIRES(&lock_table_mutex)
{
    if (lockfile->fd >= 0) {
        close(lockfile->fd);
        lockfile->fd = -1;
    ...
    PAG_REQUIRES(&lock_table_mutex)
{
    ...
    *pidp = 0;
    ...
    if (!retval) {
        VLOG_WARN("Failed to lock file : %s", pag_lasterror_to_string());
        return EEXIST;
    }
