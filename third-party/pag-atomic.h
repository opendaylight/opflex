 ...
 * implementations of the PAG atomics do need them.  Thus, the following
 ...
 * of the PAG atomics do need them.  Thus, the following two functions are
...
#define IN_PAG_ATOMIC_H
    #if __CHECKER__
        /* sparse doesn't understand some GCC extensions we use. */
        #include "pag-atomic-pthreads.h"
    #elif HAVE_STDATOMIC_H
        #include "pag-atomic-c11.h"
    #elif __has_extension(c_atomic)
        #include "pag-atomic-clang.h"
    #elif __GNUC__ >= 4 && __GNUC_MINOR__ >= 7
        #include "pag-atomic-gcc4.7+.h"
    #elif HAVE_GCC4_ATOMICS
        #include "pag-atomic-gcc4+.h"
    #else
        #include "pag-atomic-pthreads.h"
    #endif
#undef IN_PAG_ATOMIC_H
...
static inline void
pag_refcount_init(struct pag_refcount *refcount)
{
...
/* Destroys 'refcount'. */
static inline void
pag_refcount_destroy(struct pag_refcount *refcount)
{
...
/* Increments 'refcount'. */
static inline void
pag_refcount_ref(struct pag_refcount *refcount)
{
    unsigned int old_refcount;

    atomic_add(&refcount->count, 1, &old_refcount);
    pag_assert(old_refcount > 0);
}

/* Decrements 'refcount' and returns the previous reference count.  Often used
 * in this form:
 *
 * if (pag_refcount_unref(&object->ref_cnt) == 1) {
 *     // ...uninitialize object...
 *     free(object);
 * }
 */
static inline unsigned int
pag_refcount_unref(struct pag_refcount *refcount)
{
    unsigned int old_refcount;
 ...
 * Rarely useful. */
static inline unsigned int
pag_refcount_read(const struct pag_refcount *refcount_)
{
    struct pag_refcount *refcount
        = CONST_CAST(struct pag_refcount *, refcount_);
    unsigned int count;
