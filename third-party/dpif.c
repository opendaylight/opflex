...
static struct pag_mutex dpif_mutex = PAG_MUTEX_INITIALIZER;
...
dp_register_provider(const struct dpif_class *new_class)
{
    ...
    pag_mutex_lock(&dpif_mutex);
    error = dp_register_provider__(new_class);
    pag_mutex_unlock(&dpif_mutex);
...
do_open(const char *name, const char *type, bool create, struct dpif **dpifp)
{
    ...
    error = registered_class->dpif_class->open(registered_class->dpif_class,
                                               name, create, &dpif);
    if (!error) {
        pag_assert(dpif->dpif_class == registered_class->dpif_class);
    } else {
        dp_class_unref(registered_class);
    }
...
dpif_port_add(struct dpif *dpif, struct netdev *netdev, odp_port_t *port_nop)
{
    ...
    if (!error) {
        ...
        VLOG_WARN_RL(&error_rl, "%s: failed to add %s as port: %s",
                     dpif_name(dpif), netdev_name, pag_strerror(error));
        port_no = ODPP_NONE;
    }
    if (port_nop) {
        *port_nop = port_no;
...
dpif_port_exists(const struct dpif *dpif, const char *devname)
{
    ...
    if (error != 0 && error != ENOENT && error != ENODEV) {
        VLOG_WARN_RL(&error_rl, "%s: failed to query port %s: %s",
                     dpif_name(dpif), devname, pag_strerror(error));
    }
                          ...
                          struct dpif_port *port)
{
    ...
    if (!error) {
        ...
        VLOG_WARN_RL(&error_rl, "%s: failed to query port %"PRIu32": %s",
                     dpif_name(dpif), port_no, pag_strerror(error));
    }
    return error;
}
...
/* Returns the Netlink PID value to supply in PAG_ACTION_ATTR_USERSPACE actions
 * as the PAG_USERSPACE_ATTR_PID attribute's value, for use in flows whose
 ...
 * PAG_ACTION_ATTR_USERSPACE actions. */
uint32_t
                   ...
                   char *name, size_t name_size)
{
    ...
    pag_assert(name_size > 0);

    error = dpif_port_query_by_number(dpif, port_no, &port);
    if (!error) {
        pag_strlcpy(name, port.name, name_size);
 ...
 * attributes with types PAG_KEY_ATTR_* in the 'key_len' bytes starting at
 * 'key'.
 *
...
dpif_flow_put__(struct dpif *dpif, const struct dpif_flow_put *put)
{
    ...
    pag_assert(!(put->flags & ~(DPIF_FP_CREATE | DPIF_FP_MODIFY
                                | DPIF_FP_ZERO_STATS)));
 ...
 * attribute PAG_FLOW_ATTR_KEY with types PAG_KEY_ATTR_* in the 'key_len' bytes
 * starting at 'key', and PAG_FLOW_ATTR_MASK with types of PAG_KEY_ATTR_* in
 * the 'mask_len' bytes starting at 'mask'. The associated actions are
 * specified by the Netlink attributes with types PAG_ACTION_ATTR_* in the
 ...
 * with types PAG_KEY_ATTR_* in the 'key_len' bytes starting at 'key'.
 *
 * If the operation succeeds, then 'stats', if nonnull, will be set to the
 ...
 * will be set to Netlink attributes with types PAG_KEY_ATTR_* representing the
 * dumped flow's key.  If 'actions' and 'actions_len' are nonnull then they are
 * set to Netlink attributes with types PAG_ACTION_ATTR_* representing the
...
dpif_execute_helper_cb(void *aux_, struct ofpbuf *packet,
                       ...
                       const struct nlattr *action, bool may_steal PAG_UNUSED)
{
 ...
 * actions that it can, and for PAG_ACTION_ATTR_OUTPUT and
 * PAG_ACTION_ATTR_USERSPACE actions it passes the packet through to the dpif
 * implementation.
 *
 ...
 * must update all of the flows that have PAG_ACTION_ATTR_USERSPACE actions
...
log_operation(const struct dpif *dpif, const char *operation, int error)
{
    if (!error) {
        ...
        VLOG_WARN_RL(&error_rl, "%s: %s failed (%s)",
                     dpif_name(dpif), operation, pag_strerror(error));
    }
}
                 ...
                 const struct nlattr *actions, size_t actions_len)
{
    ...
    if (error) {
        ds_put_format(&ds, "(%s) ", pag_strerror(error));
    }
                    ...
                    int error)
{
    if (!(error ? VLOG_DROP_WARN(&error_rl) : VLOG_DROP_DBG(&dpmsg_rl))) {
        ...
        if (error) {
            ds_put_format(&ds, " failed (%s)", pag_strerror(error));
        }
