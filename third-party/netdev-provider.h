...
/* ## ---------------- ## */
     ...
     * datapath".  It will also prevent the PAG implementation of bonding from
     ...
     * in vswitchd/vswitch.xml (which is built as pag-vswitchd.conf.db(8)).
     *
     ...
     * pag-vswitchd.conf.db(8)).  Returns 0 if successful, otherwise a positive
     ...
     * This function may be set to null if it would always return EOPNOTSUPP
     * anyhow. */
    int (*arp_lookup)(const struct netdev *netdev, pag_be32 ip,
                      uint8_t mac[6]);
