...
check_vconn_classes(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(vconn_classes); i++) {
        ...
        pag_assert(class->name != NULL);
        pag_assert(class->open != NULL);
        if (class->close || class->recv || class->send
            || class->run || class->run_wait || class->wait) {
            pag_assert(class->close != NULL);
            pag_assert(class->recv != NULL);
            pag_assert(class->send != NULL);
            pag_assert(class->wait != NULL);
        } else {
    ...
    for (i = 0; i < ARRAY_SIZE(pvconn_classes); i++) {
        ...
        pag_assert(class->name != NULL);
        pag_assert(class->listen != NULL);
        if (class->close || class->accept || class->wait) {
            pag_assert(class->close != NULL);
            pag_assert(class->accept != NULL);
            pag_assert(class->wait != NULL);
        } else {
            /* This class delegates to another one. */
        }
...
void
vconn_usage(bool active, bool passive, bool bootstrap PAG_UNUSED)
{
           ...
           struct vconn **vconnp)
{
    ...
    /* Success. */
    pag_assert(vconn->state != VCS_CONNECTING || vconn->class->connect);
    *vconnp = vconn;
    return 0;
            ...
            if (ofptype_decode(&type, msg->data)
                ...
                if (retval) {
                    VLOG_INFO_RL(&bad_ofmsg_rl,
                                 ...
                                 vconn->name, pag_strerror(retval));
                    ofpbuf_delete(reply);
                }
...
do_send(struct vconn *vconn, struct ofpbuf *msg)
{
    ...
    pag_assert(msg->size >= sizeof(struct ofp_header));
    ...
    if (!VLOG_IS_DBG_ENABLED()) {
        ...
        if (retval != EAGAIN) {
            VLOG_DBG_RL(&ofmsg_rl, "%s: sent (%s): %s",
                        vconn->name, pag_strerror(retval), s);
        }
        free(s);
    }
    return retval;
...
int
vconn_recv_xid(struct vconn *vconn, pag_be32 xid, struct ofpbuf **replyp)
{
    for (;;) {
        pag_be32 recv_xid;
        struct ofpbuf *reply;
...
{
    pag_be32 send_xid = ((struct ofp_header *) request->data)->xid;
    int error;
...
vconn_transact_noreply(struct vconn *vconn, struct ofpbuf *request,
                       struct ofpbuf **replyp)
{
    pag_be32 request_xid;
    pag_be32 barrier_xid;
...
{
    pag_assert(wait == WAIT_CONNECT || wait == WAIT_RECV || wait == WAIT_SEND);
...
pvconn_accept(struct pvconn *pvconn, struct vconn **new_vconn)
{
    ...
    if (retval) {
        *new_vconn = NULL;
        ...
        pag_assert((*new_vconn)->state != VCS_CONNECTING
           ...
           int connect_status, const char *name, uint32_t allowed_versions)
{
    ...
    pag_assert(vconn->state != VCS_CONNECTING || class->connect);
}
