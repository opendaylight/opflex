 ...
 * minimum of _XOPEN_IOV_MAX (16) and a maximum of MAX_IPAG.
 *
...
nl_sock_create(int protocol, struct nl_sock **sockp)
{
    ...
    if (pagthread_once_start(&once)) {
        ...
        max_ipag = sysconf(_SC_UIO_MAXIOV);
        if (max_ipag < _XOPEN_IOV_MAX) {
            if (max_ipag == -1 && errno) {
                VLOG_WARN("sysconf(_SC_UIO_MAXIOV): %s", pag_strerror(errno));
            }
            max_ipag = _XOPEN_IOV_MAX;
        } else if (max_ipag > MAX_IPAG) {
            max_ipag = MAX_IPAG;
        }
    ...
    *sockp = NULL;
    ...
    if (sock->fd < 0) {
        VLOG_ERR("fcntl: %s", pag_strerror(errno));
        goto error;
    }
    ...
    if (connect(sock->fd, (struct sockaddr *) &remote, sizeof remote) < 0) {
        VLOG_ERR("connect(0): %s", pag_strerror(errno));
        goto error;
    }
    ...
    if (getsockname(sock->fd, (struct sockaddr *) &local, &local_size) < 0) {
        VLOG_ERR("getsockname: %s", pag_strerror(errno));
        goto error;
    }
...
nl_sock_join_mcgroup(struct nl_sock *sock, unsigned int multicast_group)
{
    if (setsockopt(sock->fd, SOL_NETLINK, NETLINK_ADD_MEMBERSHIP,
        ...
        VLOG_WARN("could not join multicast group %u (%s)",
                  multicast_group, pag_strerror(errno));
        return errno;
    }
    return 0;
}
...
nl_sock_recv__(struct nl_sock *sock, struct ofpbuf *buf, bool wait)
{
    ...
    pag_assert(buf->allocated >= sizeof *nlmsghdr);
    ofpbuf_clear(buf);
                            ...
                            size_t *done)
{
    ...
    *done = 0;
    for (i = 0; i < n; i++) {
        ...
        ipag[i].iov_base = txn->request->data;
        ipag[i].iov_len = txn->request->size;
    }
                          ...
                          struct nl_transaction **transactions, size_t n)
{
    ...
    max_batch_count = MIN(max_batch_count, max_ipag);
    ...
    while (n > 0) {
        ...
        if (error == ENOBUFS) {
            ...
            VLOG_ERR_RL(&rl, "transaction error (%s)", pag_strerror(error));
...
static struct pag_mutex pool_mutex = PAG_MUTEX_INITIALIZER;
static struct nl_pool pools[MAX_LINKS] PAG_GUARDED_BY(pool_mutex);
...
nl_pool_alloc(int protocol, struct nl_sock **sockp)
{
    ...
    pag_assert(protocol >= 0 && protocol < ARRAY_SIZE(pools));

    pag_mutex_lock(&pool_mutex);
    pool = &pools[protocol];
    if (pool->n > 0) {
        sock = pool->socks[--pool->n];
    }
    pag_mutex_unlock(&pool_mutex);
    ...
    if (h) {
        ...
        if (h->nlmsg_type == NLMSG_ERROR) {
            ...
            if (e) {
                ...
                if (e->error < 0) {
                    ds_put_format(&ds, "(%s)", pag_strerror(-e->error));
                }
          ...
          const void *message, size_t size, int protocol)
{
    ...
    VLOG_DBG_RL(&rl, "%s (%s): %s", function, pag_strerror(error), nlmsg);
    free(nlmsg);
}
