...
struct flow {
    ...
    pag_be64 metadata;          /* OpenFlow Metadata. */
    ...
    pag_be16 vlan_tci;          /* If 802.1Q, TCI | VLAN_CFI; otherwise 0. */
    pag_be32 mpls_lse[FLOW_MAX_MPLS_LABELS]; /* MPLS label stack entry. */

    /* L3 */
    struct in6_addr ipv6_src;   /* IPv6 source address. */
    struct in6_addr ipv6_dst;   /* IPv6 destination address. */
    struct in6_addr nd_target;  /* IPv6 neighbor discovery (ND) target. */
    pag_be32 ipv6_label;        /* IPv6 flow label. */
    pag_be32 nw_src;            /* IPv4 source address. */
    pag_be32 nw_dst;            /* IPv4 destination address. */
    ...
    pag_be16 tcp_flags;         /* TCP flags. With L3 to avoid matching L4. */
    pag_be16 pad;               /* Padding. */
...
struct flow_metadata {
    pag_be64 tun_id;                 /* Encapsulating tunnel ID. */
    pag_be32 tun_src;                /* Tunnel outer IPv4 src addr */
    pag_be32 tun_dst;                /* Tunnel outer IPv4 dst addr */
    pag_be64 metadata;               /* OpenFlow 1.1+ metadata field. */
    uint32_t regs[FLOW_N_REGS];      /* Registers. */
    uint32_t pkt_mark;               /* Packet mark. */
...
void flow_set_dl_vlan(struct flow *, pag_be16 vid);
void flow_set_vlan_vid(struct flow *, pag_be16 vid);
...
void flow_push_mpls(struct flow *, int n, pag_be16 mpls_eth_type,
                    struct flow_wildcards *);
bool flow_pop_mpls(struct flow *, int n, pag_be16 eth_type,
                   struct flow_wildcards *);
void flow_set_mpls_label(struct flow *, int idx, pag_be32 label);
...
void flow_set_mpls_lse(struct flow *, int idx, pag_be32 lse);
...
{
    return (PAG_FORCE uint16_t) ofp_port;
}

static inline uint32_t
odp_to_u32(odp_port_t odp_port)
{
    return (PAG_FORCE uint32_t) odp_port;
}

static inline uint32_t
ofp11_to_u32(ofp11_port_t ofp11_port)
{
    return (PAG_FORCE uint32_t) ofp11_port;
}
...
static inline pag_be64 miniflow_get_metadata(const struct miniflow *);
...
static inline pag_be64 minimask_get_metadata_mask(const struct minimask *);
...
miniflow_get_metadata(const struct miniflow *flow)
{
    ...
    pag_be32 hi = (PAG_FORCE pag_be32) miniflow_get(flow, MD_OFS / 4);
    pag_be32 lo = (PAG_FORCE pag_be32) miniflow_get(flow, MD_OFS / 4 + 1);
