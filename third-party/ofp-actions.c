                              ...
                              struct ofpbuf *out)
{
    ...
    if (ids_size < ids->n_controllers * sizeof(pag_be16)) {
        VLOG_WARN_RL(&rl, "Nicira action dec_ttl_cnt_ids only has %"PRIuSIZE" bytes "
                     ...
                     ids->n_controllers * sizeof(pag_be16), ids->n_controllers);
        return OFPERR_OFPBAC_BAD_LEN;
    }

    for (i = 0; i < ids->n_controllers; i++) {
        uint16_t id = ntohs(((pag_be16 *)(nac_ids + 1))[i]);
        ofpbuf_put(out, &id, sizeof id);
        ids = out->l2;
    }
...
static enum ofperr
push_mpls_from_openflow(pag_be16 ethertype, struct ofpbuf *out)
{
...
ofpact_from_openflow10(const union ofp_action *a,
                       enum ofp_version version PAG_UNUSED,
                       struct ofpbuf *out)
{
    enum ofputil_action_code code;
    enum ofperr error;
                        ...
                        struct ofpbuf *ofpacts)
{
    ...
    pag_assert(mf->n_bytes == oxm_length);
...
set_field_to_nxast(const struct ofpact_set_field *sf, struct ofpbuf *openflow)
{
    ...
    if (mf->n_bits > 64) {
        pag_assert(mf->n_bytes == 16); /* IPv6 addr. */
...
#define DEFINE_INST(ENUM, STRUCT, EXTENSIBLE, NAME)             \
    static inline const struct STRUCT * PAG_UNUSED              \
    instruction_get_##ENUM(const struct ofp11_instruction *inst)\
    {                                                           \
        pag_assert(inst->type == htons(ENUM));                  \
        return ALIGNED_CAST(struct STRUCT *, inst);             \
    }                                                           \
                                                                \
    static inline void PAG_UNUSED                               \
    ...
    {                                                           \
        ...
        s->len = htons(sizeof *s);                              \
                                                                ...
                                                                \
    static inline struct STRUCT * PAG_UNUSED                    \
...
struct instruction_type_info {
    enum pag_instruction_type type;
    const char *name;
};

static const struct instruction_type_info inst_info[] = {
#define DEFINE_INST(ENUM, STRUCT, EXTENSIBLE, NAME)    {PAGINST_##ENUM, NAME},
PAG_INSTRUCTIONS
#undef DEFINE_INST
};

const char *
pag_instruction_name_from_type(enum pag_instruction_type type)
{
    return inst_info[type].name;
}

int
pag_instruction_type_from_name(const char *name)
{
...
enum pag_instruction_type
pag_instruction_type_from_ofpact_type(enum ofpact_type type)
{
    switch (type) {
    case OFPACT_METER:
        return PAGINST_OFPIT13_METER;
    case OFPACT_CLEAR_ACTIONS:
        return PAGINST_OFPIT11_CLEAR_ACTIONS;
    case OFPACT_WRITE_ACTIONS:
        return PAGINST_OFPIT11_WRITE_ACTIONS;
    case OFPACT_WRITE_METADATA:
        return PAGINST_OFPIT11_WRITE_METADATA;
    case OFPACT_GOTO_TABLE:
        return PAGINST_OFPIT11_GOTO_TABLE;
    case OFPACT_OUTPUT:
    case OFPACT_GROUP:
    case OFPACT_CONTROLLER:
    case OFPACT_ENQUEUE:
    ...
    case OFPACT_SAMPLE:
    default:
        return PAGINST_OFPIT11_APPLY_ACTIONS;
    }
}
                               ...
                               const struct ofp11_instruction *out[])
{
    ...
    memset(out, 0, N_PAG_INSTRUCTIONS * sizeof *out);
                                   ...
                                   struct ofpbuf *ofpacts)
{
    ...
    const struct ofp11_instruction *insts[N_PAG_INSTRUCTIONS];
    enum ofperr error;
    ...
    if (insts[PAGINST_OFPIT13_METER]) {
        const struct ofp13_instruction_meter *oim;
        struct ofpact_meter *om;

        oim = ALIGNED_CAST(const struct ofp13_instruction_meter *,
                           insts[PAGINST_OFPIT13_METER]);

        om = ofpact_put_METER(ofpacts);
        om->meter_id = ntohl(oim->meter_id);
    }
    if (insts[PAGINST_OFPIT11_APPLY_ACTIONS]) {
        const union ofp_action *actions;
        size_t max_actions;

        get_actions_from_instruction(insts[PAGINST_OFPIT11_APPLY_ACTIONS],
                                     &actions, &max_actions);
    ...
    if (insts[PAGINST_OFPIT11_CLEAR_ACTIONS]) {
        instruction_get_OFPIT11_CLEAR_ACTIONS(
            insts[PAGINST_OFPIT11_CLEAR_ACTIONS]);
        ofpact_put_CLEAR_ACTIONS(ofpacts);
    }
    if (insts[PAGINST_OFPIT11_WRITE_ACTIONS]) {
        ...
        get_actions_from_instruction(insts[PAGINST_OFPIT11_WRITE_ACTIONS],
                                     &actions, &max_actions);
...
{
    struct ofpact *a;
    pag_be16 dl_type = flow->dl_type;
    pag_be16 vlan_tci = flow->vlan_tci;
...
{
    const struct ofpact *a;
    enum pag_instruction_type inst;

    inst = PAGINST_OFPIT13_METER;
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        enum pag_instruction_type next;

        next = pag_instruction_type_from_ofpact_type(a->type);
        if (a > ofpacts
            && (inst == PAGINST_OFPIT11_APPLY_ACTIONS
                ? next < inst
                : next <= inst)) {
            const char *name = pag_instruction_name_from_type(inst);
            const char *next_name = pag_instruction_name_from_type(next);
                                  ...
                                  enum ofp_version ofp_version)
{
    ...
    pag_assert(ofp_version >= OFP11_VERSION);

    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        switch (pag_instruction_type_from_ofpact_type(a->type)) {
        case PAGINST_OFPIT11_CLEAR_ACTIONS:
            instruction_put_OFPIT11_CLEAR_ACTIONS(openflow);
            break;

        case PAGINST_OFPIT11_GOTO_TABLE: {
        ...
        case PAGINST_OFPIT11_WRITE_METADATA: {
        ...
        case PAGINST_OFPIT11_APPLY_ACTIONS: {
            ...
            OFPACT_FOR_EACH(action, a, ofpacts_len_left) {
                if (pag_instruction_type_from_ofpact_type(action->type)
                    != PAGINST_OFPIT11_APPLY_ACTIONS) {
                    break;
                }
        ...
        case PAGINST_OFPIT11_WRITE_ACTIONS: {
            const size_t ofs = openflow->size;
...
ofpacts_get_meter(const struct ofpact ofpacts[], size_t ofpacts_len)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        inst = pag_instruction_type_from_ofpact_type(a->type);
        if (a->type == OFPACT_METER) {
            return ofpact_get_METER(a)->meter_id;
        } else if (inst > PAGINST_OFPIT13_METER) {
            break;
        }
    }
...
ofpact_format(const struct ofpact *a, struct ds *s)
{
    ...
    case OFPACT_WRITE_ACTIONS: {
        struct ofpact_nest *on = ofpact_get_WRITE_ACTIONS(a);
        ds_put_format(s, "%s(",
                      pag_instruction_name_from_type(
                          PAGINST_OFPIT11_WRITE_ACTIONS));
    ...
    case OFPACT_WRITE_METADATA:
        ...
        ds_put_format(s, "%s:%#"PRIx64,
                      pag_instruction_name_from_type(
                          PAGINST_OFPIT11_WRITE_METADATA),
                      ntohll(metadata->metadata));
        if (metadata->mask != PAG_BE64_MAX) {
    ...
    case OFPACT_GOTO_TABLE:
        ds_put_format(s, "%s:%"PRIu8,
                      pag_instruction_name_from_type(
                          PAGINST_OFPIT11_GOTO_TABLE),
                      ofpact_get_GOTO_TABLE(a)->table_id);
        break;

    case OFPACT_METER:
        ds_put_format(s, "%s:%"PRIu32,
                      pag_instruction_name_from_type(PAGINST_OFPIT13_METER),
