...
{
    return (!name
            ? xasprintf("%s/%s.pid", pag_rundir(), program_name)
            : abs_file_name(pag_rundir(), name));
}
...
make_pidfile(void)
{
    ...
    if (!file) {
        VLOG_FATAL("%s: create failed (%s)", tmpfile, pag_strerror(errno));
    }
    ...
    if (fstat(fileno(file), &s) == -1) {
        VLOG_FATAL("%s: fstat failed (%s)", tmpfile, pag_strerror(errno));
    }

    if (ftruncate(fileno(file), 0) == -1) {
        VLOG_FATAL("%s: truncate failed (%s)", tmpfile, pag_strerror(errno));
    }

    fprintf(file, "%ld\n", pid);
    if (fflush(file) == EOF) {
        VLOG_FATAL("%s: write failed (%s)", tmpfile, pag_strerror(errno));
    }
    ...
    if (error < 0) {
        VLOG_FATAL("failed to rename \"%s\" to \"%s\" (%s)",
                   tmpfile, pidfile, pag_strerror(errno));
    }
    ...
    if (pid > 0) {
        ...
        if (read_fully(fds[0], &c, 1, &bytes_read) != 0) {
            ...
            if (retval == pid) {
                ...
                VLOG_FATAL("waitpid failed (%s)", pag_strerror(errno));
            } else {
                PAG_NOT_REACHED();
            }
        }
...
fork_notify_startup(int fd)
{
    if (fd != -1) {
        ...
        if (error) {
            VLOG_FATAL("pipe write failed (%s)", pag_strerror(error));
        }
...
get_null_fd(void)
{
    ...
    if (!null_fd) {
        ...
        if (null_fd < 0) {
            int error = errno;
            VLOG_ERR("could not open /dev/null: %s", pag_strerror(error));
            null_fd = -error;
        }
    }
...
read_pidfile__(const char *pidfile, bool delete_if_stale)
{
    ...
    if (!file) {
        ...
        error = errno;
        VLOG_WARN("%s: open: %s", pidfile, pag_strerror(error));
        goto error;
    }

    error = lock_pidfile__(file, F_GETLK, &lck);
    if (error) {
        VLOG_WARN("%s: fcntl: %s", pidfile, pag_strerror(error));
        goto error;
    }
    ...
    if (!fgets(line, sizeof line, file)) {
        if (ferror(file)) {
            error = errno;
            VLOG_WARN("%s: read: %s", pidfile, pag_strerror(error));
        } else {
            error = ESRCH;
...
void
service_start(int *argc PAG_UNUSED, char **argv[] PAG_UNUSED)
{
}
