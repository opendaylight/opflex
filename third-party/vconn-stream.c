...
vconn_stream_open(const char *name, uint32_t allowed_versions,
                  char *suffix PAG_UNUSED, struct vconn **vconnp, uint8_t dscp)
{
    struct stream *stream;
    int error;
...
vconn_stream_run(struct vconn *vconn)
{
    ...
    if (retval < 0) {
        if (retval != -EAGAIN) {
            VLOG_ERR_RL(&rl, "send: %s", pag_strerror(-retval));
            vconn_stream_clear_txbuf(s);
            return;
        }
    } else if (retval > 0) {
...
pvconn_pstream_listen(const char *name, uint32_t allowed_versions,
                      char *suffix PAG_UNUSED, struct pvconn **pvconnp,
                      uint8_t dscp)
{
    ...
    if (error) {
        if (error != EAGAIN) {
            VLOG_DBG_RL(&rl, "%s: accept: %s",
                        pstream_get_name(ps->pstream), pag_strerror(error));
        }
        return error;
    }
