...
{
    pag_assert(wait == STREAM_CONNECT || wait == STREAM_RECV
               || wait == STREAM_SEND);
...
pstream_accept(struct pstream *pstream, struct stream **new_stream)
{
    ...
    if (retval) {
        *new_stream = NULL;
        ...
        pag_assert((*new_stream)->state != SCS_CONNECTING
            ...
            int connect_status, const char *name)
{
    ...
    pag_assert(stream->state != SCS_CONNECTING || class->connect);
}
...
void
pstream_set_bound_port(struct pstream *pstream, pag_be16 port)
{
                              ...
                              uint8_t dscp)
{
    ...
    if ((!strncmp(name_, "tcp:", 4) || !strncmp(name_, "ssl:", 4))
        ...
        } else if (default_port == PAGDB_OLD_PORT) {
            VLOG_WARN_ONCE("The default PAGDB port number will change "
                           "from %d to %d in a future release",
                           PAGDB_OLD_PORT, PAGDB_PORT);
        }
