...
{
    if (header) {
        pag_assert(!mf_from_nxm_header__(header));
...
nxm_do_init(void)
{
    ...
    for (i = 0; i < MFF_N_IDS; i++) {
        ...
        pag_assert(mf->id == i); /* Fields must be in the enum order. */
...
mf_mask_field_and_prereqs(const struct mf_field *mf, struct flow *mask)
{
    ...
    case MFP_ND_SOLICIT:
    case MFP_ND_ADVERT:
        mask->tp_src = PAG_BE16_MAX;
        mask->tp_dst = PAG_BE16_MAX;
        /* Fall through. */
    case MFP_TCP:
    case MFP_UDP:
    case MFP_SCTP:
                        ...
                        uint8_t mask[ETH_ADDR_LEN])
{
    ...
    pag_assert(mf->n_bytes == ETH_ADDR_LEN);

    n = -1;
    if (pag_scan(s, ETH_ADDR_SCAN_FMT"%n", ETH_ADDR_SCAN_ARGS(mac), &n)
        && n == strlen(s)) {
    ...
    n = -1;
    if (pag_scan(s, ETH_ADDR_SCAN_FMT"/"ETH_ADDR_SCAN_FMT"%n",
...
mf_from_ipv4_string(const struct mf_field *mf, const char *s,
                    pag_be32 *ip, pag_be32 *mask)
{
    int prefix;

    pag_assert(mf->n_bytes == sizeof *ip);

    if (pag_scan(s, IP_SCAN_FMT"/"IP_SCAN_FMT,
                 IP_SCAN_ARGS(ip), IP_SCAN_ARGS(mask))) {
        /* OK. */
    } else if (pag_scan(s, IP_SCAN_FMT"/%d", IP_SCAN_ARGS(ip), &prefix)) {
    ...
    } else if (pag_scan(s, IP_SCAN_FMT, IP_SCAN_ARGS(ip))) {
        *mask = PAG_BE32_MAX;
    } else {
                    ...
                    struct in6_addr *value, struct in6_addr *mask)
{
    ...
    pag_assert(mf->n_bytes == sizeof *value);
...
mf_from_ofp_port_string(const struct mf_field *mf, const char *s,
                        pag_be16 *valuep, pag_be16 *maskp)
{
    ofp_port_t port;

    pag_assert(mf->n_bytes == sizeof(pag_be16));
...
mf_from_ofp_port_string32(const struct mf_field *mf, const char *s,
                          pag_be32 *valuep, pag_be32 *maskp)
{
    ofp_port_t port;

    pag_assert(mf->n_bytes == sizeof(pag_be32));
    if (ofputil_port_from_string(s, &port)) {
...
static char *
mf_from_tun_flags_string(const char *s, pag_be16 *valuep, pag_be16 *maskp)
{
...
static char *
mf_from_tcp_flags_string(const char *s, pag_be16 *flagsp, pag_be16 *maskp)
{
    uint16_t flags = 0;
    uint16_t mask = 0;
    uint16_t bit;
    ...
    if (pag_scan(s, "%"SCNi16"/%"SCNi16"%n", &flags, &mask, &n) && !s[n]) {
        *flagsp = htons(flags);
        *maskp = htons(mask);
        return NULL;
    }
    if (pag_scan(s, "%"SCNi16"%n", &flags, &n) && !s[n]) {
        *flagsp = htons(flags);
        *maskp = PAG_BE16_MAX;
        return NULL;
    }
...
static void
mf_format_tnl_flags_string(const pag_be16 *valuep, struct ds *s)
{
    format_flags(s, flow_tun_flag_to_string, ntohs(*valuep), '|');
}

static void
mf_format_tcp_flags_string(pag_be16 value, pag_be16 mask, struct ds *s)
{
          ...
          struct ds *s)
{
    ...
    case MFS_IPV4:
        ip_format_masked(value->be32, mask ? mask->be32 : PAG_BE32_MAX, s);
        break;
...
mf_parse_subfield__(struct mf_subfield *sf, const char **sp)
{
    ...
    s += name_len;
    if (pag_scan(s, "[%d..%d]", &start, &end)) {
        /* Nothing to do. */
    } else if (pag_scan(s, "[%d]", &start)) {
        end = start;
    } else if (!strncmp(s, "[]", 2)) {
        start = 0;
        end = field->n_bits - 1;
