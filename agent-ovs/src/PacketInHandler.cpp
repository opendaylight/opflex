...
bool PacketInHandler::writeLearnFlow(SwitchConnection *conn,
    ...
    fm.new_cookie = IntFlowManager::GetLearnEntryCookie();
...
static void removeLearnFlow(SwitchConnection* conn, const FlowEntryPtr& fe) {
    ...
    fm.cookie = IntFlowManager::GetLearnEntryCookie();
...
void PacketInHandler::dstFlowCb(const FlowEntryList& flows,
    ...
    BOOST_FOREACH(const FlowEntryPtr& fe, flows) {
        if (fe->entry->cookie == IntFlowManager::GetLearnEntryCookie()) {
...
void PacketInHandler::Handle(SwitchConnection *conn,
    ...
    if (pi.cookie == IntFlowManager::GetLearnEntryCookie() ||
        pi.cookie == IntFlowManager::GetProactiveLearnEntryCookie())
        handleLearnPktIn(conn, pi, proto, pkt, flow);
    else if (pi.cookie == IntFlowManager::GetNDCookie())
        handleNDPktIn(agent, intFlowManager, conn, pi, proto, pkt, flow);
    else if (pi.cookie == IntFlowManager::GetDHCPCookie(true))
        handleDHCPPktIn(true, agent, intFlowManager,
                        conn, pi, proto, pkt, flow);
    else if (pi.cookie == IntFlowManager::GetDHCPCookie(false))
        handleDHCPPktIn(false, agent, intFlowManager,
                        conn, pi, proto, pkt, flow);
    else if (pi.cookie == IntFlowManager::GetVIPCookie(true))
        handleVIPPktIn(true, agent, *portMapper, pi, flow);
    else if (pi.cookie == IntFlowManager::GetVIPCookie(false))
        handleVIPPktIn(false, agent, *portMapper, pi, flow);
    else if (pi.cookie == IntFlowManager::GetICMPErrorCookie(true))
        handleICMPErrPktIn(true, agent, intFlowManager, conn,
                           pi, proto, pkt);
    //else if (pi.cookie == IntFlowManager::GetICMPErrorCookie(false))
