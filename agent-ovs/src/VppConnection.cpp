...
namespace ovsagent {
  ...
  VppConnection::message_map_t VppConnection::messageMap;
  VppConnection::message_map_t VppConnection::streamMap;

  VppConnection::messageIdsBiMap VppConnection::messageIds;
  std::map< std::string, std::string> VppConnection::messageCRCByName; //Name, CRC
  ...
  int VppConnection::populateMessageIds(const string apiPath) {
    fs::recursive_directory_iterator end;
    set<string> files;
    for (fs::recursive_directory_iterator it(apiPath);
         it != end; ++it) {
        ...
        for (pt::ptree::value_type &message : messages.second) {
          ...
          for (pt::ptree::value_type &field : message.second) {
            if (field.first == "crc") {
              ...
              VppConnection::messageCRCByName.insert(make_pair(msg, msg_crc));
              msgId = getMsgIndex(msg_crc);
              VppConnection::messageIds.insert( VppConnection::messageIdsPair( msgId, msg));
            }
          }
        }
      }
  ...
  u16 VppConnection::getMessageIdByName(string name) {
    VppConnection::messageIdsNameIter nameIter = VppConnection::messageIds.right.find(name);
    if ( nameIter != VppConnection::messageIds.right.end() ) {
  ...
  string VppConnection::getMessageNameById(u16 id) {
    VppConnection::messageIdsIdIter idIter = VppConnection::messageIds.left.find(id);
    if ( idIter != VppConnection::messageIds.left.end() ) {
  ...
  int VppConnection::connect (string n, string apiPath) {
    name = n;
    int rv = -1 ;
    if ( connected )
      return 0;
    ...
    if ( 0 != name.length() ) {
      ...
      if ( 0 == rv ) {
        ...
        connected = true;
        if ( populateMessageIds(apiPath) < 0) {
