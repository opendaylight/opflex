...
void PolicyStatsManager::getFlowCounterState(uint8_t table_id,
                                              flowCounterState_t& flowCounterState) {
    if (table_id == IntFlowManager::POL_TABLE_ID) {
        flowCounterState = contractFlowState;
    }
}
...
void PolicyStatsManager::updatePolicyFlowEntryMap(uint64_t cookie,
    ...
    updateFlowEntryMap(cookie, priority,contractFlowState,match);
}
...
void PolicyStatsManager::on_timer(const error_code& ec) {
    ...
    on_timer_base(ec,contractFlowState,newClassCountersMap);
