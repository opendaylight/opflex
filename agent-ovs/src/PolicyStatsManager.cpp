...
updatePolicyStatsCounters(const std::string& srcEpg,
    ...
    if (su) {
        ...
        classifierList_[key][idCount_[key]-1] = l24Classifier;
        srcEpgList_[key][idCount_[key]-1] = srcEpg;
        dstEpgList_[key][idCount_[key]-1] = dstEpg;
...
updatePolicyStatsDropCounters(const std::string& rdStr,
   ...
   uint64_t nextId = getNextDropGenId();
    if (reachedLimitDropCounter_ && dropCounterList_[idCountDropCounter_] != -1) {
        modelgbp::gbpe::RoutingDomainDropCounter::remove(getAgentUUID(),
                     dropCounterList_[idCountDropCounter_],dropCounterDomainList_[idCountDropCounter_]);
    }   
    dropCounterList_[idCountDropCounter_] = nextId;
    dropCounterDomainList_[idCountDropCounter_] = rdStr;
    idCountDropCounter_++;
    if (idCountDropCounter_ == MAX_DROP_COUNTER_LIMIT) {
          reachedLimitDropCounter_ = true;
          idCountDropCounter_ = 0;
    }   
            ...
            for (const opflex::modb::URI& dstEpg : consURIs) {
                if (srcEpg.toString() == dstEpg.toString())
                 ...
                 for (shared_ptr<PolicyRule>& pc : rules) {
                     ...
                     removeAllCounterObjects(ruleURI.toString()+srcEpg.toString()+dstEpg.toString());
                 }
            }
        }
    }   
