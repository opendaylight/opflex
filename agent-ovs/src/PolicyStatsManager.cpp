...
void PolicyStatsManager::Handle(SwitchConnection* connection,
    ...
    PolicyCounterMap_t diffCountersMap;
        ...
        if (ret != 0) {
            ...
            if (srcEpgUri && dstEpgUri && idStr) {
                ...
                PolicyCounters_t&  diffCounters = 
                                       diffCountersMap[flowMatchKey];
                if (oldCounters.packet_count) {
                    diffCounters.packet_count.get() += (fentry->packet_count - 
                                                oldCounters.packet_count.get());
                    diffCounters.byte_count.get() += (fentry->byte_count -
                ...
                oldCounters.byte_count.get() += fentry->byte_count;
                oldCounters.packet_count.get() += fentry->packet_count;
#if 0
    ...
    // walk through diffCountersMap to create new set of MOs

    for (PolicyCounterMap_t:: iterator itr = diffCountersMap.begin();
         itr != diffCountersMap.end();
         itr++) {
        const FlowMatchKey_t& flowKey = itr->first;
        PolicyCounters_t&  diffCounters = itr->second;
