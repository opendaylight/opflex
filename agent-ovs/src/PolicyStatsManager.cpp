...
void PolicyStatsManager::on_timer(const error_code& ec) {
    ...
    for (FlowEntryCounterMap_t:: iterator itr = oldFlowCounterMap.begin();
        ...
        if (!newFlowCounters.visited) {
            ...
            if (newFlowCounters.age >= (30000 * 5)) {
                LOG(DEBUG) << "Unvisited entry for last 5 polling interval";
            }
            continue;
        }
     ...
     * not been visited but age is equal to five times
     * polling interval.
     */
    for (FlowEntryCounterMap_t:: iterator itr = oldFlowCounterMap.begin();
         itr != oldFlowCounterMap.end();) {
        ...
        // Have we visited this flow entry yet
        if (!flowCounters.visited && (flowCounters.age >= (30000 * 5)))
     ...
     * that have age equal to five times polling interval. 
     */
...
void PolicyStatsManager::updateNewFlowCounters(uint32_t cookie,
    ...
    if (it != oldFlowCounterMap.end()) {
        ...
        oldFlowCounters.diff_byte_count = 
                                 ...
                                 byte_count);
        oldFlowCounters.last_packet_count = make_optional(true,
                                                flow_packet_count);
        oldFlowCounters.last_byte_count = make_optional(true,
                                                flow_byte_count);
        ...
        if (it != newFlowCounterMap.end()) {
            ...
            LOG(DEBUG) << "Received flow stats for an uknown flow";
        }
    }
}
