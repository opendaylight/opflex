...
void ContractStatsManager::clearCounterObject(const std::string& key,
    uint8_t index) {
        ...
        modelgbp::gbpe::L24ClassifierCounter::remove(agent->getFramework(),getAgentUUID(),genIdList_[key]->uidList[index],
            srcEpg,dstEpg,l24Classifier);
}
    ...
    if (polMgr.contractExists(contractURI)) { 
        ...
        for (const opflex::modb::URI& srcEpgURI : provURIs) {
            ...
            if (!classifierKeyMap_.count(srcEpg)) {
                classifierKeyMap_[srcEpg] = std::vector<std::tuple<std::string,std::string,std::string> >();
            }
            for (const opflex::modb::URI& dstEpgURI : consURIs) {
                ...
                if (!classifierKeyMap_.count(dstEpg)) {
                    classifierKeyMap_[dstEpg] = std::vector<std::tuple<std::string,std::string,std::string> >();
                }
                 for (shared_ptr<PolicyRule>& pc : rules) {
                     ...
                     if (!epgKeyMap_.count(l24Classifier)) {
                         epgKeyMap_[l24Classifier] = std::vector<std::tuple<std::string,std::string,std::string> >();
                     }
                     epgKeyMap_[l24Classifier].push_back(std::tuple<std::string,std::string,std::string>(l24Classifier,srcEpg,dstEpg));
                     classifierKeyMap_[srcEpg].push_back(std::tuple<std::string,std::string,std::string>(l24Classifier,srcEpg,dstEpg));
                     classifierKeyMap_[dstEpg].push_back(std::tuple<std::string,std::string,std::string>(l24Classifier,srcEpg,dstEpg));
                     std::string key = l24Classifier+srcEpg+dstEpg;
                     keyMap_[key] = std::tuple<std::string,std::string,std::string>(l24Classifier,srcEpg,dstEpg);
                 }
            }
        }
    }
...
void ContractStatsManager::egDomainUpdated(const opflex::modb::URI& epgURI) {
    ...
    if (!polMgr.groupExists(epgURI)) {
        ...
        if (classifierKeyMap_.count(epgName)) {
            std::unordered_map<std::string,std::vector<std::tuple<std::string,std::string,std::string> > >::iterator it = classifierKeyMap_.find(epgName);
    ...
    if (cid == RoutingDomain::CLASS_ID) {
        ...
        if (!rd) {
            ...
            for (int i = 0;i < dropCounterList_[rdName]->uidList.size();i++) {
                modelgbp::gbpe::RoutingDomainDropCounter::remove(getAgentUUID(),
                     dropCounterList_[rdName]->uidList[dropCounterList_[rdName]->count],rdName);
            }
