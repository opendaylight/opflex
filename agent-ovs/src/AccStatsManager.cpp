...
void AccStatsManager::updateAccInFlowEntryMap(uint64_t cookie, uint16_t priority, const struct match& match) {
  BaseStatsManager::updateFlowEntryMap(cookie, priority,0,match);
}

void AccStatsManager::updateAccOutFlowEntryMap(uint64_t cookie, uint16_t priority,const struct match& match) {
  BaseStatsManager::updateFlowEntryMap(cookie, priority,1,match);
}
...
void AccStatsManager::on_timer(const error_code& ec) {
    ...
    BaseStatsManager::on_timer(ec,0,newClassCountersMap1);
    BaseStatsManager::on_timer(ec,1,newClassCountersMap2);
