...
updatePolicyStatsCounters(const std::string& l24Classifier,
    ...
    if (su) {
        ...
        classifierList_[l24Classifier][idCount_[l24Classifier]-1] = l24Classifier;
...
void AccStatsManager::secGroupUpdated(const opflex::modb::URI& uri) {
    std::unordered_set<uri_set_t> secGrpSets;
    agent->getEndpointManager().getSecGrpSetsForSecGrp(uri, secGrpSets);
    for (const uri_set_t& secGrpSet : secGrpSets) {
       if (agent->getEndpointManager().secGrpSetEmpty(secGrpSet)) {
          for (const opflex::modb::URI& secGrp : secGrpSet) {
              ...
              for (shared_ptr<PolicyRule>& pc : rules) {
                 ...
                 removeAllCounterObjects(ruleURI.toString());
              }
          }
       }
    }   
