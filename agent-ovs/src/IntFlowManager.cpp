    ...
    if (rd) {
        ...
        for (auto const& sm : as.getServiceMappings()) {
            ...
            for (const address& nextHopAddr : nextHopAddrs) {
                ...
                if (ofPort == OFPP_NONE) {
                    vector<std::function<void(FlowBuilder&)>> functors;
                    if (zoneId != static_cast<uint16_t>(-1)) {
                        auto ct1 = [zoneId](FlowBuilder& f) {
                            f.conntrackState(0, FlowBuilder::CT_STATE_TRACKED);
                            f.action().conntrack(0, static_cast<mf_field_id>(0),
                                                 zoneId, SRC_TABLE_ID);
                        };
                        functors.push_back(ct1);
                        auto ct2 = [](FlowBuilder& f) {
                            ...
                            f.action().go(BRIDGE_TABLE_ID);
                        };
                        functors.push_back(ct2);
                    } else {
                        auto noct = [](FlowBuilder& f) {
                            f.action().go(BRIDGE_TABLE_ID);
                        };
                        functors.push_back(noct);
                    }
