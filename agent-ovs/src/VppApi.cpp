...
namespace ovsagent {
  ...
  /*
    VPP API requests/responses are serialised C-structs over a shared memory queue.

    The global struct defined below is used to handle both "discrete" and "continuous" replies to VPP API requests.

    A typical struct has the form:
    ...
    The _vl_msg_id defines the type of <api specific parms> as well as which function handles the request TO VPP.
    ...
    u32 context;
    i32 retval;
    <optional api specific parms>
    }

    An API request to VPP may return 1...N replies to a request.
    e.g.
    bridge_domain_add_del() returns -> bridge_domain_add_del_reply simply indicating success/failure

    BUT
    acl_dump() returns 1..N discrete responses of msgId:acl_details data:acl_details_t

    There is no "refcount" or "remaining_messages" counter in a reply. This is an improvement that can be made
    to VPP's API at a later date.

    To manage this, since there is one shared memory API queue to VPP, a "barrier" message is sent using
    the control_ping/control_ping_reply API calls.

    The "context" field is set by the client. VPP copies this into each reply received.

    For instance:
    client -> VPP: acl_dump(context = 42); control_ping(context = 42);
    VPP -> client: acl_details(context=42, acl_index=1); acl_details(context=42, acl_index=2); control_ping_reply(context=42);

    The control_ping_reply + context is the barrier for the "discrete" case.

    In the case where an API request to VPP returns "continuous" data, specifically counters and events, the control_ping
    barrier will not work.

    The VPP API supports the use of a "callback" function. In this case, whenever there is a response to a request, the callback
    function is called from VPP into the client to handle the response.

    For both "discrete" and "continuous" cases, rather than re-create an event loop, the std::promise/std::future features are used.

    Another benefit to maintaining a map of outstanding request/responses is that a client "transaction" may require multiple
    discrete VPP API calls:

    ie
    createEndpoint() ->
    - add port/device
    - set admin flags to up
    - associate with a bridge-domain or FIB
    -- create bridge-domain/FIB if it does not exist
    - associate an IP address to the interface OR a host route in a FIB if doing
      port -> bridge -> BVI -> FIB
    - associate ACL to interface
    - associate port-labels to interface
    - enable statistics on all the above
    etc...

    If any of the intermediate VPP API calls fails, it is desirable to rollback. To do this, the context field is used
    to track a client atomic transaction. ie

    createEndpoint(context = 42) ->
    - add port/device (msgID=444, context=42)
    - reply (msgID=445, context=42)
    - set admin flags to up (msgID=446, context=42)
    - reply (msgID=447, context=42)
    etc

    Given that VPP API messages are serialised C-structs, they are treated as <char*>.
    Given that there may be 1..N replies before a barrier is received, we treat an API response as vector< char*>

    Each API request msgID has a corresponding response msgID (ie request=444, response=445)

    (NOTE: responseMsgId is not always ++requestMsgId, see acl_dump / acl_details pair)

    By using future/promises, until we have the notion of discrete (either refId or barrier) and
    continuous API calls handled (potentially) as fd's in 17.07+ we need to spoof this functionality
    on our end.
  ...
  auto  VppApi::_connect (bool useCB) -> int
  {
    ...
    if ( 0 != name.length() ) {
      rv = pneum_connect( (char*) name.c_str(), NULL, cb, 32 /* rx queue length */);
      if ( 0 == rv ) {
        clientIndex = pneum_client_index();
        connected = true;
  ...
  auto VppApi::barrierSend (u32 context) -> int
  {
    ...
    return pneum_write ( (char *)mp, sizeof(*mp));
  }
  ...
  auto VppApi::barrierReceived() -> std::pair<bool, char*>
  {
    ...
    rv = pneum_read (&_reply, &reply_len, 0);
