...
namespace ovsagent {
    ...
    void VppRenderer::start() {
        if (started) return;
        started = true;
        vppManager.start(instanceName, apiPath);
    ...
    void VppRenderer::setProperties(const ptree& properties) {
        static const std::string VPP_INSTANCE_NAME("instance");
        instanceName = properties.get<std::string>(VPP_INSTANCE_NAME, "vpp-opflex");

        static const std::string VPP_API_PATH("api-path");
        apiPath = properties.get<std::string>(VPP_API_PATH, "/usr/share/vpp/api");
