...
void AccessFlowManager::createStaticFlows() {
    ...
    uint32_t emptySecGrpSetId =
        idGen.getId(flow::id::SECGROUP_SET, "");
...
void AccessFlowManager::handleEndpointUpdate(const string& uuid) {
    ...
    uint32_t secGrpSetId = idGen.getId(flow::id::SECGROUP_SET,
    ...
    switchManager.writeFlow(flow::id::ENDPOINT, uuid, GROUP_MAP_TABLE_ID, el);
}
...
void AccessFlowManager::handleSecGrpSetUpdate(const uri_set_t& secGrps,
    ...
    uint32_t secGrpSetId =
        idGen.getId(flow::id::SECGROUP_SET, secGrpsIdStr);
    ...
    BOOST_FOREACH(const opflex::modb::URI& secGrp, secGrps) {
        ...
        uint64_t secGrpCookie =
            idGen.getId(flow::id::SECGROUP, secGrp.toString());
