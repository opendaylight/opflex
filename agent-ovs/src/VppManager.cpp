...
VppManager::VppManager(Agent& agent_, IdGenerator& idGen_)
...
void VppManager::handleEndpointUpdate(const string& uuid) {
     ...
     */
    VOM::interface itf(vppInterfaceName.get(), VOM::interface::type_t::AFPACKET,
     ...
     * UNDER DEVELOPEMENT
     *
     std::shared_ptr<VOM::cmd> itfstats(new
     VOM::interface_cmds::stats_cmd(*this,
     std::vector<VOM::handle_t>{itf.handle()}));
    ...
    if (fd) {
        ...
        if (hasMac) {
            ...
            if (VOM::rc_t::OK != VOM::OM::write(uuid, be)) {
                LOG(ERROR) << "bridge-domain-entry: " << vppInterfaceName.get();
                return;
            }
...
void VppManager::updateBVIs(const URI& epgURI, VOM::bridge_domain& bd,
    ...
    for (shared_ptr<Subnet>& sn : subnets) {
        ...
        if (routerIp) {
            ...
            VOM::bridge_domain_arp_entry bae(bd, bvi.l2_address().to_mac(),
                                             raddr);
            ...
            VOM::route::path dvr_path(*encap_link, VOM::nh_proto_t::ETHERNET);
