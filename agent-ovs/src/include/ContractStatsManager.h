...
public:
     ...
     * Update which classifiers and epgs are 
     * associated with which contracts
     * */
    void contractUpdated(const opflex::modb::URI& contractURI);

    /* Policy Listener call back to capture when 
     * epg is created or deleted and delete 
     * corresponding counter objects for contracts
     * associated with given epgs
    */
    void egDomainUpdated(const opflex::modb::URI& epgURI);

    /* Policy Listener call back to capture when 
     * routing domain is created or deleted
     * Deletes drop domain counter objects
     * */
    void handleDomainUpdate(opflex::modb::class_id_t cid, const opflex::modb::URI& domURI);
private:
    std::unordered_map<std::string,std::tuple<
        std::string,std::string,std::string> > keyMap_;

    std::unordered_map<std::string,std::vector<
        std::tuple<std::string,std::string,
        std::string> > > classifierKeyMap_;

    std::unordered_map<std::string,
        std::vector<std::tuple<std::string,
        std::string,std::string> > > epgKeyMap_;
