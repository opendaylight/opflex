...
public:
    ...
    /**
     * initialize the ip route object
     */
     ...
     */
    int setIntfFlags(std::string name, u8 flags);

    /**
     * set/delete interface ip address
     *
     * @param name - interface name
     * @param isAdd - 1 to add, 0 to remove
     * @param delAll - 1 to remove all the ip addresses assigned to
     *                specified interface
     * @param isIpv6 - 1 if address is IPv6, otherwise 0
     * @param ipAddr - ip address to set on given interface
     * @param mask - subnet mask
     */
    int setIntfIpAddr(std::string name, u8 isAdd, u8 delAll, u8 isIpv6,
                      ip46_address ipAddr, u8 mask);
    ...
    /**
     * Create/Delete loopback interface
     */
    int createLoopback(std::string loopbackName, mac_addr_t macAddr);
    int deleteLoopback(std::string loopbackName);

    /**
     * Create/Delete VxLAN
     */
    int createVxlan(std::string vxlanIntfName, u8 isAdd, u8 isIpv6,
                    ip46_address srcAddr, ip46_address dstAddr,
                    std::string mcastIntf, u32 encapVrfId, u32 vni);

    /**
     * set interface ip table
     */
    int setIntfIpTable(std::string portName, u8 isIpv6, u8 vrfId);
