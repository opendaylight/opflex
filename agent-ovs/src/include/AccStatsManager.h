 ...
 */
class AccStatsManager : public BaseStatsManager, public PolicyListener {
public:
    /**
    ...
    void clearCounterObject(const std::string& key,uint8_t index) {
      modelgbp::gbpe::HPPClassifierCounter::remove(agent->getFramework(),getAgentUUID(),genIdList_[key][index],classifierList_[key][index]);
    }
