...
public:
   ...
   /**
    ...
    virtual void clearOldCounters(const std::string& key,uint64_t nextId) {
        if (!reachedLimit_.count(key)) {
              genIdList_[key] = new uint64_t[MAX_COUNTER_LIMIT];
              classifierList_[key] = new std::string[MAX_COUNTER_LIMIT];
              idCount_[key] = 0;
              reachedLimit_[key] = false;
        }
        if (idCount_[key] == MAX_COUNTER_LIMIT) {
          reachedLimit_[key] = true;
          idCount_[key] = 0;
        }
        if (reachedLimit_.find(key)->second) {
            clearCounterObject(key,idCount_[key]);
        }
        genIdList_[key][idCount_[key]] = nextId;
        idCount_[key]++;
    }
...
protected:
    std::unordered_map<std::string,bool> reachedLimit_;
    std::unordered_map<std::string,uint8_t> idCount_;
    std::unordered_map<std::string,uint64_t*> genIdList_;
    std::unordered_map<std::string,std::string*> classifierList_;
