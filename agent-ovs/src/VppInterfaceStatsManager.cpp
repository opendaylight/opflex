...
/*
 * Implementation for VppInterfaceStatsManager class.
 *
...
#include "VppInterfaceStatsManager.h"
#include "Agent.h"
#include "logging.h"
#include <limits>
...
VppInterfaceStatsManager::VppInterfaceStatsManager(Agent* agent_)
    : agent(agent_), stopping(false) {}

VppInterfaceStatsManager::~VppInterfaceStatsManager() {}

void VppInterfaceStatsManager::registerConnection(
    SwitchConnection* intConnection, SwitchConnection* accessConnection) {
    this->intConnection = intConnection;
    this->accessConnection = accessConnection;
}

void VppInterfaceStatsManager::start() {
    stopping = false;
    ...
    timer->async_wait(bind(&VppInterfaceStatsManager::on_timer, this, error));
}

void VppInterfaceStatsManager::stop() {
...
void VppInterfaceStatsManager::endpointUpdated(const std::string& uuid) {
...
void VppInterfaceStatsManager::on_timer(const error_code& ec) {
    if (ec) {
    ...
    if (!stopping) {
        ...
        timer->async_wait(
            bind(&VppInterfaceStatsManager::on_timer, this, error));
    }
}

void VppInterfaceStatsManager::updateEndpointCounters(
    const std::string& uuid, SwitchConnection* connection,
...
void VppInterfaceStatsManager::Handle(SwitchConnection* connection, int msgType,
                                      ofpbuf* msg) {
    ...
    while (1) {
        ...
        try {
            if (connection == intConnection) {
                const std::string& intPortName =
                    intPortMapper.FindPort(ps.port_no);
                epMgr.getEndpointsByIface(intPortName, endpoints);
            } else {
                const std::string& accessPortName =
                    accessPortMapper.FindPort(ps.port_no);
