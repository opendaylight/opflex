...
FlowManager::HandleRoutingDomainUpdate(const URI& rdURI) {
    ...
    typedef boost::unordered_map<std::string, uint16_t> subnet_t;
    subnet_t intSubnets;
    ...
    BOOST_FOREACH(shared_ptr<RoutingDomainToIntSubnetsRSrc>& subnets_ref,
        ...
        BOOST_FOREACH(shared_ptr<Subnet>& subnet, subnets) {
            ...
            if (!maskedAddr.empty()) {
                intSubnets[maskedAddr] = subnet->getPrefixLen().get();
            }
        }
    }
        ...
        BOOST_FOREACH(const std::string& cidrSn,
            ...
            if (comps.size() == 2) {
                try {
                    ...
                    if (!maskedAddr.empty()) {
                        intSubnets[maskedAddr] = prefixLen;
                    }
    ...
    BOOST_FOREACH(const subnet_t::value_type& sn, intSubnets) {
