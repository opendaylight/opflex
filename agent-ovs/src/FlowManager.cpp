...
static void SetEntryTcpFlags(FlowEntry *fe, uint32_t tcpFlags) {
    ...
    ovs_be16 flags = 0;
    if (tcpFlags & TcpFlagsEnumT::CONST_ACK) flags |= 0x01;
    if (tcpFlags & TcpFlagsEnumT::CONST_RST) flags |= 0x02;
    if (tcpFlags & TcpFlagsEnumT::CONST_SYN) flags |= 0x04;
    if (tcpFlags & TcpFlagsEnumT::CONST_FIN) flags |= 0x10;
