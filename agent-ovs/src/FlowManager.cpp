...

    {
        ...
        if (fallbackMode == FALLBACK_PROXY && tunPort != OFPP_NONE &&
            ...
            if (virtualRouterEnabled) {
                {
                    FlowEntry* unknownBridged = new FlowEntry();
                    unknownBridged->entry->priority = 1;
                    unknownBridged->entry->table_id = ROUTE_TABLE_ID;
                    SetDestActionOutputToEPGTunnel(unknownBridged);
                    unknownTunnelRt.push_back(FlowEntryPtr(unknownBridged));
                }
                {
                    FlowEntry* unknownRouted = new FlowEntry();
                    unknownRouted->entry->priority = 50;
                    unknownRouted->entry->table_id = ROUTE_TABLE_ID;
                    match_set_dl_dst(&unknownRouted->entry->match,
                                     GetRouterMacAddr());
                    ActionBuilder ab;
                    SetActionTunnelMetadata(ab, encapType, GetTunnelDst());
                    ab.SetOutputToPort(tunPort);
                    ab.Build(unknownRouted->entry);
                    unknownTunnelRt.push_back(FlowEntryPtr(unknownRouted));
                }
            }
        }
