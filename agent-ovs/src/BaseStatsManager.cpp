...
void BaseStatsManager::on_timer_base(const error_code& ec,uint8_t table_id,PolicyCounterMap_t& newClassCountersMap) {
...
void BaseStatsManager::handleFlowStats(int msgType, ofpbuf *msg) {
    ...
    do {
        ...
        if (ret != 0) {
          ...
          if (!isTableIdFound(fentry->table_id)) {
...
void BaseStatsManager::removeAllCounterObjects(const std::string& key) {
    if (!reachedLimit_.count(key)) return;
    Mutator mutator(agent->getFramework(), "policyelement");
    int max = idCount_[key];
    if (reachedLimit_[key]) {
        max = MAX_COUNTER_LIMIT;
    }   
    for (int i = 0;i < max;i++) {
        clearCounterObject(key,i);
    }
    idCount_.erase(idCount_.find(key));   
    if (reachedLimit_.count(key)) reachedLimit_.erase(reachedLimit_.find(key));   
    delete [] genIdList_[key];
    if (genIdList_.count(key)) genIdList_.erase(genIdList_.find(key));   
    if (classifierList_.count(key)) classifierList_.erase(classifierList_.find(key));  
    mutator.commit();
}
