...
void *
pneum_alloc_msg_size(int nbytes) {
  return vl_msg_api_alloc_as_if_client(nbytes);
}

int
vpp_sync_api(char *request,
             int  request_size,
             char **reply,
             int *reply_size)
{
  ...
  rv = pneum_write (request, request_size);
  if (0 != rv) {
    return rv - 100; //-100 indicates write failure
  }

  rv = pneum_read (&_reply, reply_size, read_timeout);
  *reply = _reply;
  if (0 != rv) {
    return rv - 200; //-200 indicates read failure
  }
