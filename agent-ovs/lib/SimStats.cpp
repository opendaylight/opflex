...
void SimStats::on_timer_contract(const boost::system::error_code& ec) {
    ...
    timer_restart(agent.contractInterval, fn, contract_timer);
}
...
void SimStats::on_timer_security_group(const boost::system::error_code& ec) {
    ...
    timer_restart(agent.securityGroupInterval, fn, security_group_timer);
}
...
void SimStats::on_timer_interface(const boost::system::error_code& ec) {
    ...
    timer_restart(agent.interfaceInterval, fn, interface_timer);
}
...
void SimStats::start() {
    ...
    if (agent.contractInterval != 0) {
        contract_timer.reset(new deadline_timer(io, milliseconds(agent.contractInterval)));
        contract_timer->async_wait([this](const boost::system::error_code& ec) {
                                          on_timer_contract(ec);
        });
    }

    if (agent.securityGroupInterval != 0) {
        security_group_timer.reset(new deadline_timer(io, milliseconds(agent.securityGroupInterval)));
        security_group_timer->async_wait([this](const boost::system::error_code& ec) {
                                                on_timer_security_group(ec);
        });
    }

    if (agent.interfaceInterval != 0) {
        interface_timer.reset(new deadline_timer(io, milliseconds(agent.interfaceInterval)));
