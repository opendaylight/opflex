...
bool PolicyManager::updateExternalInterface(const URI& uri, bool &toRemove) {
    ...
    optional<URI> extDomURI, extBDURI, extRDURI;
    optional<shared_ptr<RoutingDomain>> newRD;
        ...
        if(routeIter != static_route_map.end() &&
           !routeIter->second.isPresent()) {
            ...
            if(delLocalRoute) {
                ...
                resolveEpdrLocalRouteToSrtRSrc(routeIter->first.toString());
                srtRef.get()->remove();
            }
            mutator.commit();
            vector<shared_ptr<LocalRouteToSrtRSrc>> ecmpStaticRoutes;
            vector<shared_ptr<LocalRouteToDrtRSrc>> ecmpDynRoutes;
            if(delLocalRoute) {
                delLocalRoute.get()->
                resolveEpdrLocalRouteToSrtRSrc(ecmpStaticRoutes);
                delLocalRoute.get()->
                resolveEpdrLocalRouteToDrtRSrc(ecmpDynRoutes);
                if((ecmpStaticRoutes.size()==0) &&
                   (ecmpDynRoutes.size()==0)) {
                    delLocalRoute.get()->remove();
                    if(delRepRoute) {
                        delRepRoute.get()->remove();
                    }
                } else if(ecmpStaticRoutes.size()==0) {
                    if(delRepRoute) {
                        delRepRoute.get()->
                        setCost(POLICYMANAGER_DYNAMIC_ROUTE_COST);
                    }
                }
            }
            mutator.commit();
