...
void PolicyManager::updateRemoteRouteChildrenForPolicyPrefix(
    ...
    for(auto remoteRt : rs.remote_routes) {
        ...
        PolicyRoute &route = route_iter->second;
        boost::system::error_code ec;
...
bool PolicyManager::getRoute(
         ...
         boost::asio::ip::address self_tep,
    ...
    for (auto extNet : rs.extNets) {
        ...
        for (auto &extSubItr: l3s.subnet_map) {
            ...
            if(network::prefix_match(targetAddr, pfxLen, addr,
                ...
                lrtToPrt = localRoute.get()->resolveEpdrLocalRouteToPrtRSrc();
                if(lrtToRrt) {
                    if(lrtToRrt.get()->getTargetURI().get() == routeURI) {
                    Mutator mutator(framework, "policyelement");
                    if(parentRemoteRt) {
                        localRoute.get()->
    ...
    for(shared_ptr<RemoteRoute> route: remoteRoutes) {
        ...
        if(it == rs.remote_routes.end()) {
            ...
            updatePolicyPrefixChildrenForRemoteRoute(
                rd.get()->getURI(),
                (parentRemoteRt? parentRemoteRt.get():route->getURI()),
                route->getAddress().get(),
                route->getPrefixLen().get(),
                route->getURI(),
                notifyLocalRoutes);
...
void PolicyManager::updateRoutes(class_id_t class_id, const URI& uri) {
    using namespace modelgbp::gbp;
    unique_lock<mutex> guard(state_mutex);
    uri_set_t notifyStaticRoutes,notifyRemoteRoutes,notifyLocalRoutes;

    if(class_id == ExternalNode::CLASS_ID) {
        updateStaticRoutes(uri, notifyStaticRoutes, notifyLocalRoutes);
    } else if(class_id == RoutingDomain::CLASS_ID) {
        updateRemoteRoutes(uri, notifyRemoteRoutes, notifyLocalRoutes);
    } else if((class_id == StaticRoute::CLASS_ID)||
              (class_id == StaticNextHop::CLASS_ID)) {
        updateStaticRoute(class_id, uri,
                          notifyStaticRoutes,
                          notifyLocalRoutes);
    } else if((class_id == RemoteRoute::CLASS_ID) ||
              (class_id == RemoteNextHop::CLASS_ID)) {
        updateRemoteRoute(class_id, uri,
                          notifyRemoteRoutes,
                          notifyLocalRoutes);
    }
    guard.unlock();
