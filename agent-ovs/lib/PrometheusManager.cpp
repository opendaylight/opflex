...
static std::string ep_family_names[] =
{
...
static std::string ep_family_help[] =
{
...
{
    const std::lock_guard<std::mutex> lock(ep_counter_mutex);
     ...
     */
    exposer_ptr = std::unique_ptr<Exposer>(new Exposer{"127.0.0.1:8080", "/metrics", 1});
    registry_ptr = std::make_shared<Registry>();
...
bool PrometheusManager::checkMetricName (const std::string& metric_name)
{
    ...
    return std::regex_match(metric_name, metric_name_regex);
}

// sanitize metric family name for prometheus to accept
std::string PrometheusManager::sanitizeMetricName (std::string metric_name)
{
...
void PrometheusManager::addNUpdateEpCounter (const std::string& uuid,
    ...
    if (su) {
        ...
        if (ep_counter) {
            ...
            for (EP_METRICS metric=EP_RX_BYTES;
                ...
                boost::optional<uint64_t>   metric_opt;
                switch (metric) {
                case EP_RX_BYTES:
...
void PrometheusManager::removeEpCounter (const std::string& uuid,
                                         const std::string& ep_name)
{
    const std::lock_guard<std::mutex> lock(ep_counter_mutex);
