...
namespace opflexagent {
    ...
    void NetFlowManager::NetFlowUniverseListener::objectUpdated(class_id_t classId,
                                                          const URI &uri) {
        LOG(DEBUG) << "update on NetFlowUniverseListener class ID " << classId << " URI " << uri;
        ...
        if (classId == modelgbp::platform::Config::CLASS_ID) {
            ...
            if (exporterconfig) {
                ...
                LOG(DEBUG) << "update on exporterconfig and dst address is " << exporterconfig.get()->getDstAddr();
                LOG(DEBUG) << "update on exporterconfig and dst port is " << exporterconfig.get()->getDstPort();
    ...
    void NetFlowManager::NetFlowUniverseListener::processExporterConfig(const shared_ptr<modelgbp::netflow::ExporterConfig>& exporterconfig) {
        ...
        if(dstAddr) {
             ...
             LOG(DEBUG) << "NetFlowUniverseListener processExporterConfig dst address " << dstAddr.get();

        }
        boost::optional<uint16_t > dstPort =   exporterconfig->getDstPort();
        if(dstPort) {
             exportState->setDestinationPort(dstPort.get());
             LOG(DEBUG) << "NetFlowUniverseListener processExporterConfig dst port " << dstPort.get();
        }
        ...
        if(activeTimeout) {
            ...
            LOG(DEBUG) << "NetFlowUniverseListener processExporterConfig timepout " << activeTimeout.get();
        }
        netflowmanager.exporter_map.insert(make_pair(exporterconfig->getURI(), exportState));
        print_map(netflowmanager.exporter_map);
        LOG(DEBUG) << "NetFlowUniverseListener processExporterConfig uri " << exporterconfig->getURI();
    }
 }
