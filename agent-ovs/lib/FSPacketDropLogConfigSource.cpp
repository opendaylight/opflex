...
static bool isPacketDropLogConfig(fs::path filePath) {
    ...
    return (boost::algorithm::ends_with(fstr, "opflex_agent.droplogcfg") &&
    ...
    try {
        if (isPacketDropLogConfig(filePath)) {
            ...
            opflex::modb::URI uri = opflex::modb::URIBuilder()
                    .addElement("DropFlowConfigUniverse")
                    .addElement("dropFlowCfg").addElement(uuid.get()).build();
    ...
    try {
        ...
        if (isPacketDropLogConfig(filePath)) {
            ...
            if(it != dropFlowMap.end()) {
                opflex::modb::URI uri = opflex::modb::URIBuilder()
                        .addElement("DropFlowConfigUniverse")
                        .addElement("dropFlowCfg").addElement(it->second.uuid).build();
                PacketDropFlowConfig flowCfg(pathStr, uri);
