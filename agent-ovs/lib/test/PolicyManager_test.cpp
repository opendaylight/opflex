...
BOOST_FIXTURE_TEST_CASE( static_route_add_mod_del, PolicyFixture ) {
    ...
    uint8_t pfx_len;
    std::string expectedRoute("101.101.0.0");
    ...
    BOOST_CHECK(nhList == expectedNh);
    boost::optional<shared_ptr<modelgbp::epdr::LocalRouteDiscovered>> lD;
    boost::optional<shared_ptr<modelgbp::epdr::LocalRoute>> localRoute;
    lD = modelgbp::epdr::LocalRouteDiscovered::resolve(framework);
    localRoute = lD.get()->resolveEpdrLocalRoute(
                    rd_->getName().get(), addr_.to_string(), pfx_len);
    BOOST_CHECK(localRoute);
    std::vector<std::shared_ptr<modelgbp::epdr::LocalRouteToSrtRSrc>> srt;
    localRoute.get()->resolveEpdrLocalRouteToSrtRSrc(srt);
    BOOST_CHECK(srt.size()==1);
    BOOST_CHECK(srt[0]->getTargetURI() == static_route1->getURI());
    Mutator m0(framework, "policyreg");
    static_nh1->remove();
    m0.commit();
    nhList.clear();
