...
*/
        ...
        mutator2.commit();
        pSess = agent.getSpanManager().getSessionState(sess->getURI());
...
static bool checkSess(boost::optional<shared_ptr<SessionState>> pSess) {
...
static bool checkSrcEps(boost::optional<shared_ptr<SessionState>> pSess,
        shared_ptr<span::SrcMember> srcMem1, shared_ptr<L2Ep> l2e1) {

        unordered_set<shared_ptr<SourceEndPoint>> srcEps =
                pSess.get()->getSrcEndPointSet();
        if (srcEps.size() == 0) {
            return false;
        }
        unordered_set<shared_ptr<SourceEndPoint>>::iterator it =
                srcEps.begin();
        shared_ptr<SourceEndPoint> srcEp = *it;
        if (srcMem1->getDir().get() != srcEp->getDirection())
            return false;
        if (l2e1->getInterfaceName().get().compare(srcEp->getPort()) != 0)
            return false;
...
BOOST_FIXTURE_TEST_CASE( verify_artifacts, SpanFixture ) {
    ...
    WAIT_FOR(checkSess(pSess), 500);
    WAIT_FOR(checkSpan(pSess, sess->getURI()), 500);
    WAIT_FOR(checkSrcEps(pSess, srcMem1, l2E1), 500);
    WAIT_FOR(checkDst(pSess, dstSumm1), 500);
