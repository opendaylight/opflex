...
namespace opflexagent {
    ...
    void SpanManager::SpanUniverseListener::objectUpdated(class_id_t classId,
        ...
        if (classId == Universe::CLASS_ID) {
            ...
            if (L2Ep::resolve(spanmanager.framework, uri)) {
                ...
                LOG(DEBUG) << "L2Ep mac " << l2Ep->getMac();
                processL2Ep(l2Ep);
            }
        }
    ...
    void SpanManager::notifyListeners(const opflex::modb::URI& spanURI) {
    ...
    optional<shared_ptr<SpanManager::SessionState>>
        ...
        std::unordered_map<opflex::modb::URI, shared_ptr<SpanManager::SessionState>>
    ...
    void SpanManager::SpanUniverseListener::processSession(const URI& uri) {
        shared_ptr<SessionState> sessState;
        auto itr = spanmanager.sess_map.find(uri);
        if (itr == spanmanager.sess_map.end()) {
            sessState = make_shared<SessionState>(uri);
            spanmanager.sess_map.insert(std::make_pair(uri, sessState));
        } else {
            ...
            if (memRefOpt) {
                ...
                if (memRef->getTargetClass()) {
                    ...
                    if (class_id == modelgbp::gbp::EpGroup::CLASS_ID) {
                        LOG(DEBUG) << "class ID EpGroup";
                    } else if (class_id == modelgbp::span::LocalEp::CLASS_ID) {
                        LOG(DEBUG) << "Got class ID LocalEp";
    ...
    void SpanManager::SpanUniverseListener::processDstGrp(DstGrp& dstGrp,
        ...
        if (seSt != spanmanager.sess_map.end()) {
            ...
            for (shared_ptr <DstMember> dstMem : dstMemVec) {
                optional <shared_ptr<modelgbp::span::DstSummary>> dstSumm =
                        dstMem->resolveSpanDstSummary();
                if (dstSumm) {
                    optional<const string &> dest = dstSumm.get()->getDest();
                    if (dest) {
                        LOG(DEBUG) << "Dst summ dest " << dest.get();
    ...
    const std::unordered_map<URI, shared_ptr<SpanManager::SourceEndPoint>>&
        SpanManager::SessionState::getSrcEndPointMap() const {
        return srcEndPoints;
    }

    const std::unordered_map<URI, shared_ptr<SpanManager::DstEndPoint>>&
    ...
    void SpanManager::SpanUniverseListener::
        ...
        LOG(DEBUG) << "get parent lEp " << (lEp ? "set" : "null") <<
                      " l2Ep " << (l2Ep ? "set" : "null");
        optional<std::pair<URI, prop_id_t>> parent = spanmanager.framework.
                               getParent(lEp->CLASS_ID, lEp->getURI());
        if (parent) {
            LOG(DEBUG) << "found parent " << parent.get().first;
            spanmanager.notifyUri.insert(parent.get().first);
            optional<shared_ptr<Session>> sess = Session::resolve(spanmanager.framework,
                    parent.get().first);
            if (sess) {
                LOG(DEBUG) << "found Session " << sess.get()->getName();
    ...
    void SpanManager::SpanUniverseListener::processL2Ep(shared_ptr<L2Ep> l2Ep) {
        ...
        shared_ptr<LocalEp> lEp = spanmanager.l2EpUri[l2Ep->getURI()];
        addEndPoint(lEp, l2Ep);
    }

    SpanManager::SessionState::SessionState(const opflex::modb::URI& uri) : uri(uri) {}

    SpanManager::SourceEndPoint::SourceEndPoint(string name_, string port_) : name(name_),
                                port(port_) {}
