...
public:
    //PolicyStatsManagerFixture() : switchManager(agent, exec, reader, portmapper),
...
void PolicyStatsManagerFixture::intExpFlowClassifier3() {

    std::vector<FlowEntryList> tables;
    ...
    switchManager.writeFlow(classifier3Id, 
                            ...
                            tables[IntFlowManager::POL_TABLE_ID]);
}
...
BOOST_FIXTURE_TEST_CASE(testFlowMatchStats, PolicyStatsManagerFixture) {
    ...
    boost::optional<uint32_t> reg0 =
        policyManager.getVnidForGroup(epg1->getURI());
    boost::optional<uint32_t> reg2 =
        policyManager.getVnidForGroup(epg2->getURI());
    ...
    LOG(ERROR) << "FlowStatsReplyMessage handling successful";
    ofpbuf_delete(res_msg);
    verifyFlowStats(epg1, epg2, classifier3,
                    packet_count, byte_count);
    LOG(ERROR) << "FlowStatsReplyMessage verification successful";
...
BOOST_FIXTURE_TEST_CASE(testRdDropStats, PolicyStatsManagerFixture) {
    ...
    policyStatsManager.Handle(&integrationPortConn,
                          OFPTYPE_FLOW_STATS_REPLY, res_msg);
    ofpbuf_delete(res_msg);
    LOG(ERROR) << "testRd:FlowStatsReplyMessage handling successful";

    verifyRoutingDomainDropStats(rd0, packet_count, byte_count);
    LOG(ERROR) << "testRd:FlowStatsReplyMessage verification successful";
    policyStatsManager.stop();
}
...
BOOST_FIXTURE_TEST_CASE(testFlowRemoved, PolicyStatsManagerFixture) {
    ...
    LOG(ERROR) << "FlowRemovedMessage handling successful";
    ofpbuf_delete(res_msg);
    verifyFlowStats(epg1, epg2, classifier3,
                    last_packet_count - packet_count,
                    last_byte_count -  byte_count);
    LOG(ERROR) << "FlowRemovedMessage verification successful";
    policyStatsManager.stop();
}
