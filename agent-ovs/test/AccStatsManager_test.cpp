...
BOOST_FIXTURE_TEST_CASE(testFlowMatchStats, AccStatsManagerFixture) {
    ...
    accStatsManager.registerConnection(&accPortConn);
    ...
    testOneFlow(accPortConn,classifier1,AccessFlowManager::SEC_GROUP_IN_TABLE_ID,1,&accStatsManager);
    // 2 entries in flow table now - testing second flow
    testOneFlow(accPortConn,classifier2,AccessFlowManager::SEC_GROUP_IN_TABLE_ID,2,&accStatsManager);
    // changing flow table entry 
    testOneFlow(accPortConn,classifier1,AccessFlowManager::SEC_GROUP_IN_TABLE_ID,2,&accStatsManager);
    // same 3 steps above for OUT table
    testOneFlow(accPortConn,classifier1,AccessFlowManager::SEC_GROUP_OUT_TABLE_ID,1,&accStatsManager);
    testOneFlow(accPortConn,classifier2,AccessFlowManager::SEC_GROUP_OUT_TABLE_ID,2,&accStatsManager);
    testOneFlow(accPortConn,classifier1,AccessFlowManager::SEC_GROUP_OUT_TABLE_ID,2,&accStatsManager);
