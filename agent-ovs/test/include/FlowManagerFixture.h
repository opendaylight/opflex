...
void writeClassifierFlows(FlowEntryList& entryList,uint32_t table_id,
    ...
    if (srcEpg != NULL && dstEpg != NULL)
    {
        ...
        FlowBuilder().cookie(cookie).inPort(portNum).table(table_id)
            .priority(priority).reg(SEPG,epg1_vnid).reg(DEPG,epg2_vnid).build(entryList);
    } else {
        uint32_t secGrpSetId = idGen.getId("secGroupSet",
                                       getSecGrpSetId(ep0->getSecurityGroups()));
        FlowBuilder().cookie(cookie).inPort(portNum).table(table_id)
            .priority(priority).reg(SEPG,secGrpSetId).build(entryList);
    }
...
void testCircBuffer(MockConnection& portConn,
    ...
    uint64_t firstId;
    for (int statCount = 0;statCount < MAX_COUNTER_LIMIT+1;statCount++) {

        // create subsequent flow stats reply message
        res_msg = makeFlowStatReplyMessage_2(&portConn,
                                         FINAL_PACKET_COUNT * (statCount+1),
                                         table_id,
                                         entryList);
    ...
    if (srcEpg != NULL && dstEpg != NULL) {
        optional<shared_ptr<L24ClassifierCounter> > myCounter =
            su.get()-> resolveGbpeL24ClassifierCounter(uuid,firstId,
                                                   ...
                                                   classifier->getURI()
                                                   .toString());
        BOOST_REQUIRE(!myCounter);
    } else {
        optional<shared_ptr<HPPClassifierCounter> > myCounter =
            su.get()-> resolveGbpeHPPClassifierCounter(uuid,firstId,
                                                   classifier->getURI()
                                                   .toString());
        BOOST_REQUIRE(!myCounter);
    }
