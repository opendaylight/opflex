...
enum TABLE {
    SEC = 0, SRC = 1, BR    = 2, RT  = 3, NAT = 4,
    LRN = 5, POL = 6, NEIGH = 7, OUT = 8
};
string rstr[] = {
...
// Initialize flows related to floating IP
void FlowManagerFixture::initExpFloatingIp() {
    uint8_t rmacArr[6];
    memcpy(rmacArr, flowManager.GetRouterMacAddr(), sizeof(rmacArr));
    string rmac(MAC(rmacArr).toString());
    ...
    ADDF(Bldr().table(RT).priority(262).ipv6().reg(RD, 1).isIpv6Dst("fdf1::/16")
         .actions().move(SEPG, TUNID).load(TUNDST, tunDst)
         .outPort(tunPort).done());
    ADDF(Bldr().table(RT).priority(174).ip().reg(RD, 1).isIpDst("5.0.0.0/8")
         .actions().move(SEPG, TUNID).load(TUNDST, tunDst)
         .outPort(tunPort).done());

    ADDF(Bldr().table(NAT).priority(262).ipv6().reg(RD, 1).isIpv6Src("fdf1::/16")
         .actions().load(SEPG, 0xabcd).go(POL).done());
    ADDF(Bldr().table(NAT).priority(174).ip().reg(RD, 1).isIpSrc("5.0.0.0/8")
         .actions().load(SEPG, 0xabcd).go(POL).done());

    ADDF(Bldr().table(OUT).priority(10).ipv6().reg(RD, 1).isMdAct(2)
         .isIpv6Src("2001:db8::2")
    ...
    ADDF(Bldr().table(OUT).priority(10).ip().reg(RD, 1).isMdAct(2)
         .isIpSrc("10.20.44.2")
         .actions().ethSrc(epmac).ethDst(rmac)
