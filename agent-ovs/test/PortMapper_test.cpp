...
public:
    MockConnection() : SwitchConnection("mockBridge") {
    }
    ...
    boost::optional<OfpBuf> lastSentMsg;
};
...
public:
    ...
    boost::optional<OfpBuf> MakeReplyMsg(size_t startIdx,
...
BOOST_FIXTURE_TEST_CASE(portdesc_multi, PortMapperFixture) {
    ...
    BOOST_REQUIRE(reply3);
    BOOST_CHECK(pm.FindPort("test-port-45") == OFPP_NONE);
    Received(pm, reply3.get());

    auto reply1 = MakeReplyMsg(0, 2, true);
    BOOST_REQUIRE(reply1);
    BOOST_CHECK(pm.FindPort("test-port-5") == OFPP_NONE);
    Received(pm, reply1.get());

    auto reply2 = MakeReplyMsg(2, 5, false);
    BOOST_REQUIRE(reply2);
    Received(pm, reply2.get());
    BOOST_CHECK(pm.FindPort("test-port-5") == 5);
    BOOST_CHECK(pm.FindPort("test-port-15") == 15);
