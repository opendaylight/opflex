...
BOOST_FIXTURE_TEST_CASE(startAndStopBeforeInitialization,StatsManagerFixture) {
   ...
   BOOST_REQUIRE_NO_THROW(statsManager.stop());
   LOG(DEBUG) << "done: startAndStopBeforeInitialization";
}
...
BOOST_FIXTURE_TEST_CASE(registerUsingVariousCombinations,StatsManagerFixture) {
   ...
   LOG(DEBUG) << "done: registerUsingVariousCombinations";
}
...
LOG(DEBUG) << "start: useIntConnectionAlone";
   MockConnection integrationPortConn(TEST_CONN_TYPE_INT);
   BOOST_REQUIRE_NO_THROW(statsManager.registerConnection(&integrationPortConn,NULL));
   BOOST_REQUIRE_NO_THROW(statsManager.start());
...
//   BOOST_REQUIRE_THROW(statsManager.Handle(&integrationPortConn,OFPTYPE_PORT_STATS_REPLY+1,NULL),
//                          boost::execution_exception);
   ...
   optional<shared_ptr<EpStatUniverse> > su = EpStatUniverse::resolve(framework);
...
LOG(DEBUG) << "done: 5";
      ...
      if (su) {
         optional<std::shared_ptr<modelgbp::gbpe::EpCounter> > myCounter = su.get()->resolveGbpeEpCounter(uuid);
...
LOG(DEBUG) << "done: 7";
            ...
            BOOST_CHECK(myCounter.get()->getTxPackets().get() == dummy_stats[0]);
            BOOST_CHECK(myCounter.get()->getRxPackets().get() == dummy_stats[1]);
            BOOST_CHECK(myCounter.get()->getTxBytes().get() == dummy_stats[2]);
            BOOST_CHECK(myCounter.get()->getRxBytes().get() == dummy_stats[3]);
            BOOST_CHECK(myCounter.get()->getTxDrop().get() == dummy_stats[4]);
            BOOST_CHECK(myCounter.get()->getRxDrop().get() == dummy_stats[5]);
         }
         break;
      }
   }
...
BOOST_FIXTURE_TEST_CASE(useBothConnections,StatsManagerFixture) {
   ...
   dummy_stats_1[0]+= dummy_stats_2[0];
   dummy_stats_1[1]+= dummy_stats_2[1];
   dummy_stats_1[2]+= dummy_stats_2[2];
   dummy_stats_1[3]+= dummy_stats_2[3];
   dummy_stats_1[4]+= dummy_stats_2[4];
   dummy_stats_1[5]+= dummy_stats_2[5];
   ...
   for (const std::string& uuid : endpoints) { // assuming only one entry
      if (su) {
         ...
         if (myCounter) {
            BOOST_CHECK(myCounter.get()->getTxPackets().get() == dummy_stats_1[0]);
            BOOST_CHECK(myCounter.get()->getRxPackets().get() == dummy_stats_1[1]);
            BOOST_CHECK(myCounter.get()->getTxBytes().get() == dummy_stats_1[2]);
            BOOST_CHECK(myCounter.get()->getRxBytes().get() == dummy_stats_1[3]);
            BOOST_CHECK(myCounter.get()->getTxDrop().get() == dummy_stats_1[4]);
            BOOST_CHECK(myCounter.get()->getRxDrop().get() == dummy_stats_1[5]);
         }
         break;
      }
   }
