...
BOOST_FIXTURE_TEST_CASE(useIntConnectionAlone, StatsManagerFixture) {
   ...
   MockConnection integrationPortConn(TEST_CONN_TYPE_INT);
   BOOST_REQUIRE_NO_THROW(statsManager.registerConnection(&integrationPortConn,
                          NULL));
   BOOST_REQUIRE_NO_THROW(statsManager.start());

   BOOST_REQUIRE_NO_THROW(statsManager.Handle(NULL,
                          OFPTYPE_PORT_STATS_REPLY, NULL));
   BOOST_REQUIRE_NO_THROW(statsManager.Handle(&integrationPortConn,
                          OFPTYPE_PORT_STATS_REPLY, NULL));
...
void StatsManagerFixture::verifyCounters(int *dummy_stats, int port_num) {
   ...
   for (const std::string& uuid : endpoints) { // assuming only one entry
      ...
      if (su) {
         ...
         if (myCounter) {
            BOOST_CHECK(myCounter.get()->getRxPackets().get() == dummy_stats[0]);
            BOOST_CHECK(myCounter.get()->getTxPackets().get() == dummy_stats[1]);
            BOOST_CHECK(myCounter.get()->getRxBytes().get() == dummy_stats[2]);
            BOOST_CHECK(myCounter.get()->getTxBytes().get() == dummy_stats[3]);
            BOOST_CHECK(myCounter.get()->getRxDrop().get() == dummy_stats[4]);
            BOOST_CHECK(myCounter.get()->getTxDrop().get() == dummy_stats[5]);
         }
         break;
      }
   }
...
BOOST_FIXTURE_TEST_CASE(useBothConnections, StatsManagerFixture) {
   ...
   BOOST_REQUIRE_NO_THROW(statsManager.Handle(&accessPortConn,
