...
using boost::uuids::basic_random_generator;
    ...
    bool NetFlowJsonRpc::createNetFlow(string brUuid, string targets, string timeout) {
        map<string, string> portUuidMap;
        transData td1;
        td1.operation = "insert";
        td1.table = "NetFlow";
        // active timeout
        std::shared_ptr<StringData> sTimeout = make_shared<StringData>(timeout);
        td1.rows.emplace("active_timeout", sTimeout);
        std::shared_ptr<StringData> sTargets = make_shared<StringData>(targets);
        td1.rows.emplace("targets", sTargets);
        std::shared_ptr<StringData> bAddIsdToInterface = make_shared<StringData>("false");
        td1.rows.emplace("add_id_to_interface", bAddIsdToInterface);
    ...
    bool NetFlowJsonRpc::deleteNetFlow(string brName) {
            list<string> mirList;
