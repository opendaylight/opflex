...
void IntFlowManager::handleLearningBridgeIfaceUpdate(const string& uuid) {
    ...
    shared_ptr<const LearningBridgeIface> ifaceWrapper = lbMgr.getLBIface(uuid);
    ...
    const LearningBridgeIface& iface = *ifaceWrapper;
    uint32_t ofPort = OFPP_NONE;
    ...
    if (ofPort != OFPP_NONE) {
        for (const Mask& m : trunkVlans) {
            ...
            FlowBuilder().priority(501).inPort(ofPort)
                ...
                .build(secFlows);
            // Cookie is assigned based on ofPort; removal of this
            // flow removes all associated learned flows because of
            // the NX_LEARN_F_DELETE_LEARNED set in the learn flags.

            // Two "resubmit" actions essentially treats the resubmit
            // to the LEARN table as a subroutine that will set the
            // output port in reg7 if available then move to the OUT
            // table.

            // setting metadata with reg64 action rather than metadata
            // action to avoid OVS sorting the metadata action to the
            // end.
            FlowBuilder().priority(500).inPort(ofPort)
                .tci(m.first, 0x1fff & m.second)
                .action()
                .reg64(MFF_METADATA, flow::meta::out::LEARN)
                .macVlanLearn(OFP_DEFAULT_PRIORITY,
                              flow::cookie::LEARN | ovs_htonll(ofPort),
                              LEARN_TABLE_ID)
                .resubmit(OFPP_IN_PORT, LEARN_TABLE_ID)
                .resubmit(OFPP_IN_PORT, OUT_TABLE_ID)
                .parent().build(secFlows);
        }
    }
...
handleLearningBridgeVlanUpdate(LearningBridgeIface::vlan_range_t vlan) {
    LOG(DEBUG) << "Updating learning vlan range " << vlan;

    LearningBridgeManager& lbMgr = agent.getLearningBridgeManager();

    unordered_set<string> ifaces;
    lbMgr.getIfacesByVlanRange(vlan, ifaces);
    std::unordered_set<uint16_t> ofPorts;
