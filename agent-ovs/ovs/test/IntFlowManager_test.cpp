...
void BaseIntFlowManagerFixture::initExpLearningBridge() {
    ...
    for (auto& s : secs) {
        ...
        ADDF(Bldr().table(SEC).priority(500).in(s.port)
             .isVlanTci(s.tci)
             .actions()
             .load(METADATA, flow::meta::out::LEARN)
             .learn("table=7,idle_timeout=300,delete_learned,cookie="
                    + (boost::format("0x%lx") %
                       (flow::cookie::LEARN | ovs_htonll(s.port))).str() +
                    ",NXM_OF_VLAN_TCI[],"
                    "NXM_OF_ETH_DST[]=NXM_OF_ETH_SRC[],"
                    "load:NXM_OF_IN_PORT[]->NXM_NX_REG7[0..15]")
             .resubmit(LRN).resubmit(OUT).done());
    }

    ADDF(Bldr().table(OUT).priority(2)
         .reg(OUTPORT, 0).isMdAct(flow::meta::out::LEARN)
         .isVlanTci("0x0ffd/0x1fff")
         .actions().outPort(100).outPort(101).outPort(102).done());
    ADDF(Bldr().table(OUT).priority(2)
         .reg(OUTPORT, 0).isMdAct(flow::meta::out::LEARN)
         .isVlanTci("0x0400/0x1f00")
         .actions().outPort(100).outPort(102).done());
    ADDF(Bldr().table(OUT).priority(2)
         .reg(OUTPORT, 0).isMdAct(flow::meta::out::LEARN)
         .isVlanTci("0x0020/0x1ff0")
