...
ovsagent::Renderer*
SampleRendererPlugin::create(ovsagent::Agent& agent) const {
    return new SampleRenderer(agent);
}

SampleRenderer::SampleRenderer(ovsagent::Agent& agent)
    : ovsagent::Renderer(agent) {
}
...
setProperties(const boost::property_tree::ptree& properties) {
    ...
    LOG(ovsagent::INFO) << "Setting configuration for sample renderer";
}

void SampleRenderer::start() {
    // Called during agent startup
    LOG(ovsagent::INFO) << "Starting sample renderer plugin";
}

void SampleRenderer::stop() {
    // Called during agent shutdown
    LOG(ovsagent::INFO) << "Stopping sample renderer plugin";
}

} /* namespace samplerenderer */

extern "C" const ovsagent::RendererPlugin* init_renderer_plugin() {
