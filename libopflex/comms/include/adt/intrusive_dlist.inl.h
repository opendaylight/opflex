        ...
        )
{
    ...
    assert(new_elem->link[0] == new_elem);
    assert(new_elem->link[1] == new_elem);
...
#define D_INTR_LIST_FOR_EACH_04args(elem, head, hook_name, dir)       \
    for (                                                             \
          ...
          ;                                                           \
            ...
            assert(____si_iter == head || ____si_next != ____si_iter),\
            ____si_iter != head                                       \
          ;                                                           \
...
static inline size_t dlist_len(d_intr_head_t *head) {
    ...
    D_INTR_LIST_FOR_EACH(elem, head, hook) {
        ++len;
    }
