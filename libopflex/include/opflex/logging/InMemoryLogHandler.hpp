...
class InMemoryLogHandler : public OFLogHandler {
  public:
    ...
    bool operator()(Handler& handler) {
        ...
        if (from_->full_) {
            if (!handler.String("chunk0")) {
                return false;
            }

            handler.String(from_->cursor_, from_->to_ - from_->cursor_, false);
        }

        if (!handler.String("chunk1")) {
            return false;
        }

        handler.String(buffer(), from_->cursor_ - buffer(), false);
