...
void sendTransaction(list<transData> tl,shared_ptr<RpcConnection> p,
        uint64_t reqId) {
...
Value getValue(const Value& val, list<string> idx) {
    ...
    for (list<string>::iterator itr=idx.begin(); itr != idx.end();
            itr++) {
        LOG(DEBUG) << "index " << *itr;
        bool isArr = false;
        try {
...
void OvsdbConnection::handleTransaction(uint64_t reqId,
    ...
    VLOG(5) << "OvsdbConnection::handleTransaction";
