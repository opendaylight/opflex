...
#define FORMAT_PROP(gfunc, output)                                      \
    {                                                                   \
...
void MOSerializer::displayObject(std::ostream& ostream,
    ...
    for (pit = pmap.begin(); pit != pmap.end(); ++pit) {
        ...
        case modb::PropertyInfo::STRING:
            FORMAT_PROP(String, oi->getString(pit->first))
            break;
        case modb::PropertyInfo::S64:
            FORMAT_PROP(Int64, oi->getInt64(pit->first))
            break;
        case modb::PropertyInfo::U64:
            FORMAT_PROP(UInt64, oi->getUInt64(pit->first))
            break;
        case modb::PropertyInfo::MAC:
            FORMAT_PROP(MAC, oi->getMAC(pit->first))
            break;
        ...
        case modb::PropertyInfo::ENUM64:
            FORMAT_PROP(UInt64,
                        oi->getUInt64(pit->first)
                        << " (" << getEnumVal(pit->second,
                                              oi->getUInt64(pit->first))
                        << ")");
            break;
        case modb::PropertyInfo::REFERENCE:
            FORMAT_PROP(UInt64,
                        getRefSubj(*store, oi->getReference(pit->first))
                        << "," << oi->getReference(pit->first).second);
            break;
