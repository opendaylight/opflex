 ...
 *
 * In Policy Agent terminology, a "datapath" is a flow-based software switch.
 * A datapath has no intelligence of its own.  Rather, it relies entirely on
 * its client to set up flows.  The datapath layer is core to the Policy Agent
 * software switch: one could say, without much exaggeration, that everything
 * in pag-vswitchd above dpif exists only to make the correct decisions
 * interacting with dpif.
 *
 * Typically, the client of a datapath is the software switch module in
 * "pag-vswitchd", but other clients can be written.  The "pag-dpctl" utility
 ...
 * Policy Agent's pag-vswitchd daemon can directly control what ports are used
 ...
 *
 *      In Policy Agent userspace, "struct flow" is the typical way to describe
 ...
 *      describes the rationale and design.  Refer to PAG_KEY_ATTR_* and
 *      "struct pag_key_*" in include/linux/openvswitch.h for details.
 ...
 *      within a flow.  Some examples of actions are PAG_ACTION_ATTR_OUTPUT,
 *      which transmits the packet out a port, and PAG_ACTION_ATTR_SET, which
 *      modifies packet headers.  Refer to PAG_ACTION_ATTR_* and "struct
 *      pag_action_*" in include/linux/openvswitch.h for details.
 ...
 *
 *    - A datapath action of type PAG_ACTION_ATTR_USERSPACE causes an upcall of
 ...
 * may run "pag-dpctl show" or "pag-dpctl dump-flows" while "pag-vswitchd" is
 ...
 *      any port.  dpif_port_get_pid() also provides this PID.  (pag-vswitchd
...
struct dpif_execute {
     ...
     * of the actions that it can, and for PAG_ACTION_ATTR_OUTPUT and
     * PAG_ACTION_ATTR_USERSPACE actions it passes the packet through to the
     * dpif implementation. */
    bool needs_help;
};
...
enum dpif_upcall_type {
    ...
    DPIF_UC_ACTION,             /* PAG_ACTION_ATTR_USERSPACE action. */
    DPIF_N_UC_TYPES
};
...
struct dpif_upcall {
    ...
    /* DPIF_UC_ACTION only. */
    struct nlattr *userdata;    /* Argument to PAG_ACTION_ATTR_USERSPACE. */
};
