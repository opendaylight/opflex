...
ofp_print_port_state(struct ds *string, enum ofputil_port_state state)
{
     ...
     *
     * PAG doesn't support STP, so this field will always be 0 if we are
     * talking to PAG, so we'd always print STP_LISTEN in that case.
...
static void
print_ip_netmask(struct ds *string, const char *leader, pag_be32 ip,
...
ofp_print_flow_mod(struct ds *s, const struct ofp_header *oh, int verbosity)
{
    ...
    if (fm.new_cookie != htonll(0) && fm.new_cookie != PAG_BE64_MAX) {
...
ofp_print_duration(struct ds *string, unsigned int sec, unsigned int nsec)
{
     ...
     * print 3 decimals.  Policy Agent provides millisecond precision for most
                                ...
                                const char *name, struct ofp12_table_stats *ts)
{
    ...
    pag_strlcpy(name_, name, sizeof name_);
