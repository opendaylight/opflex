...
#define pag_assert(CONDITION)                                           \
    if (!PAG_LIKELY(CONDITION)) {                                       \
        pag_assert_failure(SOURCE_LOCATOR, __func__, #CONDITION);       \
    }
void pag_assert_failure(const char *, const char *, const char *) NO_RETURN;
 ...
 * assigned to OBJECT. */
#ifdef __GNUC__
#define PAG_TYPEOF(OBJECT) typeof(OBJECT)
#else
#define PAG_TYPEOF(OBJECT) void *
#endif
...
void pag_print_version(uint8_t min_ofp, uint8_t max_ofp);
...
void pag_strlcpy(char *dst, const char *src, size_t size);
void pag_strzcpy(char *dst, const char *src, size_t size);

void pag_abort(int err_no, const char *format, ...)
    PRINTF_FORMAT(2, 3) NO_RETURN;
void pag_abort_valist(int err_no, const char *format, va_list)
    PRINTF_FORMAT(2, 0) NO_RETURN;
void pag_fatal(int err_no, const char *format, ...)
    PRINTF_FORMAT(2, 3) NO_RETURN;
void pag_fatal_valist(int err_no, const char *format, va_list)
    PRINTF_FORMAT(2, 0) NO_RETURN;
void pag_error(int err_no, const char *format, ...) PRINTF_FORMAT(2, 3);
void pag_error_valist(int err_no, const char *format, va_list)
    PRINTF_FORMAT(2, 0);
const char *pag_retval_to_string(int);
const char *pag_strerror(int);
void pag_hex_dump(FILE *, const void *, size_t, uintptr_t offset, bool ascii);
...
bool pag_scan(const char *s, const char *format, ...) SCANF_FORMAT(2, 3);
...
bool directory_exist(char *) ;
char *timestring( void );
void *Realloc(void *,size_t) ; 

#ifdef _WIN32

char *pag_format_message(int error);
char *pag_lasterror_to_string(void);
#endif
