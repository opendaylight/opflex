 ...
 * error.  The caller is responsible for freeing the returned string. */
static char * WARN_UNUSED_RESULT
str_to_be64(const char *str, pag_be64 *valuep)
{
    uint64_t value = 0;
    char *error;
...
{
    if (!pag_scan(str, ETH_ADDR_SCAN_FMT, ETH_ADDR_SCAN_ARGS(mac))) {
 ...
 * error.  The caller is responsible for freeing the returned string. */
static char * WARN_UNUSED_RESULT
str_to_ip(const char *str, pag_be32 *ip)
{
 ...
 * error.  The caller is responsible for freeing the returned string. */
static char * WARN_UNUSED_RESULT
parse_named_instruction(enum pag_instruction_type type,
                        char *arg, struct ofpbuf *ofpacts,
                        enum ofputil_protocol *usable_protocols)
{
    char *error_s = NULL;
    ...
    *usable_protocols &= OFPUTIL_P_OF11_UP;
    ...
    case PAGINST_OFPIT11_APPLY_ACTIONS:
        PAG_NOT_REACHED();  /* This case is handled by str_to_inst_ofpacts() */
        break;
                    ...
                    enum ofputil_protocol *usable_protocols)
{
    ...
    while (ofputil_parse_key_value(&pos, &inst, &arg)) {
        type = pag_instruction_type_from_name(inst);
        if (type < 0) {
            ...
            type = PAGINST_OFPIT11_APPLY_ACTIONS;
            if (prev_type == type) {
                n_actions++;
                continue;
            }
        } else if (type == PAGINST_OFPIT11_APPLY_ACTIONS) {
            ofpacts->size = orig_size;
                ...
                enum ofputil_protocol *usable_protocols)
{
    ...
    *usable_protocols = OFPUTIL_P_ANY;
        ...
        if (parse_protocol(name, &p)) {
            ...
            *usable_protocols &= OFPUTIL_P_OF13_UP;
            ...
            if (!strcmp(name, "table")) {
                 ...
                 * "pag-ofctl dump-flows" back into commands that parse
                 * flows. */
            } else {
    ...
    if (!fm->cookie_mask && fm->new_cookie == PAG_BE64_MAX
...
/* Convert 'str_' (as described in the Flow Syntax section of the pag-ofctl man
 ...
 * in the pag-ofctl man page) into 'fmr'.
 *
 * Returns NULL if successful, otherwise a malloc()'d string describing the
 * error.  The caller is responsible for freeing the returned string. */
char * WARN_UNUSED_RESULT
 ...
 * "mod-table" command in the pag-ofctl man page) into 'tm' for sending the
    ...
    while (ofputil_parse_key_value(&pos, &key, &value_s)) {
        ...
        if (parse_protocol(key, &p)) {
            ...
            if (!strcmp(key, "in_port")
                ...
                if (mask) {
                    mask->in_port.ofp_port = u16_to_ofp(ntohs(PAG_BE16_MAX));
                }
            } else {
