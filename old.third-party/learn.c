...
{
    const pag_be32 *p = *pp;
    pag_be32 value = get_unaligned_be32(p);
    *pp = p + 1;
    return value;
}
...
learn_min_len(uint16_t header)
{
    ...
    if (src_type == NX_LEARN_SRC_FIELD) {
        min_len += sizeof(pag_be32); /* src_field */
        min_len += sizeof(pag_be16); /* src_ofs */
    } else {
        min_len += DIV_ROUND_UP(n_bits, 16);
    }
    if (dst_type == NX_LEARN_DST_MATCH ||
        dst_type == NX_LEARN_DST_LOAD) {
        min_len += sizeof(pag_be32); /* dst_field */
        min_len += sizeof(pag_be16); /* dst_ofs */
    }
    return min_len;
}
...
static void
put_be16(struct ofpbuf *b, pag_be16 x)
{
    ofpbuf_put(b, &x, sizeof x);
}

static void
put_be32(struct ofpbuf *b, pag_be32 x)
{
    ofpbuf_put(b, &x, sizeof x);
}
 ...
 * matching OFPACT_LEARN action to 'ofpacts'.  pag-ofctl(8) describes the
 * format parsed.
 *
 * Returns NULL if successful, otherwise a malloc()'d string describing the
 * error.  The caller is responsible for freeing the returned string.
...
/* Appends a description of 'learn' to 's', in the format that pag-ofctl(8)
 * describes. */
void
