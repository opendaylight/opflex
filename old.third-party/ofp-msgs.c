...
static pag_be32 alloc_xid(void);
               ...
               const struct ofp_header *oh, size_t length)
{
    ...
    if (hdrs->type == OFPT_VENDOR) {
        ...
        if (hdrs->stat == OFPST_VENDOR) {
            /* Get vendor. */
            const struct ofp10_vendor_stats_msg *pagm;

            if (length < sizeof *pagm) {
                return OFPERR_OFPBRC_BAD_LEN;
            }

            pagm = (const struct ofp10_vendor_stats_msg *) oh;
            hdrs->vendor = ntohl(pagm->vendor);
            if (hdrs->vendor == NX_VENDOR_ID) {
        ...
        if (hdrs->stat == OFPST_VENDOR) {
            /* Get vendor. */
            const struct ofp11_vendor_stats_msg *pagm;

            if (length < sizeof *pagm) {
                return OFPERR_OFPBRC_BAD_LEN;
            }

            pagm = (const struct ofp11_vendor_stats_msg *) oh;
            hdrs->vendor = ntohl(pagm->vendor);
            if (hdrs->vendor == NX_VENDOR_ID) {
                /* Get Nicira statistic type (NXST_*). */
...
static void ofpraw_put__(enum ofpraw, uint8_t version, pag_be32 xid,
...
struct ofpbuf *
ofpraw_alloc_xid(enum ofpraw raw, uint8_t version, pag_be32 xid,
...
void
ofpraw_put_xid(enum ofpraw raw, uint8_t version, pag_be32 xid,
               struct ofpbuf *buf)
{
...
static void
ofpraw_put__(enum ofpraw raw, uint8_t version, pag_be32 xid,
             size_t extra_tailroom, struct ofpbuf *buf)
{
    ...
    if (hdrs->type == OFPT_VENDOR) {
        ...
        pag_assert(hdrs->vendor == NX_VENDOR_ID);
        ...
        if (hdrs->stat == OFPST_VENDOR) {
            struct ofp10_vendor_stats_msg *pagm = buf->l2;

            pagm->vendor = htonl(hdrs->vendor);
            if (hdrs->vendor == NX_VENDOR_ID) {
        ...
        if (hdrs->stat == OFPST_VENDOR) {
            struct ofp11_vendor_stats_msg *pagm = buf->l2;
...
ofpraw_stats_request_to_reply(enum ofpraw raw, uint8_t version)
{
    ...
    switch ((enum ofp_version)hdrs.version) {
    case OFP10_VERSION:
        pag_assert(hdrs.type == OFPT10_STATS_REQUEST);
        hdrs.type = OFPT10_STATS_REPLY;
        break;
    case OFP11_VERSION:
    case OFP12_VERSION:
    case OFP13_VERSION:
        pag_assert(hdrs.type == OFPT11_STATS_REQUEST);
        hdrs.type = OFPT11_STATS_REPLY;
        break;
    default:
        PAG_NOT_REACHED();
...

static pag_be16 *ofpmp_flags__(const struct ofp_header *);
...
ofpmp_postappend(struct list *replies, size_t start_ofs)
{
    ...
    pag_assert(start_ofs <= UINT16_MAX);
...
raw_info_get(enum ofpraw raw)
{
    ...
    pag_assert(raw < ARRAY_SIZE(raw_infos));
...
{
    pag_assert(version >= info->min_version && version <= info->max_version);
