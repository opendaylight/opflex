...
/* The interface between userspace and kernel uses an "PAG_*" prefix.
 * Since this is fairly non-specific for the PAG userspace components,
 * "ODP_*" (Policy Agent Datapath) is used as the prefix for
...
/* Returns one the following for the action with the given PAG_ACTION_ATTR_*
 * 'type':
 *
...
{
    if (type > PAG_ACTION_ATTR_MAX) {
        return -1;
    }

    switch ((enum pag_action_attr) type) {
    case PAG_ACTION_ATTR_OUTPUT: return sizeof(uint32_t);
    case PAG_ACTION_ATTR_USERSPACE: return -2;
    case PAG_ACTION_ATTR_PUSH_VLAN: return sizeof(struct pag_action_push_vlan);
    case PAG_ACTION_ATTR_POP_VLAN: return 0;
    case PAG_ACTION_ATTR_PUSH_MPLS: return sizeof(struct pag_action_push_mpls);
    case PAG_ACTION_ATTR_POP_MPLS: return sizeof(pag_be16);
    case PAG_ACTION_ATTR_SET: return -2;
    case PAG_ACTION_ATTR_SAMPLE: return -2;
 ...
 * should be at least PAG_KEY_ATTR_BUFSIZE. */
enum { PAG_KEY_ATTR_BUFSIZE = 3 + INT_STRLEN(unsigned int) + 1 };
static const char *
pag_key_attr_to_string(enum pag_key_attr attr, char *namebuf, size_t bufsize)
{
    switch (attr) {
    case PAG_KEY_ATTR_UNSPEC: return "unspec";
    case PAG_KEY_ATTR_ENCAP: return "encap";
    case PAG_KEY_ATTR_PRIORITY: return "skb_priority";
    case PAG_KEY_ATTR_SKB_MARK: return "skb_mark";
    case PAG_KEY_ATTR_TUNNEL: return "tunnel";
    case PAG_KEY_ATTR_IN_PORT: return "in_port";
    case PAG_KEY_ATTR_ETHERNET: return "eth";
    case PAG_KEY_ATTR_VLAN: return "vlan";
    case PAG_KEY_ATTR_ETHERTYPE: return "eth_type";
    case PAG_KEY_ATTR_IPV4: return "ipv4";
    case PAG_KEY_ATTR_IPV6: return "ipv6";
    case PAG_KEY_ATTR_TCP: return "tcp";
    case PAG_KEY_ATTR_TCP_FLAGS: return "tcp_flags";
    case PAG_KEY_ATTR_UDP: return "udp";
    case PAG_KEY_ATTR_SCTP: return "sctp";
    case PAG_KEY_ATTR_ICMP: return "icmp";
    case PAG_KEY_ATTR_ICMPV6: return "icmpv6";
    case PAG_KEY_ATTR_ARP: return "arp";
    case PAG_KEY_ATTR_ND: return "nd";
    case PAG_KEY_ATTR_MPLS: return "mpls";

    case __PAG_KEY_ATTR_MAX:
    default:
...
{
    static const struct nl_policy pag_sample_policy[] = {
        { NL_A_NO_ATTR, 0, 0, false }, /* PAG_SAMPLE_ATTR_UNSPEC */
        { NL_A_U32, 0, 0, false },     /* PAG_SAMPLE_ATTR_PROBABILITY */
        { NL_A_NESTED, 0, 0, false },  /* PAG_SAMPLE_ATTR_ACTIONS */
    };
    struct nlattr *a[ARRAY_SIZE(pag_sample_policy)];
    double percentage;
    ...
    if (!nl_parse_nested(attr, pag_sample_policy, a, ARRAY_SIZE(a))) {
        ds_put_cstr(ds, "(error)");
        return;
    }

    percentage = (100.0 * nl_attr_get_u32(a[PAG_SAMPLE_ATTR_PROBABILITY])) /
                        UINT32_MAX;

    ds_put_format(ds, "(sample=%.1f%%,", percentage);

    ds_put_cstr(ds, "actions(");
    nla_acts = nl_attr_get(a[PAG_SAMPLE_ATTR_ACTIONS]);
    len = nl_attr_get_size(a[PAG_SAMPLE_ATTR_ACTIONS]);
            ...
            uint32_t *res)
{
    ...
    while (s[n] != ')') {
        ...
        if (pag_scan(&s[n], "%lli%n", &flags, &n0)) {
            n += n0 + (s[n + n0] == ',');
            result |= flags;
            continue;
        }
...
{
    static const struct nl_policy pag_userspace_policy[] = {
        { NL_A_NO_ATTR, 0, 0, false }, /* PAG_USERSPACE_ATTR_UNSPEC */
        { NL_A_U32, 0, 0, false },     /* PAG_USERSPACE_ATTR_PID */
        { NL_A_UNSPEC, 0, 0, true },   /* PAG_USERSPACE_ATTR_USERDATA */
    };
    struct nlattr *a[ARRAY_SIZE(pag_userspace_policy)];
    const struct nlattr *userdata_attr;

    if (!nl_parse_nested(attr, pag_userspace_policy, a, ARRAY_SIZE(a))) {
        ds_put_cstr(ds, "userspace(error)");
        return;
    }

    ds_put_format(ds, "userspace(pid=%"PRIu32,
                  nl_attr_get_u32(a[PAG_USERSPACE_ATTR_PID]));

    userdata_attr = a[PAG_USERSPACE_ATTR_USERDATA];
...
static void
format_vlan_tci(struct ds *ds, pag_be16 vlan_tci)
{
...
static void
format_mpls_lse(struct ds *ds, pag_be32 mpls_lse)
{
...
static void
format_mpls(struct ds *ds, const struct pag_key_mpls *mpls_key,
            const struct pag_key_mpls *mpls_mask, int n)
{
    if (n == 1) {
        pag_be32 key = mpls_key->mpls_lse;

        if (mpls_mask == NULL) {
            format_mpls_lse(ds, key);
        } else {
            pag_be32 mask = mpls_mask->mpls_lse;
...
{
    int expected_len;
    enum pag_action_attr type = nl_attr_type(a);
    const struct pag_action_push_vlan *vlan;
    ...
    case PAG_ACTION_ATTR_PUSH_MPLS: {
        const struct pag_action_push_mpls *mpls = nl_attr_get(a);
    ...
    case PAG_ACTION_ATTR_POP_MPLS: {
        pag_be16 ethertype = nl_attr_get_be16(a);
...
{
    {
        ...
        if (pag_scan(s, "%"SCNi32"%n", &port, &n)) {
            nl_msg_put_u32(actions, PAG_ACTION_ATTR_OUTPUT, port);
            return n;
        }
    }
    ...
    if (port_names) {
        ...
        if (node) {
            nl_msg_put_u32(actions, PAG_ACTION_ATTR_OUTPUT, node->data);
            return len;
        }
    }

    {
        ...
        if (pag_scan(s, "userspace(pid=%"SCNi32")%n", &pid, &n)) {
            odp_put_userspace_action(pid, NULL, 0, actions);
            return n;
        } else if (pag_scan(s, "userspace(pid=%"SCNi32",sFlow(vid=%i,"
            ...
            odp_put_userspace_action(pid, &cookie, sizeof cookie.sflow,
                                     actions);
            return n;
        } else if (pag_scan(s, "userspace(pid=%"SCNi32",slow_path%n",
                            &pid, &n)) {
            union user_action_cookie cookie;
            int res;
            ...
            odp_put_userspace_action(pid, &cookie, sizeof cookie.slow_path,
                                     actions);
            return n;
        } else if (pag_scan(s, "userspace(pid=%"SCNi32","
            ...
            odp_put_userspace_action(pid, &cookie, sizeof cookie.flow_sample,
                                     actions);
            return n;
        } else if (pag_scan(s, "userspace(pid=%"SCNi32",ipfix)%n", &pid, &n)) {
            union user_action_cookie cookie;
            ...
            odp_put_userspace_action(pid, &cookie, sizeof cookie.ipfix,
                                     actions);
            return n;
        } else if (pag_scan(s, "userspace(pid=%"SCNi32",userdata(%n",
                            &pid, &n)) {
            struct ofpbuf buf;
            char *end;
    ...
    if (!strncmp(s, "set(", 4)) {
        ...
        start_ofs = nl_msg_start_nested(actions, PAG_ACTION_ATTR_SET);
    ...
    {
        struct pag_action_push_vlan push;
        int tpid = ETH_TYPE_VLAN;
        int vid, pcp;
        int cfi = 1;
        int n = -1;

        if (pag_scan(s, "push_vlan(vid=%i,pcp=%i)%n", &vid, &pcp, &n)
            || pag_scan(s, "push_vlan(vid=%i,pcp=%i,cfi=%i)%n",
                        &vid, &pcp, &cfi, &n)
            || pag_scan(s, "push_vlan(tpid=%i,vid=%i,pcp=%i)%n",
                        &tpid, &vid, &pcp, &n)
            || pag_scan(s, "push_vlan(tpid=%i,vid=%i,pcp=%i,cfi=%i)%n",
                        &tpid, &vid, &pcp, &cfi, &n)) {
            push.vlan_tpid = htons(tpid);
            push.vlan_tci = htons((vid << VLAN_VID_SHIFT)
                                  | (pcp << VLAN_PCP_SHIFT)
                                  | (cfi ? VLAN_CFI : 0));
            nl_msg_put_unspec(actions, PAG_ACTION_ATTR_PUSH_VLAN,
                              &push, sizeof push);
    ...
    if (!strncmp(s, "pop_vlan", 8)) {
        nl_msg_put_flag(actions, PAG_ACTION_ATTR_POP_VLAN);
        return 8;
    }

    {
        ...
        if (pag_scan(s, "sample(sample=%lf%%,actions(%n", &percentage, &n)
            && percentage >= 0. && percentage <= 100.0) {
            size_t sample_ofs, actions_ofs;
            double probability;

            probability = floor(UINT32_MAX * (percentage / 100.0) + .5);
            sample_ofs = nl_msg_start_nested(actions, PAG_ACTION_ATTR_SAMPLE);
            nl_msg_put_u32(actions, PAG_SAMPLE_ATTR_PROBABILITY,
                           (probability <= 0 ? 0
                            : probability >= UINT32_MAX ? UINT32_MAX
                            : probability));

            actions_ofs = nl_msg_start_nested(actions,
                                              PAG_SAMPLE_ATTR_ACTIONS);
            for (;;) {
                int retval;
...
{
    if (type > PAG_KEY_ATTR_MAX) {
        return -1;
    }

    switch ((enum pag_key_attr) type) {
    case PAG_KEY_ATTR_ENCAP: return -2;
    case PAG_KEY_ATTR_PRIORITY: return 4;
    case PAG_KEY_ATTR_SKB_MARK: return 4;
    case PAG_KEY_ATTR_TUNNEL: return -2;
    case PAG_KEY_ATTR_IN_PORT: return 4;
    case PAG_KEY_ATTR_ETHERNET: return sizeof(struct pag_key_ethernet);
    case PAG_KEY_ATTR_VLAN: return sizeof(pag_be16);
    case PAG_KEY_ATTR_ETHERTYPE: return 2;
    case PAG_KEY_ATTR_MPLS: return -2;
    case PAG_KEY_ATTR_IPV4: return sizeof(struct pag_key_ipv4);
    case PAG_KEY_ATTR_IPV6: return sizeof(struct pag_key_ipv6);
    case PAG_KEY_ATTR_TCP: return sizeof(struct pag_key_tcp);
    case PAG_KEY_ATTR_TCP_FLAGS: return 2;
    case PAG_KEY_ATTR_UDP: return sizeof(struct pag_key_udp);
    case PAG_KEY_ATTR_SCTP: return sizeof(struct pag_key_sctp);
    case PAG_KEY_ATTR_ICMP: return sizeof(struct pag_key_icmp);
    case PAG_KEY_ATTR_ICMPV6: return sizeof(struct pag_key_icmpv6);
    case PAG_KEY_ATTR_ARP: return sizeof(struct pag_key_arp);
    case PAG_KEY_ATTR_ND: return sizeof(struct pag_key_nd);

    case PAG_KEY_ATTR_UNSPEC:
    case __PAG_KEY_ATTR_MAX:
        return -1;
    }
...
static const char *
pag_frag_type_to_string(enum pag_frag_type type)
{
    switch (type) {
    case PAG_FRAG_TYPE_NONE:
        return "no";
    case PAG_FRAG_TYPE_FIRST:
        return "first";
    case PAG_FRAG_TYPE_LATER:
        return "later";
    case __PAG_FRAG_TYPE_MAX:
    default:
        return "<error>";
    }
}
...
{
    switch (type) {
    case PAG_TUNNEL_KEY_ATTR_ID: return 8;
    case PAG_TUNNEL_KEY_ATTR_IPV4_SRC: return 4;
    case PAG_TUNNEL_KEY_ATTR_IPV4_DST: return 4;
    case PAG_TUNNEL_KEY_ATTR_TOS: return 1;
    case PAG_TUNNEL_KEY_ATTR_TTL: return 1;
    case PAG_TUNNEL_KEY_ATTR_DONT_FRAGMENT: return 0;
    case PAG_TUNNEL_KEY_ATTR_CSUM: return 0;
    case __PAG_TUNNEL_KEY_ATTR_MAX:
        return -1;
    }
    return -1;
}
...
odp_tun_key_from_attr(const struct nlattr *attr, struct flow_tnl *tun)
{
    ...
    NL_NESTED_FOR_EACH(a, left, attr) {
        ...
        switch (type) {
        case PAG_TUNNEL_KEY_ATTR_ID:
            tun->tun_id = nl_attr_get_be64(a);
            tun->flags |= FLOW_TNL_F_KEY;
            break;
        case PAG_TUNNEL_KEY_ATTR_IPV4_SRC:
            tun->ip_src = nl_attr_get_be32(a);
            break;
        case PAG_TUNNEL_KEY_ATTR_IPV4_DST:
            tun->ip_dst = nl_attr_get_be32(a);
            break;
        case PAG_TUNNEL_KEY_ATTR_TOS:
            tun->ip_tos = nl_attr_get_u8(a);
            break;
        case PAG_TUNNEL_KEY_ATTR_TTL:
            tun->ip_ttl = nl_attr_get_u8(a);
            ttl = true;
            break;
        case PAG_TUNNEL_KEY_ATTR_DONT_FRAGMENT:
            tun->flags |= FLOW_TNL_F_DONT_FRAGMENT;
            break;
        case PAG_TUNNEL_KEY_ATTR_CSUM:
...
tun_key_to_attr(struct ofpbuf *a, const struct flow_tnl *tun_key)
{
    ...
    tun_key_ofs = nl_msg_start_nested(a, PAG_KEY_ATTR_TUNNEL);

    /* tun_id != 0 without FLOW_TNL_F_KEY is valid if tun_key is a mask. */
    if (tun_key->tun_id || tun_key->flags & FLOW_TNL_F_KEY) {
        nl_msg_put_be64(a, PAG_TUNNEL_KEY_ATTR_ID, tun_key->tun_id);
    }
    if (tun_key->ip_src) {
        nl_msg_put_be32(a, PAG_TUNNEL_KEY_ATTR_IPV4_SRC, tun_key->ip_src);
    }
    if (tun_key->ip_dst) {
        nl_msg_put_be32(a, PAG_TUNNEL_KEY_ATTR_IPV4_DST, tun_key->ip_dst);
    }
    if (tun_key->ip_tos) {
        nl_msg_put_u8(a, PAG_TUNNEL_KEY_ATTR_TOS, tun_key->ip_tos);
    }
    nl_msg_put_u8(a, PAG_TUNNEL_KEY_ATTR_TTL, tun_key->ip_ttl);
    if (tun_key->flags & FLOW_TNL_F_DONT_FRAGMENT) {
        nl_msg_put_flag(a, PAG_TUNNEL_KEY_ATTR_DONT_FRAGMENT);
    }
    if (tun_key->flags & FLOW_TNL_F_CSUM) {
        nl_msg_put_flag(a, PAG_TUNNEL_KEY_ATTR_CSUM);
    }
...
{
    bool is_exact = false;
    enum pag_key_attr attr = nl_attr_type(ma);

    if (attr == PAG_KEY_ATTR_TUNNEL) {
                    ...
                    bool verbose)
{
    ...
    enum pag_key_attr attr = nl_attr_type(a);
    char namebuf[PAG_KEY_ATTR_BUFSIZE];
    int expected_len;
    bool is_exact;

    is_exact = ma ? odp_mask_attr_is_exact(ma) : true;

    ds_put_cstr(ds, pag_key_attr_to_string(attr, namebuf, sizeof namebuf));
    ...
    case PAG_KEY_ATTR_PRIORITY:
    case PAG_KEY_ATTR_SKB_MARK:
    ...
    case PAG_KEY_ATTR_ETHERNET:
        if (!is_exact) {
            const struct pag_key_ethernet *eth_mask = nl_attr_get(ma);
            const struct pag_key_ethernet *eth_key = nl_attr_get(a);
    ...
    case PAG_KEY_ATTR_VLAN:
        {
            pag_be16 vlan_tci = nl_attr_get_be16(a);
            if (!is_exact) {
                pag_be16 mask = nl_attr_get_be16(ma);
    ...
    case PAG_KEY_ATTR_MPLS: {
        const struct pag_key_mpls *mpls_key = nl_attr_get(a);
        const struct pag_key_mpls *mpls_mask = NULL;
    ...
    case PAG_KEY_ATTR_IPV4:
        if (!is_exact) {
            const struct pag_key_ipv4 *ipv4_key = nl_attr_get(a);
            const struct pag_key_ipv4 *ipv4_mask = nl_attr_get(ma);
            ...
            ds_put_format(ds, "src="IP_FMT"/"IP_FMT",dst="IP_FMT"/"IP_FMT
                          ...
                          pag_frag_type_to_string(ipv4_key->ipv4_frag),
                          ipv4_mask->ipv4_frag);
        } else {
            const struct pag_key_ipv4 *ipv4_key = nl_attr_get(a);
    ...
    case PAG_KEY_ATTR_IPV6:
        if (!is_exact) {
            const struct pag_key_ipv6 *ipv6_key, *ipv6_mask;
            ...
            ds_put_format(ds, "src=%s/%s,dst=%s/%s,label=%#"PRIx32"/%#"PRIx32
                          ...
                          pag_frag_type_to_string(ipv6_key->ipv6_frag),
                          ipv6_mask->ipv6_frag);
        } else {
            const struct pag_key_ipv6 *ipv6_key;
            char src_str[INET6_ADDRSTRLEN];
            char dst_str[INET6_ADDRSTRLEN];
    ...
    case PAG_KEY_ATTR_TCP:
        if (!is_exact) {
            const struct pag_key_tcp *tcp_mask = nl_attr_get(ma);
            const struct pag_key_tcp *tcp_key = nl_attr_get(a);
    ...
    case PAG_KEY_ATTR_UDP:
        if (!is_exact) {
            const struct pag_key_udp *udp_mask = nl_attr_get(ma);
            const struct pag_key_udp *udp_key = nl_attr_get(a);
    ...
    case PAG_KEY_ATTR_SCTP:
        if (ma) {
            const struct pag_key_sctp *sctp_mask = nl_attr_get(ma);
            const struct pag_key_sctp *sctp_key = nl_attr_get(a);
    ...
    case PAG_KEY_ATTR_ICMP:
        if (!is_exact) {
            const struct pag_key_icmp *icmp_mask = nl_attr_get(ma);
            const struct pag_key_icmp *icmp_key = nl_attr_get(a);
    ...
    case PAG_KEY_ATTR_ICMPV6:
        if (!is_exact) {
            const struct pag_key_icmpv6 *icmpv6_mask = nl_attr_get(ma);
            const struct pag_key_icmpv6 *icmpv6_key = nl_attr_get(a);
    ...
    case PAG_KEY_ATTR_ARP:
        if (!is_exact) {
            const struct pag_key_arp *arp_mask = nl_attr_get(ma);
            const struct pag_key_arp *arp_key = nl_attr_get(a);
    ...
    case PAG_KEY_ATTR_ND: {
        const struct pag_key_nd *nd_key, *nd_mask = NULL;
 ...
 * PAG_KEY_ATTR_* attributes in 'key'. If non-null, additionally formats the
                ...
                const struct hmap *portno_names, struct ds *ds, bool verbose)
{
    if (key_len) {
        ...
        NL_ATTR_FOR_EACH (a, left, key, key_len) {
            ...
            if (attr_type == PAG_KEY_ATTR_ETHERTYPE) {
        ...
        if (!has_ethtype_key) {
            ma = nl_attr_find__(mask, mask_len, PAG_KEY_ATTR_ETHERTYPE);
            if (ma) {
...
/* Appends to 'ds' a string representation of the 'key_len' bytes of
 * PAG_KEY_ATTR_* attributes in 'key'. */
void
...
static void
put_nd(struct pag_key_nd* nd_key, const uint8_t *nd_sll,
       const uint8_t *nd_tll, struct ofpbuf *key)
{
    ...
    nl_msg_put_unspec(key, PAG_KEY_ATTR_ND, nd_key, sizeof *nd_key);
}
...
static bool
pag_frag_type_from_string(const char *s, enum pag_frag_type *type)
{
    if (!strcasecmp(s, "no")) {
        *type = PAG_FRAG_TYPE_NONE;
    } else if (!strcasecmp(s, "first")) {
        *type = PAG_FRAG_TYPE_FIRST;
    } else if (!strcasecmp(s, "later")) {
        *type = PAG_FRAG_TYPE_LATER;
    } else {
        return false;
    }
    return true;
...
{
    {
        ...
        if (mask && pag_scan(s, "skb_priority(%"SCNi32"/%"SCNi32")%n",
                             &priority, &priority_mask, &n)) {
            nl_msg_put_u32(key, PAG_KEY_ATTR_PRIORITY, priority);
            nl_msg_put_u32(mask, PAG_KEY_ATTR_PRIORITY, priority_mask);
            return n;
        } else if (pag_scan(s, "skb_priority(%"SCNi32")%n", &priority, &n)) {
            nl_msg_put_u32(key, PAG_KEY_ATTR_PRIORITY, priority);
            if (mask) {
                nl_msg_put_u32(mask, PAG_KEY_ATTR_PRIORITY, UINT32_MAX);
            }
            return n;
        }
    }

    {
        ...
        if (mask && pag_scan(s, "skb_mark(%"SCNi32"/%"SCNi32")%n", &mark,
                             &mark_mask, &n)) {
            nl_msg_put_u32(key, PAG_KEY_ATTR_SKB_MARK, mark);
            nl_msg_put_u32(mask, PAG_KEY_ATTR_SKB_MARK, mark_mask);
            return n;
        } else if (pag_scan(s, "skb_mark(%"SCNi32")%n", &mark, &n)) {
            nl_msg_put_u32(key, PAG_KEY_ATTR_SKB_MARK, mark);
            if (mask) {
                nl_msg_put_u32(mask, PAG_KEY_ATTR_SKB_MARK, UINT32_MAX);
            }
            return n;
        }
    }

    {
        ...
        if (mask && pag_scan(s, "tunnel(tun_id=%"SCNi64"/%"SCNi64","
            ...
            return n;
        } else if (pag_scan(s, "tunnel(tun_id=%"SCNi64","
...

    {
        ...
        if (mask && pag_scan(s, "in_port(%"SCNi32"/%"SCNi32")%n",
                             &in_port, &in_port_mask, &n)) {
            nl_msg_put_u32(key, PAG_KEY_ATTR_IN_PORT, in_port);
            nl_msg_put_u32(mask, PAG_KEY_ATTR_IN_PORT, in_port_mask);
            return n;
        } else if (pag_scan(s, "in_port(%"SCNi32")%n", &in_port, &n)) {
            nl_msg_put_u32(key, PAG_KEY_ATTR_IN_PORT, in_port);
            if (mask) {
                nl_msg_put_u32(mask, PAG_KEY_ATTR_IN_PORT, UINT32_MAX);
            }
            return n;
        }
    }
    ...
    if (port_names && !strncmp(s, "in_port(", 8)) {
        ...
        if (node) {
            nl_msg_put_u32(key, PAG_KEY_ATTR_IN_PORT, node->data);
    ...
    {
        struct pag_key_ethernet eth_key;
        struct pag_key_ethernet eth_key_mask;
        int n = -1;
        ...
        if (mask && pag_scan(s,
            ...
            nl_msg_put_unspec(key, PAG_KEY_ATTR_ETHERNET,
                              &eth_key, sizeof eth_key);
            nl_msg_put_unspec(mask, PAG_KEY_ATTR_ETHERNET,
                              &eth_key_mask, sizeof eth_key_mask);
            return n;
        } else if (pag_scan(s, "eth(src="ETH_ADDR_SCAN_FMT","
...

    {
        ...
        if (mask && pag_scan(s, "vlan(vid=%i/%i,pcp=%i/%i)%n",
                            &vid, &vid_mask, &pcp, &pcp_mask, &n)) {
            nl_msg_put_be16(key, PAG_KEY_ATTR_VLAN,
                            htons((vid << VLAN_VID_SHIFT) |
                                  (pcp << VLAN_PCP_SHIFT) |
                                  VLAN_CFI));
            nl_msg_put_be16(mask, PAG_KEY_ATTR_VLAN,
                            htons((vid_mask << VLAN_VID_SHIFT) |
                                  (pcp_mask << VLAN_PCP_SHIFT) |
                                  (1 << VLAN_CFI_SHIFT)));
            return n;
        } else if (pag_scan(s, "vlan(vid=%i,pcp=%i)%n", &vid, &pcp, &n)) {
            nl_msg_put_be16(key, PAG_KEY_ATTR_VLAN,
                            htons((vid << VLAN_VID_SHIFT) |
                                  (pcp << VLAN_PCP_SHIFT) |
                                  VLAN_CFI));
            if (mask) {
                nl_msg_put_be16(mask, PAG_KEY_ATTR_VLAN, PAG_BE16_MAX);
            }
            return n;
        } else if (mask
                   && pag_scan(s, "vlan(vid=%i/%i,pcp=%i/%i,cfi=%i/%i)%n",
            ...
            return n;
        } else if (pag_scan(s, "vlan(vid=%i,pcp=%i,cfi=%i)%n",
                            &vid, &pcp, &cfi, &n)) {
            nl_msg_put_be16(key, PAG_KEY_ATTR_VLAN,
                            htons((vid << VLAN_VID_SHIFT) |
                                  (pcp << VLAN_PCP_SHIFT) |
...

    {
        ...
        if (mask && pag_scan(s, "eth_type(%i/%i)%n",
                             &eth_type, &eth_type_mask, &n)) {
            if (eth_type != 0) {
                nl_msg_put_be16(key, PAG_KEY_ATTR_ETHERTYPE, htons(eth_type));
            }
            nl_msg_put_be16(mask, PAG_KEY_ATTR_ETHERTYPE, htons(eth_type_mask));
            return n;
        } else if (pag_scan(s, "eth_type(%i)%n", &eth_type, &n)) {
            nl_msg_put_be16(key, PAG_KEY_ATTR_ETHERTYPE, htons(eth_type));
            if (mask) {
                nl_msg_put_be16(mask, PAG_KEY_ATTR_ETHERTYPE, PAG_BE16_MAX);
            }
            return n;
        }
    }

    {
        ...
        if (mask && pag_scan(s, "mpls(label=%i/%i,tc=%i/%i,"
                             "ttl=%i/%i,bos=%i/%i)%n",
                             &label, &label_mask, &tc, &tc_mask,
                             &ttl, &ttl_mask, &bos, &bos_mask, &n)) {
            struct pag_key_mpls *mpls, *mpls_mask;

            mpls = nl_msg_put_unspec_uninit(key, PAG_KEY_ATTR_MPLS,
                                            sizeof *mpls);
            mpls->mpls_lse = mpls_lse_from_components(label, tc, ttl, bos);

            mpls_mask = nl_msg_put_unspec_uninit(mask, PAG_KEY_ATTR_MPLS,
                                            sizeof *mpls_mask);
            mpls_mask->mpls_lse = mpls_lse_from_components(
                                  label_mask, tc_mask, ttl_mask, bos_mask);
            return n;
        } else if (pag_scan(s, "mpls(label=%i,tc=%i,ttl=%i,bos=%i)%n",
                            &label, &tc, &ttl, &bos, &n)) {
            struct pag_key_mpls *mpls;

            mpls = nl_msg_put_unspec_uninit(key, PAG_KEY_ATTR_MPLS,
                                            sizeof *mpls);
            mpls->mpls_lse = mpls_lse_from_components(label, tc, ttl, bos);
            if (mask) {
                mpls = nl_msg_put_unspec_uninit(mask, PAG_KEY_ATTR_MPLS,
                                            sizeof *mpls);
                mpls->mpls_lse = PAG_BE32_MAX;
            }
            return n;
        }
    }
    ...
    {
        struct pag_key_ipv4 ipv4_key;
        struct pag_key_ipv4 ipv4_mask;

        char frag[8];
        enum pag_frag_type ipv4_frag;
        int n = -1;

        if (mask
            && pag_scan(s, "ipv4(src="IP_SCAN_FMT"/"IP_SCAN_FMT","
            ...
            && pag_frag_type_from_string(frag, &ipv4_frag)) {
            ipv4_key.ipv4_frag = ipv4_frag;
            nl_msg_put_unspec(key, PAG_KEY_ATTR_IPV4,
                              &ipv4_key, sizeof ipv4_key);

            nl_msg_put_unspec(mask, PAG_KEY_ATTR_IPV4,
                              &ipv4_mask, sizeof ipv4_mask);
            return n;
        } else if (pag_scan(s, "ipv4(src="IP_SCAN_FMT",dst="IP_SCAN_FMT","
...

    {
        ...
        char frag[8];
        enum pag_frag_type ipv6_frag;
        int ipv6_frag_mask;
        int n = -1;

        if (mask && pag_scan(s, "ipv6(src="IPV6_SCAN_FMT"/"IPV6_SCAN_FMT",dst="
                             ...
                             &ipv6_frag_mask, &n)
            && pag_frag_type_from_string(frag, &ipv6_frag)) {
            struct pag_key_ipv6 ipv6_key;
            struct pag_key_ipv6 ipv6_mask;
            ...
            nl_msg_put_unspec(key, PAG_KEY_ATTR_IPV6,
            ...
            nl_msg_put_unspec(mask, PAG_KEY_ATTR_IPV6,
                              &ipv6_mask, sizeof ipv6_mask);
            return n;
        } else if (pag_scan(s, "ipv6(src="IPV6_SCAN_FMT",dst="IPV6_SCAN_FMT","
...

    {
        ...
        if (mask && pag_scan(s, "tcp(src=%i/%i,dst=%i/%i)%n",
                             &tcp_src, &tcp_src_mask, &tcp_dst,
                             &tcp_dst_mask, &n)) {
            struct pag_key_tcp tcp_key;
            struct pag_key_tcp tcp_mask;

            tcp_key.tcp_src = htons(tcp_src);
            tcp_key.tcp_dst = htons(tcp_dst);
            nl_msg_put_unspec(key, PAG_KEY_ATTR_TCP, &tcp_key, sizeof tcp_key);

            tcp_mask.tcp_src = htons(tcp_src_mask);
            tcp_mask.tcp_dst = htons(tcp_dst_mask);
            nl_msg_put_unspec(mask, PAG_KEY_ATTR_TCP,
                              &tcp_mask, sizeof tcp_mask);
            return n;
        } else if (pag_scan(s, "tcp(src=%i,dst=%i)%n",
                            &tcp_src, &tcp_dst, &n)) {
            struct pag_key_tcp tcp_key;
...

    {
        ...
        if (mask && pag_scan(s, "tcp_flags(%"SCNi16"/%"SCNi16")%n",
                             &tcp_flags, &tcp_flags_mask, &n) > 0 && n > 0) {
            nl_msg_put_be16(key, PAG_KEY_ATTR_TCP_FLAGS, htons(tcp_flags));
            nl_msg_put_be16(mask, PAG_KEY_ATTR_TCP_FLAGS, htons(tcp_flags_mask));
            return n;
        } else if (pag_scan(s, "tcp_flags(%"SCNi16")%n", &tcp_flags, &n)) {
            nl_msg_put_be16(key, PAG_KEY_ATTR_TCP_FLAGS, htons(tcp_flags));
            if (mask) {
                nl_msg_put_be16(mask, PAG_KEY_ATTR_TCP_FLAGS,
                                htons(UINT16_MAX));
            }
            return n;
        }
...

    {
        ...
        if (mask && pag_scan(s, "udp(src=%i/%i,dst=%i/%i)%n",
                             &udp_src, &udp_src_mask,
                             &udp_dst, &udp_dst_mask, &n)) {
            struct pag_key_udp udp_key;
            struct pag_key_udp udp_mask;

            udp_key.udp_src = htons(udp_src);
            udp_key.udp_dst = htons(udp_dst);
            nl_msg_put_unspec(key, PAG_KEY_ATTR_UDP, &udp_key, sizeof udp_key);

            udp_mask.udp_src = htons(udp_src_mask);
            udp_mask.udp_dst = htons(udp_dst_mask);
            nl_msg_put_unspec(mask, PAG_KEY_ATTR_UDP,
                              &udp_mask, sizeof udp_mask);
            return n;
        }
        if (pag_scan(s, "udp(src=%i,dst=%i)%n", &udp_src, &udp_dst, &n)) {
            struct pag_key_udp udp_key;
            ...
            if (mask) {
                ...
                nl_msg_put_unspec(mask, PAG_KEY_ATTR_UDP, &udp_key, sizeof udp_key);
            }
            return n;
        }
    }

    {
        ...
        if (mask && pag_scan(s, "sctp(src=%i/%i,dst=%i/%i)%n",
                             &sctp_src, &sctp_src_mask,
                             &sctp_dst, &sctp_dst_mask, &n)) {
            struct pag_key_sctp sctp_key;
            struct pag_key_sctp sctp_mask;

            sctp_key.sctp_src = htons(sctp_src);
            sctp_key.sctp_dst = htons(sctp_dst);
            nl_msg_put_unspec(key, PAG_KEY_ATTR_SCTP, &sctp_key, sizeof sctp_key);

            sctp_mask.sctp_src = htons(sctp_src_mask);
            sctp_mask.sctp_dst = htons(sctp_dst_mask);
            nl_msg_put_unspec(mask, PAG_KEY_ATTR_SCTP,
                              &sctp_mask, sizeof sctp_mask);
            return n;
        }
        if (pag_scan(s, "sctp(src=%i,dst=%i)%n", &sctp_src, &sctp_dst, &n)) {
            struct pag_key_sctp sctp_key;
            ...
            if (mask) {
                ...
                nl_msg_put_unspec(mask, PAG_KEY_ATTR_SCTP, &sctp_key, sizeof sctp_key);
            }
            return n;
        }
    }
    ...
    {
        struct pag_key_icmp icmp_key;
        struct pag_key_icmp icmp_mask;
        int n = -1;

        if (mask && pag_scan(s, "icmp(type=%"SCNi8"/%"SCNi8","
                             "code=%"SCNi8"/%"SCNi8")%n",
                   &icmp_key.icmp_type, &icmp_mask.icmp_type,
                   &icmp_key.icmp_code, &icmp_mask.icmp_code, &n)) {
            nl_msg_put_unspec(key, PAG_KEY_ATTR_ICMP,
                              &icmp_key, sizeof icmp_key);
            nl_msg_put_unspec(mask, PAG_KEY_ATTR_ICMP,
                              &icmp_mask, sizeof icmp_mask);
            return n;
        } else if (pag_scan(s, "icmp(type=%"SCNi8",code=%"SCNi8")%n",
                            &icmp_key.icmp_type, &icmp_key.icmp_code, &n)) {
            nl_msg_put_unspec(key, PAG_KEY_ATTR_ICMP,
                              &icmp_key, sizeof icmp_key);
            if (mask) {
                memset(&icmp_key, 0xff, sizeof icmp_key);
                nl_msg_put_unspec(mask, PAG_KEY_ATTR_ICMP, &icmp_key,
                              sizeof icmp_key);
            }
            return n;
        }
    ...
    {
        struct pag_key_icmpv6 icmpv6_key;
        struct pag_key_icmpv6 icmpv6_mask;
        int n = -1;

        if (mask && pag_scan(s, "icmpv6(type=%"SCNi8"/%"SCNi8","
                             "code=%"SCNi8"/%"SCNi8")%n",
                             &icmpv6_key.icmpv6_type, &icmpv6_mask.icmpv6_type,
                             &icmpv6_key.icmpv6_code, &icmpv6_mask.icmpv6_code,
                             &n)) {
            nl_msg_put_unspec(key, PAG_KEY_ATTR_ICMPV6,
                              &icmpv6_key, sizeof icmpv6_key);
            nl_msg_put_unspec(mask, PAG_KEY_ATTR_ICMPV6, &icmpv6_mask,
                              sizeof icmpv6_mask);
            return n;
        } else if (pag_scan(s, "icmpv6(type=%"SCNi8",code=%"SCNi8")%n",
                            &icmpv6_key.icmpv6_type, &icmpv6_key.icmpv6_code,
                            &n)) {
            nl_msg_put_unspec(key, PAG_KEY_ATTR_ICMPV6,
                              &icmpv6_key, sizeof icmpv6_key);
            ...
            if (mask) {
                ...
                nl_msg_put_unspec(mask, PAG_KEY_ATTR_ICMPV6, &icmpv6_key,
                              sizeof icmpv6_key);
            }
            return n;
        }
    ...
    {
        struct pag_key_arp arp_key;
        struct pag_key_arp arp_mask;
        uint16_t arp_op, arp_op_mask;
        int n = -1;

        if (mask && pag_scan(s, "arp(sip="IP_SCAN_FMT"/"IP_SCAN_FMT","
            ...
            nl_msg_put_unspec(key, PAG_KEY_ATTR_ARP, &arp_key, sizeof arp_key);
            arp_mask.arp_op = htons(arp_op_mask);
            nl_msg_put_unspec(mask, PAG_KEY_ATTR_ARP,
                              &arp_mask, sizeof arp_mask);
            return n;
        } else if (pag_scan(s, "arp(sip="IP_SCAN_FMT",tip="IP_SCAN_FMT","
...

    {
        ...
        if (mask && pag_scan(s, "nd(target="IPV6_SCAN_FMT"/"IPV6_SCAN_FMT")%n",
                ...
                if (mask) {
                   ...
                   pag_scan(s, "nd(target="IPV6_SCAN_FMT"/"IPV6_SCAN_FMT
        ...
        } else if (pag_scan(s, "nd(target="IPV6_SCAN_FMT","
    ...
    if (!strncmp(s, "encap(", 6)) {
        ...
        encap = nl_msg_start_nested(key, PAG_KEY_ATTR_ENCAP);
        if (mask) {
            encap_mask = nl_msg_start_nested(mask, PAG_KEY_ATTR_ENCAP);
        }
...
static uint8_t
pag_to_odp_frag(uint8_t nw_frag)
{
    return (nw_frag == 0 ? PAG_FRAG_TYPE_NONE
          : nw_frag == FLOW_NW_FRAG_ANY ? PAG_FRAG_TYPE_FIRST
          : PAG_FRAG_TYPE_LATER);
}

static uint8_t
pag_to_odp_frag_mask(uint8_t nw_frag_mask)
{
    uint8_t frag_mask = ~(PAG_FRAG_TYPE_FIRST | PAG_FRAG_TYPE_LATER);

    frag_mask |= (nw_frag_mask & FLOW_NW_FRAG_ANY) ? PAG_FRAG_TYPE_FIRST : 0;
    frag_mask |= (nw_frag_mask & FLOW_NW_FRAG_LATER) ? PAG_FRAG_TYPE_LATER : 0;
                         ...
                         size_t max_mpls_depth)
{
    ...
    nl_msg_put_u32(buf, PAG_KEY_ATTR_PRIORITY, data->skb_priority);

    if (flow->tunnel.ip_dst || is_mask) {
        tun_key_to_attr(buf, &data->tunnel);
    }

    nl_msg_put_u32(buf, PAG_KEY_ATTR_SKB_MARK, data->pkt_mark);

    /* Add an ingress port attribute if this is a mask or 'odp_in_port'
     * is not the magical value "ODPP_NONE". */
    if (is_mask || odp_in_port != ODPP_NONE) {
        nl_msg_put_odp_port(buf, PAG_KEY_ATTR_IN_PORT, odp_in_port);
    }

    eth_key = nl_msg_put_unspec_uninit(buf, PAG_KEY_ATTR_ETHERNET,
                                       sizeof *eth_key);
    ...
    if (flow->vlan_tci != htons(0) || flow->dl_type == htons(ETH_TYPE_VLAN)) {
        if (is_mask) {
            nl_msg_put_be16(buf, PAG_KEY_ATTR_ETHERTYPE, PAG_BE16_MAX);
        } else {
            nl_msg_put_be16(buf, PAG_KEY_ATTR_ETHERTYPE, htons(ETH_TYPE_VLAN));
        }
        nl_msg_put_be16(buf, PAG_KEY_ATTR_VLAN, data->vlan_tci);
        encap = nl_msg_start_nested(buf, PAG_KEY_ATTR_ENCAP);
    ...
    if (ntohs(flow->dl_type) < ETH_TYPE_MIN) {
         ...
         * PAG_KEY_ATTR_ETHERTYPE for key and mask:
         *
         *   key      mask    matches
         * -------- --------  -------
    ...
    nl_msg_put_be16(buf, PAG_KEY_ATTR_ETHERTYPE, data->dl_type);

    if (flow->dl_type == htons(ETH_TYPE_IP)) {
        struct pag_key_ipv4 *ipv4_key;

        ipv4_key = nl_msg_put_unspec_uninit(buf, PAG_KEY_ATTR_IPV4,
                                            sizeof *ipv4_key);
        ...
        ipv4_key->ipv4_frag = is_mask ? pag_to_odp_frag_mask(data->nw_frag)
                                      : pag_to_odp_frag(data->nw_frag);
    } else if (flow->dl_type == htons(ETH_TYPE_IPV6)) {
        struct pag_key_ipv6 *ipv6_key;

        ipv6_key = nl_msg_put_unspec_uninit(buf, PAG_KEY_ATTR_IPV6,
                                            sizeof *ipv6_key);
        ...
        ipv6_key->ipv6_frag = is_mask ? pag_to_odp_frag_mask(data->nw_frag)
                                      : pag_to_odp_frag(data->nw_frag);
    } else if (flow->dl_type == htons(ETH_TYPE_ARP) ||
               flow->dl_type == htons(ETH_TYPE_RARP)) {
        struct pag_key_arp *arp_key;

        arp_key = nl_msg_put_unspec_zero(buf, PAG_KEY_ATTR_ARP,
                                         sizeof *arp_key);
        ...
        struct pag_key_mpls *mpls_key;
        int i, n;

        n = flow_count_mpls_labels(flow, NULL);
        n = MIN(n, max_mpls_depth);
        mpls_key = nl_msg_put_unspec_uninit(buf, PAG_KEY_ATTR_MPLS,
                                            n * sizeof *mpls_key);
        for (i = 0; i < n; i++) {
    ...
    if (is_ip_any(flow) && !(flow->nw_frag & FLOW_NW_FRAG_LATER)) {
        if (flow->nw_proto == IPPROTO_TCP) {
            ...
            tcp_key = nl_msg_put_unspec_uninit(buf, PAG_KEY_ATTR_TCP,
                                               sizeof *tcp_key);
            tcp_key->tcp_src = data->tp_src;
            tcp_key->tcp_dst = data->tp_dst;

            if (data->tcp_flags) {
                nl_msg_put_be16(buf, PAG_KEY_ATTR_TCP_FLAGS, data->tcp_flags);
            }
        } else if (flow->nw_proto == IPPROTO_UDP) {
            struct pag_key_udp *udp_key;

            udp_key = nl_msg_put_unspec_uninit(buf, PAG_KEY_ATTR_UDP,
                                               sizeof *udp_key);
            ...
            sctp_key = nl_msg_put_unspec_uninit(buf, PAG_KEY_ATTR_SCTP,
                                               sizeof *sctp_key);
            ...
            icmp_key = nl_msg_put_unspec_uninit(buf, PAG_KEY_ATTR_ICMP,
                                                sizeof *icmp_key);
            ...
            icmpv6_key = nl_msg_put_unspec_uninit(buf, PAG_KEY_ATTR_ICMPV6,
                                                  sizeof *icmpv6_key);
            ...
            if (flow->tp_dst == htons(0) &&
                ...
                nd_key = nl_msg_put_unspec_uninit(buf, PAG_KEY_ATTR_ND,
                                                    sizeof *nd_key);
...
/* Appends a representation of 'flow' as PAG_KEY_ATTR_* attributes to 'buf'.
...
/* Appends a representation of 'mask' as PAG_KEY_ATTR_* attributes to
...
{
    nl_msg_put_u32(buf, PAG_KEY_ATTR_PRIORITY, md->skb_priority);

    if (md->tunnel.ip_dst) {
        tun_key_to_attr(buf, &md->tunnel);
    }

    nl_msg_put_u32(buf, PAG_KEY_ATTR_SKB_MARK, md->pkt_mark);

    /* Add an ingress port attribute if 'odp_in_port' is not the magical
     * value "ODPP_NONE". */
    if (md->in_port.odp_port != ODPP_NONE) {
        nl_msg_put_odp_port(buf, PAG_KEY_ATTR_IN_PORT, md->in_port.odp_port);
    }
}
                        ...
                        struct pkt_metadata *md)
{
    ...
    size_t left;
    uint32_t wanted_attrs = 1u << PAG_KEY_ATTR_PRIORITY |
        1u << PAG_KEY_ATTR_SKB_MARK | 1u << PAG_KEY_ATTR_TUNNEL |
        1u << PAG_KEY_ATTR_IN_PORT;
    ...
    *md = PKT_METADATA_INITIALIZER(ODPP_NONE);
    ...
    NL_ATTR_FOR_EACH (nla, left, key, key_len) {
        ...
        if (type == PAG_KEY_ATTR_PRIORITY) {
            md->skb_priority = nl_attr_get_u32(nla);
            wanted_attrs &= ~(1u << PAG_KEY_ATTR_PRIORITY);
        } else if (type == PAG_KEY_ATTR_SKB_MARK) {
            md->pkt_mark = nl_attr_get_u32(nla);
            wanted_attrs &= ~(1u << PAG_KEY_ATTR_SKB_MARK);
        } else if (type == PAG_KEY_ATTR_TUNNEL) {
            ...
            if (res == ODP_FIT_ERROR) {
                ...
                wanted_attrs &= ~(1u << PAG_KEY_ATTR_TUNNEL);
            }
        } else if (type == PAG_KEY_ATTR_IN_PORT) {
            md->in_port.odp_port = nl_attr_get_odp_port(nla);
            wanted_attrs &= ~(1u << PAG_KEY_ATTR_IN_PORT);
        }
                       ...
                       const struct nlattr *key, size_t key_len)
{
    ...
    for (i = 0; i < 64; i++) {
        if (attrs & (UINT64_C(1) << i)) {
            ...
            ds_put_format(&s, " %s",
                          pag_key_attr_to_string(i, namebuf, sizeof namebuf));
        }
    }
...
static bool
odp_to_pag_frag(uint8_t odp_frag, struct flow *flow)
{
    static struct vlog_rate_limit rl = VLOG_RATE_LIMIT_INIT(1, 5);

    if (odp_frag > PAG_FRAG_TYPE_LATER) {
        VLOG_ERR_RL(&rl, "invalid frag %"PRIu8" in flow key", odp_frag);
        return false;
    }

    if (odp_frag != PAG_FRAG_TYPE_NONE) {
                   ...
                   int *out_of_range_attrp)
{
    ...
    BUILD_ASSERT(PAG_KEY_ATTR_MAX < CHAR_BIT * sizeof present_attrs);
    present_attrs = 0;
    ...
    NL_ATTR_FOR_EACH (nla, left, key, key_len) {
        ...
        if (len != expected_len && expected_len >= 0) {
            ...
            VLOG_ERR_RL(&rl, "attribute %s has length %"PRIuSIZE" but should have "
                        "length %d", pag_key_attr_to_string(type, namebuf,
                                                            sizeof namebuf),
                        len, expected_len);
            return false;
        }
...
static bool
parse_ethertype(const struct nlattr *attrs[PAG_KEY_ATTR_MAX + 1],
                ...
                struct flow *flow, const struct flow *src_flow)
{
    ...
    if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_ETHERTYPE)) {
        flow->dl_type = nl_attr_get_be16(attrs[PAG_KEY_ATTR_ETHERTYPE]);
        ...
        *expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_ETHERTYPE;
    } else {
        if (!is_mask) {
...
static enum odp_key_fitness
parse_l2_5_onward(const struct nlattr *attrs[PAG_KEY_ATTR_MAX + 1],
                  ...
                  const struct flow *src_flow)
{
    ...
    size_t check_len = 0;
    enum pag_key_attr expected_bit = 0xff;

    if (eth_type_mpls(src_flow->dl_type)) {
        size_t size = nl_attr_get_size(attrs[PAG_KEY_ATTR_MPLS]);
        const pag_be32 *mpls_lse = nl_attr_get(attrs[PAG_KEY_ATTR_MPLS]);
        int n = size / sizeof(pag_be32);
        int i;

        if (!size || size % sizeof(pag_be32)) {
            return ODP_FIT_ERROR;
        }

        if (!is_mask) {
            expected_attrs |= (UINT64_C(1) << PAG_KEY_ATTR_MPLS);

            if (!(present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_MPLS))) {
                return ODP_FIT_TOO_LITTLE;
            }
        } else if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_MPLS)) {
        ...
        if (!is_mask) {
            expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_IPV4;
        }
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_IPV4)) {
            const struct pag_key_ipv4 *ipv4_key;

            ipv4_key = nl_attr_get(attrs[PAG_KEY_ATTR_IPV4]);
            ...
            if (is_mask) {
                ...
                expected_bit = PAG_KEY_ATTR_IPV4;
            } else if (!odp_to_pag_frag(ipv4_key->ipv4_frag, flow)) {
                return ODP_FIT_ERROR;
            }
        }
    } else if (src_flow->dl_type == htons(ETH_TYPE_IPV6)) {
        if (!is_mask) {
            expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_IPV6;
        }
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_IPV6)) {
            const struct pag_key_ipv6 *ipv6_key;

            ipv6_key = nl_attr_get(attrs[PAG_KEY_ATTR_IPV6]);
            ...
            if (is_mask) {
                ...
                expected_bit = PAG_KEY_ATTR_IPV6;
            } else if (!odp_to_pag_frag(ipv6_key->ipv6_frag, flow)) {
                return ODP_FIT_ERROR;
            }
        }
        ...
        if (!is_mask) {
            expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_ARP;
        }
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_ARP)) {
            const struct pag_key_arp *arp_key;

            arp_key = nl_attr_get(attrs[PAG_KEY_ATTR_ARP]);
    ...
    expected_bit = PAG_KEY_ATTR_UNSPEC;
    if (src_flow->nw_proto == IPPROTO_TCP
        && (src_flow->dl_type == htons(ETH_TYPE_IP) ||
            src_flow->dl_type == htons(ETH_TYPE_IPV6))
        && !(src_flow->nw_frag & FLOW_NW_FRAG_LATER)) {
        if (!is_mask) {
            expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_TCP;
        }
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_TCP)) {
            const struct pag_key_tcp *tcp_key;

            tcp_key = nl_attr_get(attrs[PAG_KEY_ATTR_TCP]);
            flow->tp_src = tcp_key->tcp_src;
            flow->tp_dst = tcp_key->tcp_dst;
            expected_bit = PAG_KEY_ATTR_TCP;
        }
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_TCP_FLAGS)) {
            expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_TCP_FLAGS;
            flow->tcp_flags = nl_attr_get_be16(attrs[PAG_KEY_ATTR_TCP_FLAGS]);
        }
               ...
               && !(src_flow->nw_frag & FLOW_NW_FRAG_LATER)) {
        if (!is_mask) {
            expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_UDP;
        }
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_UDP)) {
            const struct pag_key_udp *udp_key;

            udp_key = nl_attr_get(attrs[PAG_KEY_ATTR_UDP]);
            flow->tp_src = udp_key->udp_src;
            flow->tp_dst = udp_key->udp_dst;
            expected_bit = PAG_KEY_ATTR_UDP;
        }
               ...
               && !(src_flow->nw_frag & FLOW_NW_FRAG_LATER)) {
        if (!is_mask) {
            expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_SCTP;
        }
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_SCTP)) {
            const struct pag_key_sctp *sctp_key;

            sctp_key = nl_attr_get(attrs[PAG_KEY_ATTR_SCTP]);
            flow->tp_src = sctp_key->sctp_src;
            flow->tp_dst = sctp_key->sctp_dst;
            expected_bit = PAG_KEY_ATTR_SCTP;
        }
    } else if (src_flow->nw_proto == IPPROTO_ICMP
               && src_flow->dl_type == htons(ETH_TYPE_IP)
               && !(src_flow->nw_frag & FLOW_NW_FRAG_LATER)) {
        if (!is_mask) {
            expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_ICMP;
        }
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_ICMP)) {
            const struct pag_key_icmp *icmp_key;

            icmp_key = nl_attr_get(attrs[PAG_KEY_ATTR_ICMP]);
            flow->tp_src = htons(icmp_key->icmp_type);
            flow->tp_dst = htons(icmp_key->icmp_code);
            expected_bit = PAG_KEY_ATTR_ICMP;
        }
    } else if (src_flow->nw_proto == IPPROTO_ICMPV6
               && src_flow->dl_type == htons(ETH_TYPE_IPV6)
               && !(src_flow->nw_frag & FLOW_NW_FRAG_LATER)) {
        if (!is_mask) {
            expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_ICMPV6;
        }
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_ICMPV6)) {
            const struct pag_key_icmpv6 *icmpv6_key;

            icmpv6_key = nl_attr_get(attrs[PAG_KEY_ATTR_ICMPV6]);
            flow->tp_src = htons(icmpv6_key->icmpv6_type);
            flow->tp_dst = htons(icmpv6_key->icmpv6_code);
            expected_bit = PAG_KEY_ATTR_ICMPV6;
            if (src_flow->tp_dst == htons(0) &&
                (src_flow->tp_src == htons(ND_NEIGHBOR_SOLICIT) ||
                 src_flow->tp_src == htons(ND_NEIGHBOR_ADVERT))) {
                if (!is_mask) {
                    expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_ND;
                }
                if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_ND)) {
                    const struct pag_key_nd *nd_key;

                    nd_key = nl_attr_get(attrs[PAG_KEY_ATTR_ND]);
    ...
    if (is_mask && expected_bit != PAG_KEY_ATTR_UNSPEC) {
...
static enum odp_key_fitness
parse_8021q_onward(const struct nlattr *attrs[PAG_KEY_ATTR_MAX + 1],
                   uint64_t present_attrs, int out_of_range_attr,
                   uint64_t expected_attrs, struct flow *flow,
                   const struct nlattr *key, size_t key_len,
                   const struct flow *src_flow)
{
    ...
    const struct nlattr *encap
        = (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_ENCAP)
           ? attrs[PAG_KEY_ATTR_ENCAP] : NULL);
    ...
    if (!is_mask) {
        expected_attrs |= ((UINT64_C(1) << PAG_KEY_ATTR_VLAN) |
                          (UINT64_C(1) << PAG_KEY_ATTR_ENCAP));
    } else {
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_VLAN)) {
            expected_attrs |= (UINT64_C(1) << PAG_KEY_ATTR_VLAN);
        }
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_ENCAP)) {
            expected_attrs |= (UINT64_C(1) << PAG_KEY_ATTR_ENCAP);
        }
    }
    ...
    if (!is_mask && !(present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_VLAN))) {
        return ODP_FIT_TOO_LITTLE;
    } else {
        tci = (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_VLAN)
               ? nl_attr_get_be16(attrs[PAG_KEY_ATTR_VLAN])
               : htons(0));
        if (!is_mask) {
            if (tci == htons(0)) {
                ...
                VLOG_ERR_RL(&rl, "PAG_KEY_ATTR_VLAN 0x%04"PRIx16" is nonzero "
    ...
    if (is_mask && !(present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_ENCAP))) {
        return fitness;
    }
...
odp_flow_key_to_flow__(const struct nlattr *key, size_t key_len,
                       struct flow *flow, const struct flow *src_flow)
{
    const struct nlattr *attrs[PAG_KEY_ATTR_MAX + 1];
    uint64_t expected_attrs;
    uint64_t present_attrs;
    ...
    /* Metadata. */
    if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_PRIORITY)) {
        flow->skb_priority = nl_attr_get_u32(attrs[PAG_KEY_ATTR_PRIORITY]);
        expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_PRIORITY;
    }

    if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_SKB_MARK)) {
        flow->pkt_mark = nl_attr_get_u32(attrs[PAG_KEY_ATTR_SKB_MARK]);
        expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_SKB_MARK;
    }

    if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_TUNNEL)) {
        enum odp_key_fitness res;

        res = odp_tun_key_from_attr(attrs[PAG_KEY_ATTR_TUNNEL], &flow->tunnel);
        if (res == ODP_FIT_ERROR) {
            return ODP_FIT_ERROR;
        } else if (res == ODP_FIT_PERFECT) {
            expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_TUNNEL;
        }
    }

    if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_IN_PORT)) {
        flow->in_port.odp_port
            = nl_attr_get_odp_port(attrs[PAG_KEY_ATTR_IN_PORT]);
        expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_IN_PORT;
    } else if (!is_mask) {
        flow->in_port.odp_port = ODPP_NONE;
    }

    /* Ethernet header. */
    if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_ETHERNET)) {
        const struct pag_key_ethernet *eth_key;

        eth_key = nl_attr_get(attrs[PAG_KEY_ATTR_ETHERNET]);
        memcpy(flow->dl_src, eth_key->eth_src, ETH_ADDR_LEN);
        memcpy(flow->dl_dst, eth_key->eth_dst, ETH_ADDR_LEN);
        if (is_mask) {
            expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_ETHERNET;
        }
    }
    if (!is_mask) {
        expected_attrs |= UINT64_C(1) << PAG_KEY_ATTR_ETHERNET;
    ...
    if (is_mask) {
        ...
        if (present_attrs & (UINT64_C(1) << PAG_KEY_ATTR_VLAN)) {
            flow->vlan_tci = nl_attr_get_be16(attrs[PAG_KEY_ATTR_VLAN]);
            expected_attrs |= (UINT64_C(1) << PAG_KEY_ATTR_VLAN);
        }
    }
...
/* Converts the 'key_len' bytes of PAG_KEY_ATTR_* attributes in 'key' to a flow
 ...
 * the currently understood PAG_KEY_ATTR_* attributes require it.  Currently,
 ...
 * protocol in PAG_KEY_ATTR_IPV4 or PAG_KEY_ATTR_IPV6 is IPPROTO_TCP then we
 * know that a PAG_KEY_ATTR_TCP attribute must appear and that otherwise it
 * must be absent. */
enum odp_key_fitness
...
/* Converts the 'key_len' bytes of PAG_KEY_ATTR_* attributes in 'key' to a mask
...
/* Appends an PAG_ACTION_ATTR_USERSPACE action to 'odp_actions' that specifies
                         ...
                         struct ofpbuf *odp_actions)
{
    ...
    offset = nl_msg_start_nested(odp_actions, PAG_ACTION_ATTR_USERSPACE);
    nl_msg_put_u32(odp_actions, PAG_USERSPACE_ATTR_PID, pid);
    if (userdata) {
        userdata_ofs = odp_actions->size + NLA_HDRLEN;

        /* The PAG kernel module before PAG 1.11 and the upstream Linux kernel
        ...
        memcpy(nl_msg_put_unspec_zero(odp_actions, PAG_USERSPACE_ATTR_USERDATA,
                                      MAX(8, userdata_size)),
...
odp_put_tunnel_action(const struct flow_tnl *tunnel,
                      struct ofpbuf *odp_actions)
{
    size_t offset = nl_msg_start_nested(odp_actions, PAG_ACTION_ATTR_SET);
...
static void
commit_set_action(struct ofpbuf *odp_actions, enum pag_key_attr key_type,
...
odp_put_pkt_mark_action(const uint32_t pkt_mark,
                        struct ofpbuf *odp_actions)
{
    commit_set_action(odp_actions, PAG_KEY_ATTR_SKB_MARK, &pkt_mark,
                      sizeof(pkt_mark));
}
                             ...
                             struct flow_wildcards *wc)
{
    ...
    commit_set_action(odp_actions, PAG_KEY_ATTR_ETHERNET,
                      &eth_key, sizeof(eth_key));
}
         ...
         struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
    ...
    if (base->vlan_tci & htons(VLAN_CFI)) {
        nl_msg_put_flag(odp_actions, PAG_ACTION_ATTR_POP_VLAN);
        base->vlan_tci = 0;
    }
}

static void
commit_vlan_action(pag_be16 vlan_tci, struct flow *base,
                   struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
    ...
    if (vlan_tci & htons(VLAN_CFI)) {
        struct pag_action_push_vlan vlan;

        vlan.vlan_tpid = htons(ETH_TYPE_VLAN);
        vlan.vlan_tci = vlan_tci;
        nl_msg_put_unspec(odp_actions, PAG_ACTION_ATTR_PUSH_VLAN,
                          &vlan, sizeof vlan);
    }
                   ...
                   struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
    ...
    while (base_n > common_n) {
        if (base_n - 1 == common_n && flow_n > common_n) {
            ...
            commit_set_action(odp_actions, PAG_KEY_ATTR_MPLS,
            ...
            nl_msg_put_be16(odp_actions, PAG_ACTION_ATTR_POP_MPLS, dl_type);
    ...
    while (base_n < flow_n) {
        struct pag_action_push_mpls *mpls;

        mpls = nl_msg_put_unspec_zero(odp_actions,
                                      PAG_ACTION_ATTR_PUSH_MPLS,
                                      sizeof *mpls);
                     ...
                     struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
    ...
    ipv4_key.ipv4_frag = pag_to_odp_frag(base->nw_frag);

    commit_set_action(odp_actions, PAG_KEY_ATTR_IPV4,
                      &ipv4_key, sizeof(ipv4_key));
}
                       ...
                       struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
    ...
    ipv6_key.ipv6_frag = pag_to_odp_frag(base->nw_frag);

    commit_set_action(odp_actions, PAG_KEY_ATTR_IPV6,
                      &ipv6_key, sizeof(ipv6_key));
}
                      ...
                      struct ofpbuf *odp_actions, struct flow_wildcards *wc)
{
    ...
    commit_set_action(odp_actions, PAG_KEY_ATTR_ARP, &arp_key, sizeof arp_key);
                           ...
                           struct flow_wildcards *wc)
{
    ...
    commit_set_action(odp_actions, PAG_KEY_ATTR_PRIORITY,
