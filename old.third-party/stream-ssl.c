 ...
 * OpenSSL is /MD - "Multithreaded DLL". If we compile Policy Agent with
 ...
 *
 * XXX: The behavior of the following #include when Policy Agent is
...
static int interpret_ssl_error(const char *function, int ret, int error,
                               int *want);
static DH *tmp_dh_callback(SSL *ssl, int is_export PAG_UNUSED, int keylength);
...
do_ca_cert_bootstrap(struct stream *stream)
{
    ...
    if (fd < 0) {
        if (errno == EEXIST) {
            ...
            VLOG_ERR("could not bootstrap CA cert: creating %s failed: %s",
                     ca_cert.file_name, pag_strerror(errno));
            return errno;
        }
    }
    ...
    if (fclose(file)) {
        ...
        VLOG_ERR("could not bootstrap CA cert: writing %s failed: %s",
                 ca_cert.file_name, pag_strerror(error));
        unlink(ca_cert.file_name);
        return error;
    }
...
static int
pssl_open(const char *name PAG_UNUSED, char *suffix, struct pstream **pstreamp,
          uint8_t dscp)
{
...
static DH *
tmp_dh_callback(SSL *ssl PAG_UNUSED, int is_export PAG_UNUSED, int keylength)
{
    struct dh {
        int keylength;
        DH *dh;
...
static void
ssl_protocol_cb(int write_p, int version PAG_UNUSED, int content_type,
                const void *buf_, size_t len, SSL *ssl PAG_UNUSED, void *sslv_)
{
...
static void
clear_handle(int fd PAG_UNUSED, HANDLE wevent PAG_UNUSED)
{
#ifdef _WIN32
    if (fd) {
