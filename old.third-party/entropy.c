...
get_entropy(void *buffer, size_t n)
{
    ...
    if (fd < 0) {
        VLOG_ERR("%s: open failed (%s)", urandom, pag_strerror(errno));
    ...
    if (error) {
        VLOG_ERR("%s: read error (%s)", urandom, pag_retval_to_string(error));
    }
#else
    int error = 0;
    ...
    if (!CryptGenRandom(crypt_prov, n, buffer)) {
        VLOG_ERR("CryptGenRandom: read error (%s)", pag_lasterror_to_string());
        error = EINVAL;
    }
...
get_entropy_or_die(void *buffer, size_t n)
{
    ...
    if (error) {
        VLOG_FATAL("%s: read error (%s)",
                   urandom, pag_retval_to_string(error));
    }
}
