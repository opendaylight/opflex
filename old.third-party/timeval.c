...
struct clock {
    ...
    struct timespec warp PAG_GUARDED;  /* Offset added for unit tests. */
    bool stopped PAG_GUARDED;          /* Disable real-time updates if true. */
    struct timespec cache PAG_GUARDED; /* Last time read from kernel. */
};
...
time_timespec__(struct clock *c, struct timespec *ts)
{
    ...
    if (!slow_path) {
        ...
        pag_mutex_lock(&c->mutex);
        stopped = c->stopped;
        warp = c->warp;
        cache = c->cache;
        pag_mutex_unlock(&c->mutex);
...
int
time_poll(struct pollfd *pollfds, int n_pollfds, HANDLE *handles PAG_UNUSED,
...
xgettimeofday(struct timeval *tv)
{
    if (gettimeofday(tv, NULL) == -1) {
        VLOG_FATAL("gettimeofday failed (%s)", pag_strerror(errno));
    }
}
...
xclock_gettime(clock_t id, struct timespec *ts)
{
    if (clock_gettime(id, ts) == -1) {
        ...
        pag_abort(errno, "xclock_gettime() failed");
    }
}
...
static int
getrusage_thread(struct rusage *rusage PAG_UNUSED)
{
#ifdef RUSAGE_THREAD
...
timeval_stop_cb(struct unixctl_conn *conn,
                 int argc PAG_UNUSED, const char *argv[] PAG_UNUSED,
                 void *aux PAG_UNUSED)
{
    pag_mutex_lock(&monotonic_clock.mutex);
    atomic_store(&monotonic_clock.slow_path, true);
    monotonic_clock.stopped = true;
    xclock_gettime(monotonic_clock.id, &monotonic_clock.cache);
    pag_mutex_unlock(&monotonic_clock.mutex);
...
timeval_warp_cb(struct unixctl_conn *conn,
                int argc PAG_UNUSED, const char *argv[], void *aux PAG_UNUSED)
{
    struct timespec ts;
    int msecs;
