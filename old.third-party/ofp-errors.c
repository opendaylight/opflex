                    ...
                    pag_be32 xid, const void *data, size_t data_len)
{
    ...
    if (!triplet->vendor) {
        ...
        pag_be32 vendor = htonl(triplet->vendor);
...
ofperr_decode_msg(const struct ofp_header *oh, struct ofpbuf *payload)
{
    ...
    if (type == NXET_VENDOR && code == NXVC_VENDOR_ERROR) {
        ...
        const pag_be32 *vendorp = ofpbuf_try_pull(&b, sizeof *vendorp);
        if (!vendorp) {
            return 0;
        }
 ...
 * 'error' is a positive errno value and returns what pag_strerror() produces
 * for 'error'.  */
const char *
