...
{
    pag_assert(json->type == JSON_STRING);
...
{
    pag_assert(json->type == JSON_ARRAY);
...
{
    pag_assert(json->type == JSON_OBJECT);
...
{
    pag_assert(json->type == JSON_TRUE || json->type == JSON_FALSE);
...
{
    pag_assert(json->type == JSON_REAL || json->type == JSON_INTEGER);
...
{
    pag_assert(json->type == JSON_INTEGER);
...
json_from_stream(FILE *stream)
{
    ...
    if (ferror(stream)) {
        ...
        json = json_string_create_nocopy(
            xasprintf("error reading JSON stream: %s", pag_strerror(errno)));
    }
...
json_parser_finish(struct json_parser *p)
{
    ...
    if (!p->error) {
        pag_assert(p->height == 1);
        pag_assert(p->stack[0].json != NULL);
