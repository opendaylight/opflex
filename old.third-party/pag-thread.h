...
#ifndef PAG_THREAD_H
#define PAG_THREAD_H 1

#include <pthread.h>
#include <stddef.h>
#include <sys/types.h>
#include "pag-atomic.h"
#include "util.h"


/* Mutex. */
struct PAG_LOCKABLE pag_mutex {
    pthread_mutex_t lock;
    const char *where;
};

/* "struct pag_mutex" initializer. */
#ifdef PTHREAD_ERRORCHECK_MUTEX_INITIALIZER_NP
#define PAG_MUTEX_INITIALIZER { PTHREAD_ERRORCHECK_MUTEX_INITIALIZER_NP, NULL }
#else
#define PAG_MUTEX_INITIALIZER { PTHREAD_MUTEX_INITIALIZER, NULL }
#endif

#ifdef PTHREAD_ADAPTIVE_MUTEX_INITIALIZER_NP
#define PAG_ADAPTIVE_MUTEX_INITIALIZER                  \
    { PTHREAD_ADAPTIVE_MUTEX_INITIALIZER_NP, NULL }
#else
#define PAG_ADAPTIVE_MUTEX_INITIALIZER PAG_MUTEX_INITIALIZER
#endif

/* pag_mutex functions analogous to pthread_mutex_*() functions.
 *
 * Most of these functions abort the process with an error message on any
 * error.  pag_mutex_trylock() is an exception: it passes through a 0 or EBUSY
 * return value to the caller and aborts on any other error. */
void pag_mutex_init(const struct pag_mutex *);
void pag_mutex_init_recursive(const struct pag_mutex *);
void pag_mutex_init_adaptive(const struct pag_mutex *);
void pag_mutex_destroy(const struct pag_mutex *);
void pag_mutex_unlock(const struct pag_mutex *mutex) PAG_RELEASES(mutex);
void pag_mutex_lock_at(const struct pag_mutex *mutex, const char *where)
    PAG_ACQUIRES(mutex);
#define pag_mutex_lock(mutex) \
        pag_mutex_lock_at(mutex, SOURCE_LOCATOR)

int pag_mutex_trylock_at(const struct pag_mutex *mutex, const char *where)
    PAG_TRY_LOCK(0, mutex);
#define pag_mutex_trylock(mutex) \
        pag_mutex_trylock_at(mutex, SOURCE_LOCATOR)

void pag_mutex_cond_wait(pthread_cond_t *, const struct pag_mutex *);

/* Wrappers for pthread_mutex_*() that abort the process on any error.
 * This is still needed when pag-atomic-pthreads.h is used. */
...
/* Read-write lock.
 *
 * An pag_rwlock does not support recursive readers, because POSIX allows
 ...
 * configuration do not deadlock, but pag_rwlock_init() initializes rwlocks as
 * non-recursive (which will deadlock) for two reasons:
 *
 *     - glibc only provides fairness to writers in this mode.
 *
 *     - It's better to find bugs in the primary Policy Agent target rather
 *       than exposing them only to porters. */
struct PAG_LOCKABLE pag_rwlock {
...
#define PAG_RWLOCK_INITIALIZER \
        { PTHREAD_RWLOCK_WRITER_NONRECURSIVE_INITIALIZER_NP, NULL }
#else
#define PAG_RWLOCK_INITIALIZER { PTHREAD_RWLOCK_INITIALIZER, NULL }
#endif

/* pag_rwlock functions analogous to pthread_rwlock_*() functions.
 *
 * Most of these functions abort the process with an error message on any
...
void pag_rwlock_init(const struct pag_rwlock *);
void pag_rwlock_destroy(const struct pag_rwlock *);
void pag_rwlock_unlock(const struct pag_rwlock *rwlock) PAG_RELEASES(rwlock);
...
void pag_rwlock_wrlock_at(const struct pag_rwlock *rwlock, const char *where)
    PAG_ACQ_WRLOCK(rwlock);
#define pag_rwlock_wrlock(rwlock) \
        pag_rwlock_wrlock_at(rwlock, SOURCE_LOCATOR)

int pag_rwlock_trywrlock_at(const struct pag_rwlock *rwlock, const char *where)
    PAG_TRY_WRLOCK(0, rwlock);
#define pag_rwlock_trywrlock(rwlock) \
    pag_rwlock_trywrlock_at(rwlock, SOURCE_LOCATOR)

void pag_rwlock_rdlock_at(const struct pag_rwlock *rwlock, const char *where)
    PAG_ACQ_RDLOCK(rwlock);
#define pag_rwlock_rdlock(rwlock) \
        pag_rwlock_rdlock_at(rwlock, SOURCE_LOCATOR)

int pag_rwlock_tryrdlock_at(const struct pag_rwlock *rwlock, const char *where)
    PAG_TRY_RDLOCK(0, rwlock);
#define pag_rwlock_tryrdlock(rwlock) \
        pag_rwlock_tryrdlock_at(rwlock, SOURCE_LOCATOR)

/* Wrappers for xpthread_cond_*() that abort the process on any error.
 *
 * Use pag_mutex_cond_wait() to wait for a condition. */
 ...
 * ==========
 *
 * PAG provides some extensions and wrappers:
 *
 ...
 *
 *     - struct pag_tsd provides an alternative to pthread_key_t that isn't
 ...
 * by pagthread_) but it a much larger limit that can be raised if necessary
 * (by recompiling).  Thus, one may more freely use this form of
 * thread-specific data.
 *
 * pagthread_key_t also differs from pthread_key_t in the following ways:
 *
 *    - Destructors must not access thread-specific data (via pagthread_key).
 *
 ...
 *      pagthread_key_t API guarantees that, when pagthread_key_delete()
 *      returns, all destructors have returned and no new ones will start
 *      execution.
 */
typedef struct pagthread_key *pagthread_key_t;

void pagthread_key_create(pagthread_key_t *, void (*destructor)(void *));
void pagthread_key_delete(pagthread_key_t);

void pagthread_setspecific(pagthread_key_t, const void *);
void *pagthread_getspecific(pagthread_key_t);

 ...
 * ========
 *
 * Use pagthread_once, like this, instead:
 *
...
struct pagthread_once {
    atomic_bool done;
    struct pag_mutex mutex;
};

#define PAGTHREAD_ONCE_INITIALIZER              \
    {                                           \
        ATOMIC_VAR_INIT(false),                 \
        PAG_MUTEX_INITIALIZER,                  \
    }

static inline bool pagthread_once_start(struct pagthread_once *once)
    PAG_TRY_LOCK(true, once->mutex);
void pagthread_once_done(struct pagthread_once *once)
    PAG_RELEASES(once->mutex);

bool pagthread_once_start__(struct pagthread_once *once)
    PAG_TRY_LOCK(false, once->mutex);

static inline bool
pagthread_once_is_done__(struct pagthread_once *once)
{
    bool done;
...
/* Returns true if this is the first call to pagthread_once_start() for
 * 'once'.  In this case, the caller should perform whatever initialization
 * actions it needs to do, then call pagthread_once_done() for 'once'.
 *
 * Returns false if this is not the first call to pagthread_once_start() for
 * 'once'.  In this case, the call will not return until after
 * pagthread_once_done() has been called. */
static inline bool
pagthread_once_start(struct pagthread_once *once)
{
    return PAG_UNLIKELY(!pagthread_once_is_done__(once)
                        && !pagthread_once_start__(once));
}

/* Thread ID.
 *
...
DECLARE_EXTERN_PER_THREAD_DATA(unsigned int, pagthread_id);

/* Returns a per-thread identifier unique within the lifetime of the
 * process. */
static inline unsigned int
pagthread_id_self(void)
{
    return *pagthread_id_get();
}

...
struct pagthread_counter *pagthread_counter_create(void);
void pagthread_counter_destroy(struct pagthread_counter *);
void pagthread_counter_inc(struct pagthread_counter *, unsigned long long int);
unsigned long long int pagthread_counter_read(
    const struct pagthread_counter *);

