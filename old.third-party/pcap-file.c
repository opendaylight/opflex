...
FILE *
pag_pcap_open(const char *file_name, const char *mode)
{
    struct stat s;
    FILE *file;
    int error;

    pag_assert(!strcmp(mode, "rb") ||
               !strcmp(mode, "wb") ||
               !strcmp(mode, "ab"));
    ...
    switch (mode[0]) {
    case 'r':
        error = pag_pcap_read_header(file);
        if (error) {
            errno = error;
            fclose(file);
            return NULL;
...
int
pag_pcap_read_header(FILE *file)
{
    struct pcap_hdr ph;
    if (fread(&ph, sizeof ph, 1, file) != 1) {
        int error = ferror(file) ? errno : EOF;
        VLOG_WARN("failed to read pcap header: %s", pag_retval_to_string(error));
        return error;
    }
...
void
pag_pcap_write_header(FILE *file)
{
...
int
pag_pcap_read(FILE *file, struct ofpbuf **bufp, long long int *when)
{
...
void
pag_pcap_write(FILE *file, struct ofpbuf *buf)
{
    struct pcaprec_hdr prh;
    struct timeval tv;
...

struct tcp_key {
    pag_be32 nw_src, nw_dst;
    pag_be16 tp_src, tp_dst;
};
