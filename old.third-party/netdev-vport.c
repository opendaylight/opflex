...
{
    pag_assert(is_vport_class(class));
...
{
    pag_assert(is_vport_class(netdev_get_class(netdev)));
                           ...
                           char namebuf[], size_t bufsize)
{
    if (netdev_vport_needs_dst_port(netdev)) {
        ...
        pag_assert(strlen(type) + 10 < IFNAMSIZ);
...
static int
netdev_vport_update_flags(struct netdev *netdev PAG_UNUSED,
                          enum netdev_flags off,
                          enum netdev_flags on PAG_UNUSED,
...
set_tunnel_config(struct netdev *dev_, const struct smap *args)
{
    ...
    if (tnl_cfg.ipsec) {
        static struct pag_mutex mutex = PAG_MUTEX_INITIALIZER;
        static pid_t pid = 0;

        pag_mutex_lock(&mutex);
        if (pid <= 0) {
            char *file_name = xasprintf("%s/%s", pag_rundir(),
                                        "pag-monitor-ipsec.pid");
            pid = read_pidfile(file_name);
            free(file_name);
        }
        pag_mutex_unlock(&mutex);

        if (pid < 0) {
            VLOG_ERR("%s: IPsec requires the pag-monitor-ipsec daemon",
                     name);
            return EINVAL;
        }
