...
/* NOTE: 19-MAY-2014 - dkehn@noironetworks.com: all malloc, calloc,
 * realloc,etc. routines have been modified to use Googles 
 * tcmalloc (gperftools) instead, for performance reasons.
 */
...
/* Buffer used by pag_strerror() and pag_format_message(). */
...
void
pag_assert_failure(const char *where, const char *function,
...
out_of_memory(void)
{
    pag_abort(0, "virtual memory exhausted");
}
...
void
pag_strlcpy(char *dst, const char *src, size_t size)
{
    if (size > 0) {
 ...
 *     pag_strzcpy(dst, src, size);
 * is equivalent to these two calls:
 *     memset(dst, '\0', size);
 *     pag_strlcpy(dst, src, size);
 *
 * (Thus, pag_strzcpy() is similar to strncpy() without some of the pitfalls.)
 */
void
pag_strzcpy(char *dst, const char *src, size_t size)
{
    if (size > 0) {
        size_t len = strnlen(src, size - 1);
        memcpy(dst, src, len);
 ...
 * nonzero, then it is formatted with pag_retval_to_string() and appended to
 * the message inside parentheses.  Then, terminates with abort().
 *
 * This function is preferred to pag_fatal() in a situation where it would make
 ...
 * itself. */
void
pag_abort(int err_no, const char *format, ...)
{
    va_list args;

    va_start(args, format);
    pag_abort_valist(err_no, format, args);
}

/* Same as pag_abort() except that the arguments are supplied as a va_list. */
void
pag_abort_valist(int err_no, const char *format, va_list args)
{
    pag_error_valist(err_no, format, args);
    abort();
}
 ...
 * itself. */
void
pag_fatal(int err_no, const char *format, ...)
{
    va_list args;

    va_start(args, format);
    pag_fatal_valist(err_no, format, args);
}

/* Same as pag_fatal() except that the arguments are supplied as a va_list. */
void
pag_fatal_valist(int err_no, const char *format, va_list args)
{
    pag_error_valist(err_no, format, args);
    exit(EXIT_FAILURE);
}
 ...
 * itself. */
void
pag_error(int err_no, const char *format, ...)
{
    va_list args;
...
/* Same as pag_error() except that the arguments are supplied as a va_list. */
void
pag_error_valist(int err_no, const char *format, va_list args)
{
    ...
    if (err_no != 0) {
        fprintf(stderr, " (%s)", pag_retval_to_string(err_no));
    }
    putc('\n', stderr);
...
/* Many PAG functions return an int which is one of:
 * - 0: no error yet
 * - >0: errno value
 ...
 */
const char *
pag_retval_to_string(int retval)
{
    return (!retval ? ""
 ...
 * instead.  For WINAPI calls, look at pag_lasterror_to_string(). */
const char *
pag_strerror(int error)
{
 ...
 * is the same as the VERSION #define, the caller is assumed to be part of 
 * Policy Agent.  Otherwise, it is assumed to be an external program linking 
 * against the Policy Agent libraries.
 *
                   ...
                   const char *time)
{
    ...
    if (!strcmp(version, VERSION)) {
        program_version = xasprintf("%s (Policy Agent) "VERSION"\n"
                                    "Compiled %s %s\n",
                                    program_name, date, time);
    } else {
        program_version = xasprintf("%s %s\n"
                                    "Policy Agent Library "VERSION"\n"
                                    "Compiled %s %s\n",
...
void
pag_print_version(uint8_t min_ofp, uint8_t max_ofp)
{
...
void
pag_hex_dump(FILE *stream, const void *buf_, size_t size,
...
get_cwd(void)
{
    ...
    for (;;) {
        ...
        if (getcwd(buf, size)) {
            ...
            free(buf);
            if (error != ERANGE) {
                VLOG_WARN("getcwd failed (%s)", pag_strerror(error));
                return NULL;
            }
            size *= 2;
        }
 ...
 */
bool
pag_scan(const char *s, const char *format, ...)
{
...
/*
 * @brief directory_exist() 
 *  check if a directory exists
 *
 * @param0 char * directory path
 *
 * @return True if flie exists, else False if not
 *
 */
...
/*
 * @brief Realloc() - my realloc routine, can deal witha null string
 *           expand a pointer to be a particular size
 *
 * @param0 void * what is to be re-sized
 * @param1 int how much is needed
 *
 * @return void * to the newly allocated memory
 *
 */
...
char *
pag_lasterror_to_string(void)
{
    return pag_format_message(GetLastError());
}
#endif
