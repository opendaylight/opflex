...
/* List of PAG abstracted actions.
 *
...
enum PAG_PACKED_ENUM ofpact_type {
...
#ifdef __GNUC__
/* Make sure that PAG_PACKED_ENUM really worked. */
...
#define DEFINE_OFPACT(ENUM, STRUCT, MEMBER)                             \
    ...
    {                                                                   \
        pag_assert(ofpact->type == OFPACT_##ENUM);                      \
...
#define DEFINE_INST(ENUM, STRUCT, EXTENSIBLE, NAME) PAGINST_##ENUM,
    PAG_INSTRUCTIONS
#undef DEFINE_INST
};
...
#define DEFINE_INST(ENUM, STRUCT, EXTENSIBLE, NAME) + 1
    N_PAG_INSTRUCTIONS = PAG_INSTRUCTIONS
#undef DEFINE_INST
};

const char *pag_instruction_name_from_type(enum pag_instruction_type type);
int pag_instruction_type_from_name(const char *name);
enum pag_instruction_type pag_instruction_type_from_ofpact_type(
    enum ofpact_type);
#endif /* ofp-actions.h */
