                      ...
                      int family, uint32_t flags, uint8_t cmd, uint8_t version)
{
    ...
    pag_assert(msg->size == NLMSG_HDRLEN);
...
nl_msg_put_unspec_uninit(struct ofpbuf *msg, uint16_t type, size_t size)
{
    ...
    pag_assert(NLA_ALIGN(total_size) <= UINT16_MAX);
 ...
 * byte order 'value' to 'msg'. */
void
nl_msg_put_be16(struct ofpbuf *msg, uint16_t type, pag_be16 value)
{
    nl_msg_put_unspec(msg, type, &value, sizeof value);
}
 ...
 * byte order 'value' to 'msg'. */
void
nl_msg_put_be32(struct ofpbuf *msg, uint16_t type, pag_be32 value)
{
    nl_msg_put_unspec(msg, type, &value, sizeof value);
}
 ...
 * byte order 'value' to 'msg'. */
void
nl_msg_put_be64(struct ofpbuf *msg, uint16_t type, pag_be64 value)
{
    nl_msg_put_unspec(msg, type, &value, sizeof value);
}
...
nl_msg_push_unspec_uninit(struct ofpbuf *msg, uint16_t type, size_t size)
{
    ...
    pag_assert(!nl_attr_oversized(size));
    nla->nla_len = total_size;
    nla->nla_type = type;
    return nla + 1;
}
...
void
nl_msg_push_be16(struct ofpbuf *msg, uint16_t type, pag_be16 value)
{
    nl_msg_push_unspec(msg, type, &value, sizeof value);
}
 ...
 * network byte order 'value' to 'msg'. */
void
nl_msg_push_be32(struct ofpbuf *msg, uint16_t type, pag_be32 value)
{
    nl_msg_push_unspec(msg, type, &value, sizeof value);
}
 ...
 * network byte order 'value' to 'msg'. */
void
nl_msg_push_be64(struct ofpbuf *msg, uint16_t type, pag_be64 value)
{
    nl_msg_push_unspec(msg, type, &value, sizeof value);
}
...
{
    pag_assert(nla->nla_len >= NLA_HDRLEN);
    return nla + 1;
}
...
{
    pag_assert(nla->nla_len >= NLA_HDRLEN + size);
    return nla + 1;
}
...
{
    const pag_32aligned_u64 *x = nl_attr_get_unspec(nla, sizeof *x);
...
{
    return NL_ATTR_GET_AS(nla, pag_be16);
}
...
{
    return NL_ATTR_GET_AS(nla, pag_be32);
}
...
{
    const pag_32aligned_be64 *x = nl_attr_get_unspec(nla, sizeof *x);
...
{
    pag_assert(nla->nla_len > NLA_HDRLEN);
    pag_assert(memchr(nl_attr_get(nla), '\0', nla->nla_len - NLA_HDRLEN));
...
min_attr_len(enum nl_attr_type type)
{
    ...
    case N_NL_ATTR_TYPES: default: PAG_NOT_REACHED();
    }
}
