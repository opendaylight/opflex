 ...
 * pag-vswitchd.  This is useful for the Policy Agent software and,
 ...
 *       autonomously, relying less on pag-vswitchd to process flow setups,
 ...
 *
 * Prefix tracking is configured via PAGDB "Flow_Table" table,
...
extern struct pag_mutex ofproto_mutex;
struct trie_node;
...
struct classifier {
    ...
    struct fat_rwlock rwlock PAG_ACQ_AFTER(ofproto_mutex);
...
struct cls_partition {
    ...
    pag_be64 metadata;          /* metadata value for this partition. */
...
void cls_cursor_init(struct cls_cursor *cursor, const struct classifier *cls,
                     const struct cls_rule *match) PAG_REQ_RDLOCK(cls->rwlock);
