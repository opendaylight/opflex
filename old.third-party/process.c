...
static void sigchld_handler(int signr PAG_UNUSED);
...
process_start(char **argv, struct process **pp)
{
    ...
    *pp = NULL;
    ...
    pid = fork();
    if (pid < 0) {
        VLOG_WARN("fork failed: %s", pag_strerror(errno));
        return errno;
    } else if (pid) {
        ...
        LIST_FOR_EACH (p, node, &all_processes) {
            if (!p->exited) {
                ...
                if (retval == p->pid) {
                    ...
                    VLOG_WARN("waitpid: %s", pag_strerror(errno));
                    p->exited = true;
                    p->status = -1;
                }
            }
...

static void
sigchld_handler(int signr PAG_UNUSED)
{
