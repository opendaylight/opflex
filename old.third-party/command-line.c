...
run_command(int argc, char *argv[], const struct command commands[])
{
    ...
    if (argc < 1) {
        pag_fatal(0, "missing command name; use --help for help");
    }
...
static struct pag_mutex proctitle_mutex = PAG_MUTEX_INITIALIZER;

/* Start of command-line arguments in memory. */
static char *argv_start PAG_GUARDED_BY(proctitle_mutex);

/* Number of bytes of command-line arguments. */
static size_t argv_size PAG_GUARDED_BY(proctitle_mutex);

/* Saved command-line arguments. */
static char *saved_proctitle PAG_GUARDED_BY(proctitle_mutex);
...
proctitle_init(int argc, char **argv)
{
    ...
    pag_mutex_lock(&proctitle_mutex);
    ...
    pag_mutex_unlock(&proctitle_mutex);
}
...
void
proctitle_init(int argc PAG_UNUSED, char **argv PAG_UNUSED)
{
}
...
void
proctitle_set(const char *format PAG_UNUSED, ...)
{
}
#endif
