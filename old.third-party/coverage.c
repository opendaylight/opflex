...
static struct pag_mutex coverage_mutex = PAG_MUTEX_INITIALIZER;
...
static void
coverage_unixctl_show(struct unixctl_conn *conn, int argc PAG_UNUSED,
...
coverage_hash(void)
{
    ...
    pag_mutex_lock(&coverage_mutex);
    for (i = 0; i < n_coverage_counters; i++) {
        c[i] = coverage_counters[i];
    }
    pag_mutex_unlock(&coverage_mutex);
