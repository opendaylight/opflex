...
static void
vlan_cache_cb(const struct rtnetlink_link_change *change PAG_UNUSED,
              void *aux PAG_UNUSED)
{
    cache_valid = false;
}
...
vlandev_linux_refresh(void)
{
    ...
    if (!stream) {
        ...
        VLOG_WARN_RL(&rl, "%s: open failed (%s)", fn, pag_strerror(error));
        return error;
    }
    ...
    while (fgets(line, sizeof line, stream)) {
        ...
        if (pag_scan(line, "%15[^ |] | %d | %15s", vlan_dev, &vid, real_dev)) {
              ...
              int cmd, const char *cmd_name)
{
    ...
    via->cmd = cmd;
    pag_strlcpy(via->device1, netdev_name, sizeof via->device1);

    error = af_inet_ioctl(SIOCSIFVLAN, via);
    if (error) {
        VLOG_WARN_RL(&rl, "%s: VLAN ioctl %s failed (%s)",
                     netdev_name, cmd_name, pag_strerror(error));
    }
    return error;
}
...
static int
vlandev_stub_add(const char *real_dev PAG_UNUSED, int vid PAG_UNUSED)
{
...
static int
vlandev_stub_del(const char *vlan_dev PAG_UNUSED)
{
    VLOG_ERR("not supported on non-Linux platform");
    return EOPNOTSUPP;
}

static const struct vlandev_class PAG_UNUSED vlandev_stub_class = {
    NULL,                       /* vd_refresh */
    vlandev_stub_add,
    vlandev_stub_del
};
