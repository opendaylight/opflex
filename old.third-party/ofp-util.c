...
int
ofputil_netmask_to_wcbits(pag_be32 netmask)
{
...
ofputil_wildcard_from_ofpfw10(uint32_t ofpfw, struct flow_wildcards *wc)
{
    ...
    if (!(ofpfw & OFPFW10_TP_SRC)) {
        wc->masks.tp_src = PAG_BE16_MAX;
    }
    if (!(ofpfw & OFPFW10_TP_DST)) {
        wc->masks.tp_dst = PAG_BE16_MAX;
    }
                               ...
                               struct match *match)
{
    ...
    if (!(ofpfw & OFPFW10_DL_VLAN) &&
         ...
         * However, older versions of PAG treated this as matching packets
         * withut an 802.1Q header, so we do here too. */
        match->flow.vlan_tci = htons(0);
        match->wc.masks.vlan_tci = htons(0xffff);
    } else {
        pag_be16 vid, pcp, tci;
        uint16_t hpcp;
 ...
 * structure. */
pag_be16
ofputil_dl_type_to_openflow(pag_be16 flow_dl_type)
{
 ...
 * flow. */
pag_be16
ofputil_dl_type_from_openflow(pag_be16 ofp_dl_type)
{
...
ofputil_protocols_to_string(enum ofputil_protocol protocols)
{
    ...
    pag_assert(!(protocols & ~OFPUTIL_P_ANY));
    if (protocols == 0) {
        return xstrdup("none");
    }
...
ofputil_protocols_from_string(const char *s)
{
    ...
    while (*s) {
        ...
        if (!p) {
            pag_fatal(0, "%.*s: unknown flow protocol", (int) n, s);
        }
        protocols |= p;
    ...
    if (!protocols) {
        pag_fatal(0, "%s: no flow protocol specified", orig_s);
    }
    return protocols;
}
                            ...
                            uint32_t *allowed_versionsp)
{
    ...
    const pag_be32 *bitmap = ALIGNED_CAST(const pag_be32 *, oheh + 1);
...
ofputil_encode_hello(uint32_t allowed_versions)
{
    ...
    if (should_send_version_bitmap(allowed_versions)) {
        ...
        *ALIGNED_CAST(pag_be32 *, oheh + 1) = htonl(allowed_versions);
...
ofputil_encode_nx_set_flow_format(enum nx_flow_format nxff)
{
    ...
    pag_assert(ofputil_nx_flow_format_is_valid(nxff));
...
static enum ofperr
ofputil_decode_flow_mod_flags(pag_be16 raw_flags_,
                        ...
                        ofp_port_t max_port, uint8_t max_table)
{
    ...
    if (raw == OFPRAW_OFPT11_FLOW_MOD) {
        ...
        fm->modify_cookie = fm->new_cookie != PAG_BE64_MAX;
                        ...
                        enum ofputil_protocol protocol)
{
    ...
    pag_be16 raw_flags = ofputil_encode_flow_mod_flags(fm->flags, version);
    struct ofpbuf *msg;
 ...
 *
 * We use this in situations where PAG internally uses UINT64_MAX to mean
...
netdev_port_features_from_ofp10(pag_be32 ofp10_)
{
...
static enum netdev_features
netdev_port_features_from_ofp11(pag_be32 ofp11)
{
                              ...
                              const struct ofp10_phy_port *opp)
{
    ...
    pag_strlcpy(pp->name, opp->name, OFP_MAX_PORT_NAME_LEN);
                          ...
                          const struct ofp11_port *op)
{
    ...
    pag_strlcpy(pp->name, op->name, OFP_MAX_PORT_NAME_LEN);
                              ...
                              struct ofp10_phy_port *opp)
{
    ...
    pag_strlcpy(opp->name, pp->name, OFP_MAX_PORT_NAME_LEN);
                          ...
                          struct ofp11_port *op)
{
    ...
    pag_strlcpy(op->name, pp->name, OFP_MAX_PORT_NAME_LEN);
...
decode_action_bits(pag_be32 of_actions,
...
ofputil_encode_switch_features(const struct ofputil_switch_features *features,
                               enum ofputil_protocol protocol, pag_be32 xid)
{
                        ...
                        enum ofputil_protocol protocol)
{
    ...
    switch (ofp_version) {
    case OFP10_VERSION: {
        pag_fatal(0, "table mod needs OpenFlow 1.1 or later "
                     "(\'-O OpenFlow11\')");
        break;
    }
    case OFP11_VERSION:
                            ...
                            struct ofputil_role_request *rr)
{
    ...
    if (raw == OFPRAW_OFPT12_ROLE_REQUEST ||
        ...
        if (raw == OFPRAW_OFPT12_ROLE_REQUEST
            ...
            : orr->generation_id == PAG_BE64_MAX) {
                           ...
                           struct ofputil_role_status *rs)
{
    ...
    ofpbuf_use_const(&b, oh, ntohs(oh->length));
    raw = ofpraw_pull_assert(&b);
    pag_assert(raw == OFPRAW_OFPT14_ROLE_STATUS);
                              ...
                              struct ofpbuf *buf)
{
    ...
    pag_strlcpy(out->name, in->name, sizeof out->name);
    out->wildcards = 0;
...
static pag_be32
oxm12_to_ofp11_flow_match_fields(pag_be64 oxm12)
{
    struct map {
...
enum ofperr
ofputil_port_from_ofp11(pag_be32 ofp11_port, ofp_port_t *ofp10_port)
{
...
#define OFPUTIL_NAMED_PORT(NAME)                        \
        case OFPP_##NAME:                               \
            pag_strlcpy(namebuf, #NAME, bufsize);       \
            break;
        OFPUTIL_NAMED_PORTS
#undef OFPUTIL_NAMED_PORT
                        ...
                        char namebuf[MAX_GROUP_NAME_LEN + 1], size_t bufsize)
{
    switch (group_id) {
    case OFPG11_ALL:
        pag_strlcpy(namebuf, "ALL", bufsize);
        break;

    case OFPG11_ANY:
        pag_strlcpy(namebuf, "ANY", bufsize);
        break;
 ...
 *       example, Policy Agent does not understand SCTP, an L4 protocol, so the
                                   ...
                                   uint32_t group_id)
{
    ...
    switch (ofp_version) {
    case OFP10_VERSION:
        pag_fatal(0, "dump-group-stats needs OpenFlow 1.1 or later "
                     "(\'-O OpenFlow11\')");
    case OFP11_VERSION:
    case OFP12_VERSION:
    case OFP13_VERSION: {
...
ofputil_encode_group_desc_request(enum ofp_version ofp_version)
{
    ...
    switch (ofp_version) {
    case OFP10_VERSION:
        pag_fatal(0, "dump-groups needs OpenFlow 1.1 or later "
                     "(\'-O OpenFlow11\')");
    case OFP11_VERSION:
    case OFP12_VERSION:
    case OFP13_VERSION: {
...
ofputil_encode_group_features_request(enum ofp_version ofp_version)
{
    ...
    case OFP10_VERSION:
    case OFP11_VERSION:
        pag_fatal(0, "dump-group-features needs OpenFlow 1.2 or later "
                     "(\'-O OpenFlow12\')");
    case OFP12_VERSION:
    case OFP13_VERSION: {
                         ...
                         const struct ofputil_group_mod *gm)
{
    ...
    case OFP10_VERSION: {
        if (gm->command == OFPGC11_ADD) {
            pag_fatal(0, "add-group needs OpenFlow 1.1 or later "
                         "(\'-O OpenFlow11\')");
        } else if (gm->command == OFPGC11_MODIFY) {
            pag_fatal(0, "mod-group needs OpenFlow 1.1 or later "
                         "(\'-O OpenFlow11\')");
        } else {
            pag_fatal(0, "del-groups needs OpenFlow 1.1 or later "
                         "(\'-O OpenFlow11\')");
        }
    }
                             ...
                             struct ofp13_queue_stats *qs13)
{
    ...
    if (oqs->duration_sec != UINT32_MAX) {
        ...
        qs13->duration_sec = PAG_BE32_MAX;
        qs13->duration_nsec = PAG_BE32_MAX;
    }
}
