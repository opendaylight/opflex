...
/* pag_assert() logs the assertion message, so using pag_assert() in this
 * source file could cause recursion. */
#undef pag_assert
#define pag_assert use_assert_instead_of_pag_assert_in_this_module
 ...
 * memory. */
static struct pag_rwlock pattern_rwlock = PAG_RWLOCK_INITIALIZER;

/* Information about each facility. */
struct facility {
    const char *name;           /* Name. */
    char *pattern PAG_GUARDED_BY(pattern_rwlock); /* Current pattern. */
 ...
 * pattern_rwlock. */
static struct pag_mutex log_file_mutex = PAG_MUTEX_INITIALIZER;
static char *log_file_name PAG_GUARDED_BY(log_file_mutex);
static int log_fd PAG_GUARDED_BY(log_file_mutex) = -1;
static struct async_append *log_writer PAG_GUARDED_BY(log_file_mutex);
static bool log_async PAG_GUARDED_BY(log_file_mutex);

/* Syslog export configuration. */
static int syslog_fd PAG_GUARDED_BY(pattern_rwlock) = -1;
...
static void
update_min_level(struct vlog_module *module) PAG_REQUIRES(&log_file_mutex)
{
                   ...
                   enum vlog_level level)
{
    ...
    pag_mutex_unlock(&log_file_mutex);
}
...
do_set_pattern(enum vlog_facility facility, const char *pattern)
{
    ...
    pag_rwlock_wrlock(&pattern_rwlock);
    ...
    pag_rwlock_unlock(&pattern_rwlock);
}
...
vlog_set_log_file(const char *file_name)
{
    ...
    new_log_file_name = (file_name
                         ? xstrdup(file_name)
                         : xasprintf("%s/%s.log", pag_logdir(), program_name));
    new_log_fd = open(new_log_file_name, O_WRONLY | O_CREAT | O_APPEND, 0666);
    if (new_log_fd < 0) {
        VLOG_WARN("failed to open %s for logging: %s",
                  new_log_file_name, pag_strerror(errno));
...
vlog_set_verbosity(const char *arg)
{
    if (arg) {
        ...
        if (msg) {
            pag_fatal(0, "processing \"%s\": %s", arg, msg);
        }
    } else {
...
static void
vlog_unixctl_list(struct unixctl_conn *conn, int argc PAG_UNUSED,
                  const char *argv[] PAG_UNUSED, void *aux PAG_UNUSED)
{
...
static void
vlog_unixctl_reopen(struct unixctl_conn *conn, int argc PAG_UNUSED,
                    const char *argv[] PAG_UNUSED, void *aux PAG_UNUSED)
{
    bool has_log_file;
    ...
    if (has_log_file) {
        ...
        if (error) {
            unixctl_command_reply_error(conn, pag_strerror(errno));
        } else {
            unixctl_command_reply(conn, NULL);
        }
    } else {
...
vlog_init(void)
{
    ...
    if (pagthread_once_start(&once)) {
         ...
         * log anything before calling pagthread_once_done() will deadlock. */
            ...
            const char *message, va_list args)
{
    ...
    if (log_to_console || log_to_syslog || log_to_file) {
        ...
        pag_rwlock_rdlock(&pattern_rwlock);
        if (log_to_console) {
                  ...
                  const char *message, va_list args)
{
     ...
     * message written by the later pag_fatal_valist(). */
    module->levels[VLF_CONSOLE] = VLL_OFF;

    vlog_valist(module, VLL_EMER, message, args);
    pag_fatal_valist(0, message, args);
}
                  ...
                  const char *message, va_list args)
{
    ...
    /* Don't log this message to the console to avoid redundancy with the
     * message written by the later pag_abort_valist(). */
    module->levels[VLF_CONSOLE] = VLL_OFF;

    vlog_valist(module, VLL_EMER, message, args);
    pag_abort_valist(0, message, args);
}
