...
{
    struct qtag_prefix {
        pag_be16 eth_type;      /* ETH_TYPE_VLAN */
        pag_be16 tci;
    };

    if (b->size >= sizeof(struct qtag_prefix) + sizeof(pag_be16)) {
...
parse_ethertype(struct ofpbuf *b)
{
    ...
    proto = *(pag_be16 *) ofpbuf_pull(b, sizeof proto);
...
parse_ipv6(struct ofpbuf *packet, struct flow *flow)
{
    ...
    while (1) {
        ...
        if ((nexthdr == IPPROTO_HOPOPTS)
            ...
            const struct pag_16aligned_ip6_frag *frag_hdr = packet->data;
...
flow_random_hash_fields(struct flow *flow)
{
    ...
    flow->vlan_tci = (PAG_FORCE pag_be16) (random_uint16() & VLAN_VID_MASK);

    /* Make most of the random flows IPv4, some IPv6, and rest random. */
    flow->dl_type = rnd < 0x8000 ? htons(ETH_TYPE_IP) :
        rnd < 0xc000 ? htons(ETH_TYPE_IPV6) : (PAG_FORCE pag_be16)rnd;

    if (dl_type_is_ip_any(flow->dl_type)) {
        if (flow->dl_type == htons(ETH_TYPE_IP)) {
            flow->nw_src = (PAG_FORCE pag_be32)random_uint32();
            flow->nw_dst = (PAG_FORCE pag_be32)random_uint32();
        } else {
        ...
        if (flow->nw_proto == IPPROTO_TCP ||
            ...
            flow->tp_src = (PAG_FORCE pag_be16)random_uint16();
            flow->tp_dst = (PAG_FORCE pag_be16)random_uint16();
        }
    }
}
...
void
flow_set_dl_vlan(struct flow *flow, pag_be16 vid)
{
 ...
 * plus CFI). */
void
flow_set_vlan_vid(struct flow *flow, pag_be16 vid)
{
    pag_be16 mask = htons(VLAN_VID_MASK | VLAN_CFI);
    flow->vlan_tci &= ~mask;
                              ...
                              struct flow_wildcards *wc)
{
    ...
    if (min_n == 0) {
        ...
        for (i = 0; i < min_n; i++) {
            if (wc) {
                wc->masks.mpls_lse[a_last - i] = PAG_BE32_MAX;
                wc->masks.mpls_lse[b_last - i] = PAG_BE32_MAX;
            }
 ...
 */
void
flow_push_mpls(struct flow *flow, int n, pag_be16 mpls_eth_type,
               struct flow_wildcards *wc)
{
    pag_assert(eth_type_mpls(mpls_eth_type));
    pag_assert(n < FLOW_MAX_MPLS_LABELS);
...
bool
flow_pop_mpls(struct flow *flow, int n, pag_be16 eth_type,
              struct flow_wildcards *wc)
{
    int i;
...
void
flow_set_mpls_label(struct flow *flow, int idx, pag_be32 label)
{
...
void
flow_set_mpls_lse(struct flow *flow, int idx, pag_be32 lse)
{
...
flow_compose_l4(struct ofpbuf *b, const struct flow *flow)
{
    if (!(flow->nw_frag & FLOW_NW_FRAG_ANY)
        ...
        if (flow->nw_proto == IPPROTO_TCP) {
            ...
            icmp->icmp6_cksum = (PAG_FORCE uint16_t)
...
flow_compose(struct ofpbuf *b, const struct flow *flow)
{
    ...
    if (flow->dl_type == htons(ETH_TYPE_IP)) {
        ...
        struct pag_16aligned_ip6_hdr *nh;
...
/* Returns the pag_be16 that would be at byte offset 'u8_ofs' if 'flow' were
...
miniflow_get_be16(const struct miniflow *flow, unsigned int u8_ofs)
{
    ...
    const pag_be16 *be16p = (const pag_be16 *) u32p;
...
{
    pag_be16 tci = miniflow_get_be16(flow, offsetof(struct flow, vlan_tci));
